...
struct AsciiOpenContext
{
	...
	void save(Ui_AsciiOpenDialog* ui)
	{
		...
		maxPointCountPerCloud = 2000000000;
	...
	, m_columnsCount(0)
{
	...
	setWindowTitle(tr("Open Settings"));
	...
	connect(m_ui->comboBox, qOverload<int>(&QComboBox::currentIndexChanged), this, &AsciiOpenDlg::onSeparatorIndexChange);
	m_ui->tableWidget->horizontalHeader()->setStyleSheet("QHeaderView::section{background-color:#484848;border: 1px solid #989898;height:32px;font-size:13px;color:#DDDDDD;}");
	QSize screenSize = QApplication::desktop()->screenGeometry().size();
	setMaximumSize(screenSize);
}

void AsciiOpenDlg::onSeparatorIndexChange(int index)
{
	if (!m_columnsCount)
		return;
...
static const unsigned DISPLAYED_LINES = 10;				//number of displayed lines
...
void AsciiOpenDlg::commaDecimalCheckBoxToggled(bool)
{
	...
	if (index == 0)
		...
		newSeparator = 59;
	onSeparatorChange(QString(newSeparator));
}
...
void AsciiOpenDlg::updateTable()
{
	...
	if (!m_headerLine.isEmpty())
	{
		...
		QString headertxt = QString("Header: ") + displayHeader;
		headertxt.replace(" ", "   ");
		m_ui->headerLabel->setText(headertxt);
	...
	//setup table and widgets
	{
		...
		int columnWidth = (850 * 9) / (columnsCount * 10);
			...
			for (unsigned i = 0; i < columnsCount; i++)
			{
				if (m_columnType[i] == TEXT)
				{
					if (!labelColumnAssigned && columnsCount > 1 && approximateTotalLineCount < s_maxLabelCount)
					{
						...
						assert(columnHeaderWidget);
						columnHeaderWidget->blockSignals(true);
						columnHeaderWidget->setCurrentIndex(ASCII_OPEN_DLG_None);
						columnHeaderWidget->blockSignals(false);
...
bool AsciiOpenDlg::CheckOpenSequence(const AsciiOpenDlg::Sequence& sequence, QString& errorMessage)
{
	...
	//check for doublons
	{
		for (size_t i = 1; i < ASCII_OPEN_DLG_Scalar; i++)
		{
			if (counters[i] > 1)
			{
				errorMessage = QString(tr("Scalars are defined at least twice."));
				return false;
			}
		}
	}
	...
	{
		errorMessage = tr("At least 2 vertex coordinates must be define.");
		return false;
	}
...
bool AsciiOpenDlg::apply()
{
	...
	{
		CS::Widgets::FramelessMessageBox::warning(nullptr, tr("Error"), errorMessage);
		return false;
	}
	else
	{
...
void AsciiOpenDlg::adjustSFIndex()
{
	switch (m_columnsCount)
	{
	case 4:
		updateParam(3, 19);
		break;
	...
	int columnsCount = m_ui->tableWidget->columnCount();
	int columnWidth = (850 * 9) / (columnsCount * 10);
	columnWidth = 840 / columnsCount;
	for (unsigned i = 0; i < columnsCount; i++)
	{
...
void AsciiOpenDlg::updateParam(int column,int index)
{
	if (m_columnsCount == 0)
	{
		return;
...
{
	return static_cast<unsigned>(floor(2000000000));
}
