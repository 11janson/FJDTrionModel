...
#include <QVariant>
#include "cswidgets/framelesswaitdialog.h"
...
{
	if (type == int64_t(CC_TYPES::POLY_LINE)
		||	type == int64_t(CC_TYPES::POINT_CLOUD))
	{
		multiple = true;
		exclusive = false;
		return true;
...
public:
	...
	void addCircle(const DL_CircleData& data) override
	{
		...
		poly->setName("Circle");
		poly->addPointIndex(0, vertexCount);
		poly->setClosed(true/*arcLength_deg >= 360.0*/);
...
CC_FILE_ERROR DxfFilter::saveToFile(ccHObject* root, const QString& filename, const SaveParameters& parameters)
{
	...
	CS::Widgets::FramelessWaitingdialog::instance()->setParent((QWidget*)(CS::Model::ProjectModel::instance()->getMainWindow()));
	CS::Widgets::FramelessWaitingdialog::instance()->startWaiting(QCoreApplication::translate("MainWindow", "Data is being saved, Please wait.", nullptr));
	...
	while (!fu.isFinished()) {
		QThread::msleep(20);
		CS::Widgets::FramelessWaitingdialog::instance((QWidget*)(CS::Model::ProjectModel::instance()->getMainWindow()))->jumpToNextFrame();
		QCoreApplication::processEvents();
	}
	CS::Widgets::FramelessWaitingdialog::instance((QWidget*)(CS::Model::ProjectModel::instance()->getMainWindow()))->stopWaiting();
	delete dw;
	dw = nullptr;
...
CC_FILE_ERROR DxfFilter::loadFile(const QString& filename, ccHObject& container, LoadParameters& parameters)
{
	...
	try
	{
...
#ifdef _WIN32
		DL_Dxf dxf;
		if (dxf.in(filename.toStdWString(), &importer))
#else
		if (CheckForSpecialChars(filename))
		{
...
#endif
		{
			...
			if (dxf.getValueBreakLoop())
			{
				result = CC_FERR_TYPE_CAD_DXF;
			}
		}
		else
		{
