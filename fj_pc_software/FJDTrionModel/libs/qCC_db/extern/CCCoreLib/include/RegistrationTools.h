...
namespace CCCoreLib
{
	...
	protected:
		...
		**/
        ...
        //Melone.Yang 基于负对称罗德里格矩阵的大旋转角坐标转换参数计算
        static bool RegistrationProcedureNew(GenericCloud* P,
            GenericCloud* X,
            ScaledTransformation& trans);

        //2022.12 Melone.Yang 已知四参数转换矩阵计算转换点云；P目标点云（新坐标）；X源点云（原始坐标）； trans，转换矩阵
        static CCVector3d PointCloudTrans(CCVector3d oriPoint,
            Eigen::Matrix3d  trans);

        //2022.12 Melone.Yang 平面坐标转换参数计算(四参数)  3 x 3 矩阵；P目标点云（新坐标）；X源点云（原始坐标）
        static bool PlaneCoordinateTrans(GenericCloud* P,
            GenericCloud* X,
            ScaledTransformation& trans);
	};
	...
	**/
	...
	public:
		...
		**/
        ...
        /** Warning: both clouds must have the same size (and at least 2 points)
            Output transformation is from the left (L) to the right (R) coordinate system
            \param lCloud left cloud {Pl}
            \param rCloud right cloud {Pr}
            \param trans resulting transformation: Pr = s.R.Pl + T
            ...
            平面坐标转换
        **/
        static bool FindPlaneAbsoluteOrientation(GenericCloud* lCloud,
            GenericCloud* rCloud,
            ScaledTransformation& trans,
            bool fixedScale = false);
