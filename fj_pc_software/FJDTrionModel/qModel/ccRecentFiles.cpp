...
ccRecentFiles::ccRecentFiles( QWidget *parent ) :
	QObject( parent ), m_settings(CS::Core::ICore::getDefaultPath() + "/config/config.ini", QSettings::IniFormat)
{
	//m_settings.setPath(QSettings::IniFormat, QSettings::SystemScope, QCoreApplication::applicationDirPath() + QString("/theme/config.ini"));
...
void ccRecentFiles::updateMenu()
{
	...
	{
		num++;
		QAction  *recentAction = new QAction(QString::number(num)+"-"+ contractFilePath( recentFile ), this );
...
void ccRecentFiles::openFileFromAction()
{
	...
	{
        CS::Widgets::FramelessMessageBox::critical(MainWindow::TheInstance(), tr("Error"), tr("File does not exist."));
		return;
	}
    ...
    ccGLWindow* pWin = MainWindow::TheInstance()->GetGLWindow("3D View");
    if (pWin)
    {
        pWin->redraw(false);
    }
