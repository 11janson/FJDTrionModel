...
bool IPlugin::installTranslator(const QString &qmPath,
	const QString &qmName,
	const QString &overrideLanguage)
{
	...
	bool bFindQM = false;
	// 加载制定的语言资源
	QString locale = "en";//默认英文
	if (!overrideLanguage.isEmpty())
		locale = overrideLanguage;
	const QString &TrPath = qmPath;

	if (m_Translator.load(QString("%1_").arg(qmName) + locale, TrPath)) {
	...
	{
		// 默认提供英文语言资源
		localeName = QLocale("en").name();
		const QString apolloQMName = QString("%1_").arg(qmName) + localeName;
		if (m_Translator.load(apolloQMName, TrPath))
		{
			qApp->installTranslator(&m_Translator);
			bFindQM = true;
		}
