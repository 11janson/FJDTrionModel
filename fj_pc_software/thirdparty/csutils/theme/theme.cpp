...
QString Theme::imageFile(const QString &imageKey, const QString &fallBack) const
{
    //const QString &file = d->imageFiles.at(imageFile);
    //return file.isEmpty() ? fallBack : file;
	QString img = fallBack;
	if (d->imageFiles.contains(imageKey))
	{
		const QString &file = d->imageFiles[imageKey];
		img = file.isEmpty() ? fallBack : file;
	}
	return img;
}
...
const QVariantMap &Theme::values() const
{
	if (d->values.isEmpty()) {
		...
		{
			QMapIterator<QString, QString> i(d->imageFiles);
			while (i.hasNext()) {
				i.next();
...
void Theme::readSettings(QSettings &settings)
{
    ...
    }
    {
        ...
        //settings.endGroup();
		foreach(const QString &key, settings.allKeys())
			d->imageFiles[key] = settings.value(key).toString();
		settings.endGroup();
    }
    {
