...
CC_FILE_ERROR PlyFilter::saveToFile(ccHObject* entity, const QString& filename, const SaveParameters& parameters)
{
	...
	{
        CS::Widgets::FramelessMessageBox msgBox(QMessageBox::Question, QCoreApplication::translate("PlyFilter", "Select output format", nullptr),
            QCoreApplication::translate("PlyFilter", "Is it saved in binary or ASCII format?", nullptr),
            QMessageBox::Cancel | QMessageBox::Retry | QMessageBox::Yes,
            nullptr);
        msgBox.changeButtonText(QDialogButtonBox::StandardButton::Yes, QCoreApplication::translate("PlyFilter", "Binary", nullptr));
        msgBox.changeButtonText(QDialogButtonBox::StandardButton::Retry, QCoreApplication::translate("PlyFilter", "ASCII", nullptr));
        msgBox.setButtonIsVisiable(QDialogButtonBox::StandardButton::Cancel,false);
        auto result = msgBox.exec();
        if (result == QMessageBox::Retry)
        {
            outputFormat = PLY_ASCII;
        }
        else if (result == QMessageBox::Yes)
        {
            outputFormat = PLY_DEFAULT;
        }
        else
        {
		...
		//CS::Widgets::FramelessMessageBox msgBox(QMessageBox::Question, QCoreApplication::translate("PlyFilter", "Choose output format", nullptr), QCoreApplication::translate("PlyFilter", "Save in BINARY or ASCII format?", nullptr));
		//msgBox.addButton(QCoreApplication::translate("PlyFilter", "BINARY", nullptr), QMessageBox::AcceptRole);
		//QPushButton *asciiButton = msgBox.addButton(QCoreApplication::translate("PlyFilter", "ASCII", nullptr), QMessageBox::AcceptRole);
		//msgBox.exec();
		//outputFormat = msgBox.clickedButton() == asciiButton ? PLY_ASCII : PLY_DEFAULT;
	}

	return saveToFile(entity, filename, outputFormat, parameters);
}

CC_FILE_ERROR PlyFilter::saveToFile(ccHObject* entity, QString filename, e_ply_storage_mode storageType, const SaveParameters& parameters)
{
			...
			if (textureCount == 1)
			{
				...
				else if (materials->size() > 1)
				{
					...
					else
					{
						...
						{
							if (CS::Widgets::FramelessMessageBox::question(	nullptr,
    ...
    if (vertices)
    {
        ...
        if (parameters.parentWidget)
        {
            ...
            pDlg->setMethodTitle(QCoreApplication::translate("PlyFilter", "Saving cloud", nullptr) 
                + (vertices->getName().length() > 30 ? "[" + vertices->getName().left(30) + "...]" : "[" + vertices->getName() + "...]"));
            pDlg->setInfo(QCoreApplication::translate("PlyFilter", "Number of points:", nullptr) + QString::number(vertCount));
            pDlg->start();
        }
        CCCoreLib::NormalizedProgress nprogress(pDlg.data(), vertCount);
    }
    CCCoreLib::NormalizedProgress nprogress(pDlg.data(), vertCount);
	//[!]对于ply保存进行校验，暂时认为如果保存到文件中点的数量和点云一致认为正常
	unsigned long int pointSize = 0;
	...
	if (mesh)
	{
        ...
        if (parameters.parentWidget)
        {
            pDlg.reset(new ccProgressDialog(true, parameters.parentWidget));
            pDlg->setWindowFlags(Qt::Window | Qt::FramelessWindowHint);
            pDlg->setMethodTitle(QCoreApplication::translate("PlyFilter", "Saving mesh", nullptr) 
                + (mesh->getName().length() > 30 ? "[" + mesh->getName().left(30) + "...]" : "[" + mesh->getName() + "...]"));
            pDlg->setInfo(QCoreApplication::translate("PlyFilter", "Number of triangles:", nullptr) + QString::number(triNum));
            pDlg->start();
        }
        CCCoreLib::NormalizedProgress nprogress(pDlg.data(), triNum);
	...
	if (pointSize != vertCount && !triNum)
	{
		qDebug() << "PLYERROR:save ply file but input file point size !=  CloudPoint size,so put error!";
	...
	qDebug() << "pointIndex:" << pointSize << "pointCloud size "<< vertCount;
...
CC_FILE_ERROR PlyFilter::loadFile(const QString& filename, const QString& inputTextureFilename, ccHObject& container, LoadParameters& parameters)
{
	...
	stdPropsText << QString("None");
	{
		for (int i = 1; i <= static_cast<int>(stdProperties.size()); ++i)
		{
			...
			if (nxIndex == 0 && (propName.contains("NX", Qt::CaseInsensitive) || (elementName.contains("NORM", Qt::CaseInsensitive) && propName.endsWith("X", Qt::CaseInsensitive))))
				nxIndex = i;
			else if (nyIndex == 0 && (propName.contains("NY", Qt::CaseInsensitive) || (elementName.contains("NORM", Qt::CaseInsensitive) && propName.endsWith("Y", Qt::CaseInsensitive))))
				nyIndex = i;
			else if (nzIndex == 0 && (propName.contains("NZ", Qt::CaseInsensitive) || (elementName.contains("NORM", Qt::CaseInsensitive) && propName.endsWith("Z", Qt::CaseInsensitive))))
				nzIndex = i;
			else if (rIndex == 0 && (propName.contains("RED", Qt::CaseInsensitive) || (elementName.contains("COL", Qt::CaseInsensitive) && propName.endsWith("R", Qt::CaseInsensitive)) || propName.compare("r", Qt::CaseInsensitive) == 0))
				rIndex = i;
			else if (gIndex == 0 && (propName.contains("GREEN", Qt::CaseInsensitive) || (elementName.contains("COL", Qt::CaseInsensitive) && propName.endsWith("G", Qt::CaseInsensitive)) || propName.compare("g", Qt::CaseInsensitive) == 0))
				gIndex = i;
			else if (bIndex == 0 && (propName.contains("BLUE", Qt::CaseInsensitive) || (elementName.contains("COL", Qt::CaseInsensitive) && propName.endsWith("B", Qt::CaseInsensitive)) || propName.compare("b", Qt::CaseInsensitive) == 0))
				bIndex = i;
			else if (iIndex == 0 && (propName.contains("INTENSITY", Qt::CaseInsensitive) || propName.contains("GRAY", Qt::CaseInsensitive) || propName.contains("GREY", Qt::CaseInsensitive) || (elementName.contains("COL", Qt::CaseInsensitive) && propName.endsWith("I", Qt::CaseInsensitive))))
			{
				//iIndex = i; //DGM: we don't load the intensities as RGB colors anymore!
				sfPropIndexes.push_back(i);
			}
			else if (elementName.contains("VERT", Qt::CaseInsensitive) || elementName.contains("POINT", Qt::CaseInsensitive))
			{
				if (propName.contains("time", Qt::CaseInsensitive))
				//if (propName.contains("SCAL", Qt::CaseInsensitive))
					sfPropIndexes.push_back(i);
				else if (xIndex == 0 && propName.endsWith("X", Qt::CaseInsensitive))
					xIndex = i;
				else if (yIndex == 0 && propName.endsWith("Y", Qt::CaseInsensitive))
					yIndex = i;
				else if (zIndex == 0 && propName.endsWith("Z", Qt::CaseInsensitive))
					zIndex = i;
			}
			else if (propName.contains("SCAL", Qt::CaseInsensitive) || propName.contains("VAL", Qt::CaseInsensitive))
				sfPropIndexes.push_back(i);
		}
	}
	...
	//WaitingDialog::MetahublFramelessWaitingdialog::instance(parameters.parentWidget)->startWaiting();
	ccProgressDialog* npDlg = nullptr;
	if (parameters.parentWidget)
	{
		if (parameters.progress)
		{
			npDlg = parameters.progress;
		}
		else
		{
			npDlg = new ccProgressDialog(true, parameters.parentWidget);
		}
		//pDlg->setInfo(QObject::tr("Loading in progress..."));
		//pDlg->setMethodTitle(QObject::tr("Open PLY file"));
		npDlg->setAutoClose(false);
		npDlg->setAutoReset(false);
		npDlg->setInfo(QCoreApplication::translate("PlyFilter", "Loading in progress...", nullptr));
		npDlg->setMethodTitle(QCoreApplication::translate("PlyFilter", "Opening PLY file", nullptr));
		npDlg->setRange(0, 100);
		npDlg->setValue(0);
		npDlg->setWindowFlags(Qt::Window | Qt::FramelessWindowHint);
		npDlg->start();
	...
	Sleep(200);
	//WaitingDialog::MetahublFramelessWaitingdialog::instance(parameters.parentWidget)->stopWaiting();
	return CC_FERR_NO_ERROR;
}
