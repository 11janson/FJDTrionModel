...
namespace Widgets {
/*
* @brief 存储界面对应的数据集合，以及是否可移动、可缩放属性
*
*/
...
public:
    FramelessHelperPrivate()
        ...
        ,m_bRubberBandOnResize(false)
        ,m_bRubberBandOnMove(false)
    {

    }
public:
    QHash<QWidget*, WidgetData*> m_widgetDataHash;
    bool m_bWidgetMovable;//设置窗体可移动
    bool m_bWidgetResizable;//设置窗体可缩放
    bool m_bRubberBandOnResize;//设置橡皮筋效果-可移动
    bool m_bRubberBandOnMove; //设置橡皮筋效果-可缩放
};
...
void WidgetData::handleMousePressEvent(QMouseEvent *event)
{
    if (event->button() == Qt::LeftButton)
    {
        ...
        m_bLeftButtonTitlePressed = event->pos().y() < m_moveMousePos.m_nTitleHeight + m_topShadowPainterHeight;
        ...
        {
            frameRect.setX(frameRect.x() + 10);
            frameRect.setY(frameRect.y() + 10);
            m_pRubberBand->setGeometry(frameRect);
            m_pRubberBand->show();
        }
    }
