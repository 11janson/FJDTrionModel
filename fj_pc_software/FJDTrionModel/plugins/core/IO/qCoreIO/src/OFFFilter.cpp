...
CC_FILE_ERROR OFFFilter::saveToFile(ccHObject* entity, const QString& filename, const SaveParameters& parameters)
{
    ...
    if (vertices)
    {
        ...
        if (parameters.parentWidget)
        {
            ...
            pDlg->setMethodTitle(QCoreApplication::translate("OFFFilter", "Saving cloud", nullptr) 
                + (vertices->getName().length() > 30 ? "[" + vertices->getName().left(30) + "...]" : "[" + vertices->getName() + "...]"));
            pDlg->setInfo(QCoreApplication::translate("OFFFilter", "Number of points:", nullptr) + QString::number(vertCount));
            pDlg->start();
        }
    ...
    if (mesh)
    {
        ...
        if (parameters.parentWidget)
        {
            pDlg.reset(new ccProgressDialog(true, parameters.parentWidget));
            pDlg->setWindowFlags(Qt::Window | Qt::FramelessWindowHint);
            pDlg->setMethodTitle(QCoreApplication::translate("OFFFilter", "Saving mesh", nullptr) 
                + (mesh->getName().length() > 30 ? "[" + mesh->getName().left(30) + "...]" : "[" + mesh->getName() + "...]"));
            pDlg->setInfo(QCoreApplication::translate("OFFFilter", "Number of triangles:", nullptr) + QString::number(triCount));
            pDlg->start();
        }
        CCCoreLib::NormalizedProgress nprogress(pDlg.data(), triCount);
...
CC_FILE_ERROR OFFFilter::loadFile(const QString& filename, ccHObject& container, LoadParameters& parameters)
{
    ...
    if(vertices)
	{
        ...
        if (parameters.parentWidget)
        {
            pDlg.reset(new ccProgressDialog(true, parameters.parentWidget));
            pDlg->setWindowFlags(Qt::Window | Qt::FramelessWindowHint);
            pDlg->setMethodTitle(QCoreApplication::translate("OFFFilter", "Loading cloud", nullptr) + "[" + vertices->getName() + "]");
            pDlg->setInfo(QCoreApplication::translate("OFFFilter", "Number of points:", nullptr) + QString::number(vertCount));
            pDlg->start();
        }
        CCCoreLib::NormalizedProgress nprogress(pDlg.data(), vertCount);
    ...
    if (mesh)
    {
        ...
        if (parameters.parentWidget)
        {
            pDlg.reset(new ccProgressDialog(true, parameters.parentWidget));
            pDlg->setWindowFlags(Qt::Window | Qt::FramelessWindowHint);
            pDlg->setMethodTitle(QCoreApplication::translate("OFFFilter", "Loading mesh", nullptr) + "[" + mesh->getName() + "]");
            pDlg->setInfo(QCoreApplication::translate("OFFFilter", "Number of triangles:", nullptr) + QString::number(triCount));
            pDlg->start();
        }
        CCCoreLib::NormalizedProgress nprogress(pDlg.data(), triCount);
