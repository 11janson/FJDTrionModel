    ...
    , m_triNormalIndexes(nullptr)
{
	...
	ccColor::Rgba yellowcolor(255, 200, 4, 255);
    setTempColor(yellowcolor, false);
}
    ...
    ccProgressDialog* progressCb/*=nullptr*/)
{
    ...
    if (progressCb)
    {
        progressCb->setMethodTitle(QCoreApplication::translate("ccMesh", "Smoothing...", nullptr));
        progressCb->setInfo(QCoreApplication::translate("ccMesh", "Vertex: %1", nullptr).arg(vertCount));
        progressCb->start();
    }
...
ccMesh* ccMesh::Triangulate(ccGenericPointCloud* inputCloud,
    ...
    unsigned char dim/*=2*/)
{
    ...
    //将原始点云和tin解除关联，防止平滑时改变原始点云
    ccPointCloud* cloud(new ccPointCloud);
    *cloud = *static_cast<ccPointCloud*>(inputCloud);
...
void ccMesh::drawMeOnly(CC_DRAW_CONTEXT& context)
{
    ...
    if (MACRO_Draw3D(context))
    {
        if (m_bDrawImage) {
            //[!].绘制数据到图片上
            if (getClassID() != CC_TYPES::SPHERE && getClassID() != CC_TYPES::CYLINDER && getClassID() != CC_TYPES::TORUS && getClassID() != CC_TYPES::CONE)
            {
                drawImage(context);
                m_bDrawImage = false;
            }
...
ccMesh* ccMesh::createNewMeshFromSelection(bool removeSelectedFaces)
{
    ...
    QString oldname = m_associatedCloud->getName();
    ...
    if (result)
    {
        ...
        {
            newMesh->setName(getName() + QString(".segmented"));
            ...
            newVertices->setName(oldname + ".part");
            newVertices->setEnabled(false);
        }
...
void ccMesh::drawData(CC_DRAW_CONTEXT & context)
{
    //get the set of OpenGL functions (version 2.1)
    QOpenGLFunctions_2_1* glFunc = context.glFunctions<QOpenGLFunctions_2_1>();
    assert(glFunc != nullptr);
    ...
    //L.O.D.
    bool lodEnabled = (triNum > context.minLODTriangleCount && context.isShowLodState && MACRO_LODActivated(context));
...
void ccMesh::drawDataNew(CC_DRAW_CONTEXT & context)
{
    //get the set of OpenGL functions (version 2.1)
    QOpenGLFunctions_2_1* glFunc = context.glFunctions<QOpenGLFunctions_2_1>();
    ...
    bool lodEnabled = (triNum > context.minLODTriangleCount && context.isShowLodState && MACRO_LODActivated(context));
    unsigned decimStep = (lodEnabled ? static_cast<unsigned>(ceil(static_cast<double>(triNum * 3) / context.minLODTriangleCount)) : 1);
    //[!]这里的类型是裁切盒的图元，不受限于mesh的背面强制颜色调整
    bool drawTemplateColor = getClassID() == CC_TYPES::SPHERE || getClassID() == CC_TYPES::CYLINDER || getClassID() == CC_TYPES::TORUS || getClassID() == CC_TYPES::CONE || getClassID() == CC_TYPES::CLIPPING_BOX_CUBE;
    //display parameters
    glDrawParams glParams;
    getDrawingParameters(glParams);
    //no normals shading without light!
    ...
    int associatedCloudSize = m_associatedCloud->size();

    if (!m_vertexArrayManager.m_isInitVBO)
    {
        m_vertexArrayManager.vertices.clear();
        m_vertexArrayManager.colors.clear();
        m_vertexArrayManager.indices.clear();
        m_vertexArrayManager.visibilityArray.clear();
        for (const auto & curindex : *m_triVertIndexes)
        {
            m_vertexArrayManager.indices.push_back(curindex.i1);
            m_vertexArrayManager.indices.push_back(curindex.i2);
            m_vertexArrayManager.indices.push_back(curindex.i3);
        }
        if (glParams.showSF)
        {
            ...
            {
                const ccColor::Rgb* sfrgb = currentDisplayedScalarField->getValueColor(j);
                m_vertexArrayManager.vertices.push_back(m_associatedCloud->getPoint(j)->x);
                m_vertexArrayManager.vertices.push_back(m_associatedCloud->getPoint(j)->y);
                m_vertexArrayManager.vertices.push_back(m_associatedCloud->getPoint(j)->z);
                m_vertexArrayManager.colors.push_back(sfrgb->r / 255.0);
                m_vertexArrayManager.colors.push_back(sfrgb->g / 255.0);
                m_vertexArrayManager.colors.push_back(sfrgb->b / 255.0);
                m_vertexArrayManager.visibilityArray.push_back(CCCoreLib::POINT_VISIBLE);
            }
        }
        else
        {
            if (m_associatedCloud->hasColors() && !drawTemplateColor)
            {
                for (int j = 0; j < associatedCloudSize; j++)
                {
                    m_vertexArrayManager.vertices.push_back(m_associatedCloud->getPoint(j)->x);
                    m_vertexArrayManager.vertices.push_back(m_associatedCloud->getPoint(j)->y);
                    m_vertexArrayManager.vertices.push_back(m_associatedCloud->getPoint(j)->z);
                    m_vertexArrayManager.colors.push_back(m_associatedCloud->getPointColor(j).r / 255.0);
                    m_vertexArrayManager.colors.push_back(m_associatedCloud->getPointColor(j).g / 255.0);
                    m_vertexArrayManager.colors.push_back(m_associatedCloud->getPointColor(j).b / 255.0);
                    m_vertexArrayManager.visibilityArray.push_back(CCCoreLib::POINT_VISIBLE);
                }
            }
            else
            {
                ...
                for (int j = 0; j < associatedCloudSize; j++)
                {
                    ...
                    m_vertexArrayManager.vertices.push_back(m_associatedCloud->getPoint(j)->y);
                    m_vertexArrayManager.vertices.push_back(m_associatedCloud->getPoint(j)->z);
                    m_vertexArrayManager.colors.push_back(tempColor.r / 255.0);
                    m_vertexArrayManager.colors.push_back(tempColor.g / 255.0);
                    m_vertexArrayManager.colors.push_back(tempColor.b / 255.0);
                    m_vertexArrayManager.visibilityArray.push_back(CCCoreLib::POINT_VISIBLE);
                }
            }
        ...
        {
            std::copy(verticesVisibility.begin(), verticesVisibility.begin() + associatedCloudSize, m_vertexArrayManager.visibilityArray.begin());
        }
        // 假设有一个包含一亿多个三角形网格的顶点、色彩、索引、法向量和可视性数组
        ...
        glFunc3->glGenBuffers(1, &m_vertexArrayManager.NBO);


        glFunc3->glBindVertexArray(m_vertexArrayManager.VAO);

        glFunc3->glBindBuffer(GL_ARRAY_BUFFER, m_vertexArrayManager.VBO);
        glFunc3->glBufferData(GL_ARRAY_BUFFER, m_vertexArrayManager.vertices.size() * sizeof(float), m_vertexArrayManager.vertices.data(), GL_STATIC_DRAW);
        // 关联位置信息到第一个顶点属性
        ...
        // 关联颜色信息到第二个顶点属性
        glFunc3->glVertexAttribPointer(1, 3, GL_FLOAT, GL_FALSE, 3 * sizeof(float), (void*)0);
        glFunc3->glEnableVertexAttribArray(1);

        glFunc3->glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, m_vertexArrayManager.EBO);
        glFunc3->glBufferData(GL_ELEMENT_ARRAY_BUFFER, m_vertexArrayManager.indices.size() * sizeof(unsigned int), m_vertexArrayManager.indices.data(), GL_STATIC_DRAW);

        glFunc3->glBindBuffer(GL_ARRAY_BUFFER, m_vertexArrayManager.NBO);
        glFunc3->glBufferData(GL_ARRAY_BUFFER, m_vertexArrayManager.visibilityArray.size() * sizeof(unsigned char), m_vertexArrayManager.visibilityArray.data(), GL_STATIC_DRAW);

        glFunc3->glVertexAttribPointer(2, 1, GL_UNSIGNED_BYTE, GL_FALSE, sizeof(unsigned char), (void*)0);
        glFunc3->glEnableVertexAttribArray(2);
        ...
        bool is_nbo_created = (glFunc3->glIsBuffer(m_vertexArrayManager.NBO) == GL_TRUE);
        glFunc3->glBindVertexArray(0);
        m_vertexArrayManager.m_isInitVBO = true;
    }
    if (m_vertexArrayManager.m_isNeedUpdateVisibilityArray)
    {
        if (verticesVisibility.size() > 0 && visFiltering)
        {
            std::copy(verticesVisibility.begin(), verticesVisibility.begin() + associatedCloudSize, m_vertexArrayManager.visibilityArray.begin());
        ...
        {
            std::fill(m_vertexArrayManager.visibilityArray.begin(), m_vertexArrayManager.visibilityArray.end(), CCCoreLib::POINT_VISIBLE);
        }
        glFunc3->glBindBuffer(GL_ARRAY_BUFFER, m_vertexArrayManager.NBO);
        glFunc3->glBufferSubData(GL_ARRAY_BUFFER, 0, m_vertexArrayManager.visibilityArray.size() * sizeof(unsigned char), m_vertexArrayManager.visibilityArray.data());
    ...
    if (m_vertexArrayManager.m_isNeedUpdateVertices)
    {
        ...
        glFunc3->glBindBuffer(GL_ARRAY_BUFFER, m_vertexArrayManager.VBO);
        glFunc3->glBufferSubData(GL_ARRAY_BUFFER, 0, m_vertexArrayManager.vertices.size() * sizeof(float), m_vertexArrayManager.vertices.data());
        m_vertexArrayManager.m_isNeedUpdateVertices = false;
    }
    if (m_vertexArrayManager.m_isNeedUpdateColor || m_vertexArrayManager.m_isShowSf != glParams.showSF)
    {
        m_vertexArrayManager.colors.clear();
        if (glParams.showSF)
        {
    ...
    glFunc3->glEnable(GL_DEPTH_TEST);

    //glFunc3->glEnable(GL_CULL_FACE);     
    //glFunc3->glCullFace(GL_BACK);       
    ...
    glFunc3->glUseProgram(context.meshNormalShaderProgram);
    // 获取 uniform 变量位置
    int viewLoc = -1;
    int projectionLoc = -1;
    ...
    int boxMatrixLoc = -1;
    viewLoc = glFunc3->glGetUniformLocation(context.meshNormalShaderProgram, "view");
    projectionLoc = glFunc3->glGetUniformLocation(context.meshNormalShaderProgram, "projection");
    lightPositionLoc = glFunc3->glGetUniformLocation(context.meshNormalShaderProgram, "lightPosition");
    ambientColorLoc = glFunc3->glGetUniformLocation(context.meshNormalShaderProgram, "ambientColor");
    diffuseColorLoc = glFunc3->glGetUniformLocation(context.meshNormalShaderProgram, "diffuseColor");
    diffuseCoefficientLoc = glFunc3->glGetUniformLocation(context.meshNormalShaderProgram, "diffuseCoefficient");
    modelviewLoc = glFunc3->glGetUniformLocation(context.meshNormalShaderProgram, "modelview");
    showNormalLoc = glFunc3->glGetUniformLocation(context.meshNormalShaderProgram, "showNormal");
    useclipboxLoc = glFunc3->glGetUniformLocation(context.meshNormalShaderProgram, "useclipbox");
    boxmincornerLoc = glFunc3->glGetUniformLocation(context.meshNormalShaderProgram, "boxmincorner");
    boxmaxcornerLoc = glFunc3->glGetUniformLocation(context.meshNormalShaderProgram, "boxmaxcorner");
    boxMatrixLoc = glFunc3->glGetUniformLocation(context.meshNormalShaderProgram, "boxMatrix");
    ...
    //glFunc3->glUniform3f(lightPositionLoc, -1000, -1000, 1000);
    //glFunc3->glUniform3f(ambientColorLoc, ambiant[0], ambiant[1], ambiant[2]);
    glFunc3->glUniform3f(ambientColorLoc, 0.8, 0.8, 0.8);//环境光
    //glFunc3->glUniform3f(diffuseColorLoc, diffuse[0], diffuse[1], diffuse[2]);
    glFunc3->glUniform3f(diffuseColorLoc, 0.7, 0.7, 0.7);//漫反射光
    glFunc3->glUniform1f(diffuseCoefficientLoc, 0.8);
    ...
    if (!showWired)
    {
        glFunc3->glDrawElements(GL_TRIANGLES, m_vertexArrayManager.indices.size(), GL_UNSIGNED_INT, 0);
    }
    else
    {
        glFunc3->glDrawElements(GL_LINES, m_vertexArrayManager.indices.size(), GL_UNSIGNED_INT, 0);
    }
    delete[] mvparams;
    ...
    int success;
    glFunc3->glGetProgramiv(context.meshNormalShaderProgram, GL_LINK_STATUS, &success);
    if (!success)
    {
        int logLength;
        char message[1024];
        glFunc3->glGetProgramiv(context.meshNormalShaderProgram, GL_INFO_LOG_LENGTH, &logLength);
        if (logLength > 0)
        {
            glFunc3->glGetProgramInfoLog(context.meshNormalShaderProgram, logLength, NULL, message);
        }
    }
...
void ccMesh::drawImage(CC_DRAW_CONTEXT & context)
{
    QOpenGLFunctions_3_0* glFunc1 = context.glFunctions<QOpenGLFunctions_3_0>();
    if (!glFunc1) {
        return;
    }
    //// 创建帧缓存数据
    int width = context.glW;
    int height = context.glH;

    try {
        // 创建帧缓存对象
        GLuint frameBuffer;
        glFunc1->glGenFramebuffers(1, &frameBuffer);
        glFunc1->glBindFramebuffer(GL_FRAMEBUFFER, frameBuffer);

        // 创建纹理附件
        GLuint texture;
        glFunc1->glGenTextures(1, &texture);
        glFunc1->glBindTexture(GL_TEXTURE_2D, texture);
        glFunc1->glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, width, height, 0, GL_RGBA, GL_UNSIGNED_BYTE, NULL);
        glFunc1->glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, texture, 0);
        drawDataNew(context);
        //// 保存图片数据
        std::vector<uint8_t> buffer(width * height * 4);
        glFunc1->glGetTexImage(GL_TEXTURE_2D, 0, GL_RGBA, GL_UNSIGNED_BYTE, buffer.data());
        QImage image(buffer.data(), width, height, QImage::Format_RGBA8888);
        m_MeshImage = image.copy(0, 0, width, height);
        glFunc1->glBindFramebuffer(GL_FRAMEBUFFER, 0);
        glFunc1->glDeleteFramebuffers(1, &frameBuffer);
        glFunc1->glDeleteTextures(1, &texture);
        glFunc1->glBindFramebuffer(GL_FRAMEBUFFER, context.activeFboid);
    }
    catch (...) {
        qDebug() << " error!, create mesh frme image error!";
    }
    return;
}

QImage ccMesh::createMeshImage(void)
{
    return m_MeshImage;
}
...
bool ccMesh::SufaceMesh(ccGenericPointCloud* cloud,
...
int ccMesh::fillHoles(double max_hole_diam, int max_num_hole_edges, std::string tempFile, int& percent)
{
...
void ccMesh::deleteVertexArrays()
{
    ...
    if (m_vertexArrayManager.m_isInitVBO)
    {
        ...
        FJStyleManager::Instance()->deleteBuffers(m_vertexArrayManager.NBO);
