	...
	if (LargeButton == m_buttonType) {
        ...
        if (((!hasArrow && opt.icon.isNull()) && !opt.text.isEmpty()) || (opt.toolButtonStyle == Qt::ToolButtonTextOnly)) {
            ...
            if (opt.toolButtonStyle != Qt::ToolButtonIconOnly) {
				...
				//此处两行英文有遮挡，文字绘制区域加高5px
				QRect ItemTextRect = textRect;
				ItemTextRect.setHeight(textRect.height() + 5);
                //再绘制文本，对于Normal模式下的Largebutton，如果有菜单，且m_isWordWrap是true，箭头将在文本旁边
                style()->drawItemText(&p,
                                      QStyle::visualRect(opt.direction, opt.rect, ItemTextRect),
                                      alignment,
                                      opt.palette,
        ...
        if (!opt.icon.isNull()) {
            ...
            style()->drawItemText(&p,
                                  QStyle::visualRect(opt.direction, opt.rect, opt.rect.adjusted(2, 2, -2, -2)),
                                  alignment,
                                  opt.palette,
                                  opt.state & QStyle::State_Enabled,
                                  opt.text,
	...
	if (LargeButton == m_buttonType) {
        ...
        if (opt.toolButtonStyle != Qt::ToolButtonIconOnly) {
            ...
            m_textRect.setRect(m_iconAndTextSpace,
                               ...
                               m_iconRect.width(),
                               opt.rect.bottom() - m_iconRect.bottom() - m_iconAndTextSpace-5  // 高度是矩形底部减去icon底部再减去2*Space
            );
            //保证能显示两行文本
            //下面确定文字区域
            if ((opt.features & QStyleOptionToolButton::Menu) || (opt.features & QStyleOptionToolButton::HasMenu)) {
				...
				m_iconRect.adjust(-SA_INDICATOR_ARROW_WIDTH, 0, 0, 0);
        ...
        if (opt.toolButtonStyle == Qt::ToolButtonIconOnly) {
			...
			m_textRect.adjust(-3, 0, 0, 0);
			m_iconRect.adjust(-3, 0, 0, 0);
        }
    }
...
QRect SARibbonToolButton::calcIndicatorArrowDownRect(const QStyleOptionToolButton& opt)
{
    ...
    if (LargeButton == m_buttonType) {
        if ((Lite == m_largeButtonType && !s_liteStyleEnableWordWrap)) {
            ...
            //菜单的下拉箭头位于第二行文本的矩形区域
			rect.setRect(this->rect().right() - SA_INDICATOR_ARROW_WIDTH - 5,
				this->rect().top() + 20,
