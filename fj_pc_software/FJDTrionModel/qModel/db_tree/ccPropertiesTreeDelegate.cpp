...
const char* ccPropertiesTreeDelegate::s_AutoPointSizeString = QT_TR_NOOP("Auto");
...
void ccPropertiesTreeDelegate::fillModel(ccHObject* hObject)
{
	...
	if (m_model)
	{
		...
		m_model->setHeaderData(0, Qt::Horizontal, QCoreApplication::translate("ccPropertiesTreeDelegate", "Property", nullptr));
		m_model->setHeaderData(1, Qt::Horizontal, QCoreApplication::translate("ccPropertiesTreeDelegate", "State/Value", nullptr));
	}
...
void ccPropertiesTreeDelegate::addSeparator(const QString& title)
{
	if (m_model)
	{
		...
		//QBrush brush(QColor(245,213,0));
		//leftItem->setBackground(brush);
...
void ccPropertiesTreeDelegate::fillWithHObject(ccHObject* _obj)
{
	...
	addSeparator(QCoreApplication::translate("ccPropertiesTreeDelegate", "Cloud Object", nullptr));

	//name
	appendRow(ITEM(QCoreApplication::translate("ccPropertiesTreeDelegate", "Name", nullptr)), ITEM(_obj->getName(), Qt::ItemIsEditable, OBJECT_NAME));

	//visibility
	if (!_obj->isVisibilityLocked())
	{
		appendRow(ITEM(QCoreApplication::translate("ccPropertiesTreeDelegate", "Visible", nullptr)), CHECKABLE_ITEM(_obj->isVisible(), OBJECT_VISIBILITY));
	}

	//normals
	if (_obj->hasNormals())
	{
		appendRow(ITEM(QCoreApplication::translate("ccPropertiesTreeDelegate", "Normals", nullptr)), CHECKABLE_ITEM(_obj->normalsShown(), OBJECT_NORMALS_SHOWN));
	}
	...
	//Bounding-box
	{
		...
		if (box.isValid())
		{
			...
			appendRow(ITEM(fitBBox ? tr( "Local box dimensions" ) : tr( "Box dimensions" )),
				ITEM(QString("X: %0\tY: %1\tZ: %2").arg(bboxDiag.x, 0, 'f', 6).arg(bboxDiag.y, 0, 'f', 6).arg(bboxDiag.z, 0, 'f', 6)));
			...
			appendRow(ITEM(shiftedObj? tr("Shifted box center") : tr("Box center")),
				ITEM(QString("X: %0\tY: %1\tZ: %2").arg(bboxCenter.x, 0, 'f', 6).arg(bboxCenter.y, 0, 'f', 6).arg(bboxCenter.z, 0, 'f', 6)));
			...
			if (shiftedObj)
			{
				...
				appendRow(ITEM(tr("Global box center")),
					ITEM(QString("X: %0\tY: %1\tZ: %2").arg(globalBBoxCenter.x, 0, 'f', 6).arg(globalBBoxCenter.y, 0, 'f', 6).arg(globalBBoxCenter.z, 0, 'f', 6)));
			}
		}
	}
...
void ccPropertiesTreeDelegate::fillWithCoordinateSystem(const ccCoordinateSystem* _obj)
{
	...
	appendRow(ITEM(tr("Origin")),
		ITEM(QStringLiteral("X: %0\tY: %1\tZ: %2").arg(origin.x).arg(origin.y).arg(origin.z)));
...
void ccPropertiesTreeDelegate::fillSFWithPointCloud(ccGenericPointCloud* _obj)
{
	...
	if (sfCount != 0)
	{
		...
		if (sf)
		{
			...
			//sf按照类别显示时隐藏界面
			if (currentSFName.compare("classification", Qt::CaseInsensitive) != 0 && currentSFName.compare("classification * gpstime", Qt::CaseInsensitive) != 0 && currentSFName.compare("classification * coord. z", Qt::CaseInsensitive) != 0 && currentSFName.compare("classification * intensity", Qt::CaseInsensitive) != 0)
			{
				addSeparator(tr("Color Scale"));

				//color scale selection combo box
				appendRow(ITEM(tr("Active")), PERSISTENT_EDITOR(OBJECT_CURRENT_COLOR_RAMP), true);
			...
			else
			{
				if (m_currentObject)
				{
					...
					if (cloud)
					{
						cloud->getCurrentDisplayedScalarField()->showNaNValuesInGrey(false);
					}
				}
			}
		}
	...
	const QModelIndex &index) const
{
	...
	case OBJECT_CURRENT_DISPLAY:
	{
		...
		{
			//105功能临时注释
			if (glWindow->objectName() == "viewer3d")
			{
	...
	case OBJECT_COLOR_RAMP_STEPS:
	{
		...
		palette.setColor(QPalette::Highlight, QColor(43, 43, 43));
	...
	case TREE_VIEW_HEADER:
	{
		...
		headerLabel->setStyleSheet("QLabel { background-color : #484848; color : white; }");

		//no signal connection, it's a display-only widget
		m_model->setData(index, QBrush(Qt::red), Qt::BackgroundRole);
	...
	case OBJECT_CLOUD_POINT_SIZE:
	{
        ...
        comboBox->addItem(tr(s_AutoPointSizeString)); //size = Auto
...
void ccPropertiesTreeDelegate::setEditorData(QWidget *editor, const QModelIndex &index) const
{
	...
	case OBJECT_CURRENT_DISPLAY:
	{
        ...
        qDebug() << "[DISPLAY] m_currentObject->getDisplay()" << m_currentObject->getDisplay();
		int pos = (win ? comboBox->findText(win->windowTitle()) : 0);

        if (!pos)
        {
            win = MainWindow::TheInstance()->GetGLWindow("3D View");
            if (win)
            {
                m_currentObject->setDisplay_recursive(win);
                m_currentObject->prepareDisplayForRefresh_recursive();
                pos = (win ? comboBox->findText(win->windowTitle()) : 0);
            }
        }
        ...
        {
            name = QString("pos:") + QString::number(pos);
        }
        qDebug() << "[DISPLAY]set currentObject " <<m_currentObject->getName() <<"display : "<< name;
	...
	case OBJECT_CLOUD_POINT_SIZE:
	{
        ...
        int index = static_cast<int>(cloud->getPointSize());
        QComboBox *comboBox = qobject_cast<QComboBox*>(editor);
        //bool result = comboBox->currentIndex() == 0 ? true : false;
...
void ccPropertiesTreeDelegate::updateDisplay()
{
    ...
    ccPointCloud* cloudobj = ccHObjectCaster::ToPointCloud(m_currentObject);
    ...
    cloudobj->getDrawingParameters(glParams);
    if (cloudobj /*&& !glParams.showSF*/)
    {
        cloudobj->scaleFieldColorChange();
    }
    if (m_currentObject->isKindOf(CC_TYPES::MESH))
    {
        ccMesh* meshobj = static_cast<ccMesh*>(m_currentObject);
        if (meshobj)
        {
            meshobj->scaleFieldColorChange();
        }
    }
...
{
    qDebug() << "[DISPLAY] slotCurrentDisplayChange : " << newDisplayTitle;
    if (newDisplayTitle == "None")
    {
        qCritical() << "[DISPLAY] None;";
    }
	if (!m_currentObject)
	{
		return;
    ...
    qDebug() << "[DISPLAY] " << m_currentObject->getName() << "display windowTitle: " << actualDisplayTitle;

//	if (actualDisplayTitle != newDisplayTitle)
	{
        qDebug() << "[DISPLAY] old display window" << actualDisplayTitle << "new display windowTitle: " << newDisplayTitle;
