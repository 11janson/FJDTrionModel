...
struct dbTreeSelectionInfo
{
	...
	size_t kdTreeCount;
	bool hasScalarFieldAndContainIntensity = false;
	void reset()
	{
...
public:
	...
	**/
    ...
    void addChildEntities();
    void exportTiffFiles();
    bool getItemHoverSelectState();
	...
	**/
    ...
    //设置勾选节点父节点是否勾选
    void setChildEnable(ccHObject *item, bool state);
    //设置勾选节点子节点是否勾选
    void setParentEnable(ccHObject *item, bool state);
private:
...
signals:
	...
	void signalSaveFile();
	void refreashCombinationDisplayState();

private slots:
	//[!].UI选择分类发生变化
	void slotUIClassifyChanged(QAction*pAction);
...
protected:
	...
	**/
	...
	//! Context menu action: add child entities
	QAction* m_addChildEntities;
    ...
    //! Context menu action: export tiff files
    QAction* m_exportTiff;
	//添加重命名按钮
