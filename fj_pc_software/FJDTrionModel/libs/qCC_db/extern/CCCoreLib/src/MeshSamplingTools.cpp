...
bool MeshSamplingTools::flagMeshVerticesByType(GenericIndexedMesh* mesh, ScalarField* flags, EdgeConnectivityStats* stats/*=nullptr*/)
{
													...
													std::vector<unsigned>* triIndices/*=nullptr*/)
{
	...
	//janson ，防止unsigned类型数据溢出
	if (ceil(Stotal * samplingDensity) > double(UINT_MAX))
		return nullptr;
													...
													std::vector<unsigned>* triIndices/*=nullptr*/)
{
	...
	if (progressCb)
	{
		...
		{
			QString m_str = QCoreApplication::translate("MeshSamplingTools", "Sampling...", nullptr);
			std::string str = m_str.toStdString();
			const char* ch = str.c_str();
			progressCb->setMethodTitle(ch);
			char buffer[256];
			m_str = QCoreApplication::translate("MeshSamplingTools", "Triangle: %u. Points: %u.", nullptr);
			str = m_str.toStdString();
			ch = str.c_str();
			sprintf(buffer, ch, triCount, theoreticNumberOfPoints);
