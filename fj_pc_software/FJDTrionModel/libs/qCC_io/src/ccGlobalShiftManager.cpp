...
double ccGlobalShiftManager::MAX_COORDINATE_ABS_VALUE = 1.0e12;
double ccGlobalShiftManager::MAX_DIAGONAL_LENGTH = 1.0e12;
...
/*				if ((	shiftInfo.shift - coordinatesShift).norm() <= CCCoreLib::ZERO_TOLERANCE_D
					&&	std::abs(shiftInfo.scale - scale) <= CCCoreLib::ZERO_TOLERANCE_D)*/		
				//�����жϵķ�Χ������ӽ��ĵ����޷�ƴ����һ��
				if ((shiftInfo.shift - coordinatesShift).norm() < 10000
						&&	std::abs(shiftInfo.scale - scale) <= CCCoreLib::ZERO_TOLERANCE_D)
				{
...
CCVector3d ccGlobalShiftManager::BestShift(const CCVector3d& P)
{
	...
	CCVector3d shift(	std::abs(P[0]) >= 1.0e4 ? -P[0] : 0,
						std::abs(P[1]) >= 1.0e4 ? -P[1] : 0,
						std::abs(P[2]) >= 1.0e4 ? -P[2] : 0 );
