...
:m_dwExceptionCode(0)
{
	// Get machine type
	m_dwMachineType = 0;
    wchar_t* wszProcessor = _tgetenv(L"PROCESSOR_ARCHITECTURE");
	if (wszProcessor)
	{
...
std::string StackTracer::GetExceptionMsg()
{
	...
	for (it = itbegin; it < itend; it++)
	{
        ...
        qDebug() << "Function : " << strFunctionName.c_str() << "\r\n";
        qDebug() << "    [Source File : " << strFileName.c_str() << "]\r\n";
        qDebug() << "    [Source Line : " << it->LineNumber << "]\r\n";
	}
