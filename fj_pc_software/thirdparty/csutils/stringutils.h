...
CSUTILS_EXPORT QString settingsKey(const QString &category);

// Return the common prefix part of a string list:
// "C:\foo\bar1" "C:\foo\bar2"  -> "C:\foo\bar"
CSUTILS_EXPORT QString commonPrefix(const QStringList &strings);

// Return the common path of a list of files:
// "C:\foo\bar1" "C:\foo\bar2"  -> "C:\foo"
CSUTILS_EXPORT QString commonPath(const QStringList &files);
...
CSUTILS_EXPORT QString withTildeHomePath(const QString &path);

// Removes first unescaped ampersand in text
CSUTILS_EXPORT QString stripAccelerator(const QString &text);

CSUTILS_EXPORT bool readMultiLineString(const QJsonValue &value, QString *out);

class CSUTILS_EXPORT AbstractMacroExpander
{
public:
...
CSUTILS_EXPORT void expandMacros(QString *str, AbstractMacroExpander *mx);
CSUTILS_EXPORT QString expandMacros(const QString &str, AbstractMacroExpander *mx);

CSUTILS_EXPORT int parseUsedPortFromNetstatOutput(const QByteArray &line);
