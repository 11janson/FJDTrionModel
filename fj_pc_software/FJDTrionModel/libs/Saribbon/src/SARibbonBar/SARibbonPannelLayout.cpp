...
QMargins SARibbonPannelLayout::s_contentsMargins = QMargins(12, 1, 5, 1);
...
void SARibbonPannelLayout::updateGeomArray(const QRect& setrect)
{
	...
	double xStart = 0;
	double totalWidthNum = mag.left();
	for (int i = 0; i < itemCount; ++i) {
		SARibbonPannelItem* item = m_items.at(i);
		if (!item->isEmpty()) 
		{
			QSize hint = item->sizeHint();
			totalWidthNum += hint.width();
			totalWidthNum += spacing;
		}
	}
	//if (m_leftSpace > (totalWidthNum) && (totalWidthNum)>0)
	//{
	//	xStart = (m_leftSpace - (totalWidthNum-spacing- mag.left())) / 2 -mag.left();
	//}
    ...
    // Medium行的y位置
	//大图标按钮和两行小图标按钮对齐操作
    const int yMediumRow0 = (2 == rowCount) ? mag.top() : (mag.top() + ((largeHeight - 2 * smallHeight) / 3)-8);
    const int yMediumRow1 = (2 == rowCount) ? (mag.top() + smallHeight + 6)
                                            : (mag.top() + ((largeHeight - 2 * smallHeight) / 3) * 2 + smallHeight-8);
    // Small行的y位置
    ...
    for (int i = 0; i < itemCount; ++i) {
        ...
        case SARibbonPannelItem::Large: {
            ...
            item->itemWillSetGeometry = QRect(x+ xStart, mag.top(), hint.width(), largeHeight);
        ...
        case SARibbonPannelItem::Medium: {
            ...
            if (2 == rowCount) {
                if (0 == row) {
                    item->rowIndex            = 0;
                    item->columnIndex         = column;
                    item->itemWillSetGeometry = QRect(x + xStart, yMediumRow0, hint.width(), smallHeight);
                    ...
                    item->rowIndex            = 1;
                    item->columnIndex         = column;
                    item->itemWillSetGeometry = QRect(x + xStart, yMediumRow1, hint.width(), smallHeight);
        ...
        case SARibbonPannelItem::Small: {
            if (0 == row) {
                ...
                item->rowIndex            = 0;
                item->columnIndex         = column;
                item->itemWillSetGeometry = QRect(x + xStart, ySmallRow0, hint.width(), smallHeight);
                ...
                item->rowIndex            = 1;
                item->columnIndex         = column;
                item->itemWillSetGeometry = QRect(x + xStart, ySmallRow1, hint.width(), smallHeight);
                ...
                item->rowIndex            = 2;
                item->columnIndex         = column;
                item->itemWillSetGeometry = QRect(x + xStart, ySmallRow2, hint.width(), smallHeight);
                //和上个进行比较得到最长宽度
                columMaxWidth = qMax(columMaxWidth, hint.width());
                //换列，x自动递增到下个坐标，列数增加，行数归零，最大列宽归零
                x += (columMaxWidth + spacing);
    ...
    this->m_sizeHint = QSize(totalWidth-20, height);
}
...
void SARibbonPannelLayout::setLeftSpace(double space)
{
	m_leftSpace = space;
}