...
public:
	...
	**/
	...
	//[!]角度
	void angularAdjustment(double);
	//[!]距离
	void distanceAdjustment(double);

	//[!]矩阵变换
	void axisMatrixTransformation(int index);

	//[!]创建角度窗口
	void creatAngularDialog();

	//[!]创建距离窗口
	void creatDistanceDialog();

	void deleteBottomDialog();

	void creatBottomDialog();
...
protected:
	...
	void sync_model_view(int index);
protected:
	...
	**/
	...
	public Q_SLOTS:
		...
		void slotPushButtonAngularPlus();

		void slotPushButtonDistanceMinus();

		void slotPushButtonAngularMinus();

		void slotPushButtonDistancePlus();

		void slotRefreshRevolveButtonStatus(int);

		void slotRefreshShiftButtonStatus();
	private:
		
		void setButtonStyle(QToolButton* pb, const QString& normalPix, const QString& clickedPix, const QString& disabledPix);
		//[!]记录往哪个轴方向移动
		rotComboBoxItems m_shiftDirection = rotComboBoxItems::XYZ;
		//[!]记录pause按钮
		bool m_bpauseButton = false;

		//[!]微调窗口的控件
        QToolButton		*m_pAngularButtonAdd = nullptr;
        QToolButton		*m_pAngularButtonCut = nullptr;
		QDoubleSpinBox	*m_pAngularSpinBox = nullptr;
		QLabel			*m_pAngularLabel= nullptr;
		

        QToolButton		*m_pDistanceButtonAdd = nullptr;
        QToolButton		*m_pDistanceButtonCut = nullptr;
		QDoubleSpinBox	*m_pDistanceSpinBox = nullptr;
		QLabel			*m_pDistanceLabel = nullptr;
		QDialog			*m_pDistanceDialog = nullptr;
		QDialog			*m_pAngularDialog = nullptr;
		QPoint			m_lastDialogPos;
        QComboBox *advTranslateComboBox = nullptr;
        QComboBox *advRotateComboBox = nullptr;
};
