...
CC_FILE_ERROR STLFilter::saveToFile(ccHObject* entity, const QString& filename, const SaveParameters& parameters)
{
	...
	{
		//QMessageBox msgBox(QMessageBox::Question, QCoreApplication::translate("STLFilter", "Choose output format", nullptr), QCoreApplication::translate("STLFilter","Save in BINARY or ASCII format?", nullptr));
		//QPushButton *binaryButton = msgBox.addButton(QCoreApplication::translate("STLFilter","BINARY", nullptr), QMessageBox::AcceptRole);
		//msgBox.addButton(QCoreApplication::translate("STLFilter","ASCII", nullptr), QMessageBox::AcceptRole);
		//msgBox.exec();
		//binaryMode = (msgBox.clickedButton() == binaryButton);

        CS::Widgets::FramelessMessageBox msgBox(QMessageBox::Question, QCoreApplication::translate("STLFilter", "Choose output format", nullptr),
            QCoreApplication::translate("PlyFilter", "Save in BINARY or ASCII format?", nullptr),
            QMessageBox::Cancel | QMessageBox::Retry | QMessageBox::Yes,
            nullptr);
        msgBox.changeButtonText(QDialogButtonBox::StandardButton::Yes, QCoreApplication::translate("STLFilter", "BINARY", nullptr));
        msgBox.changeButtonText(QDialogButtonBox::StandardButton::Retry, QCoreApplication::translate("STLFilter", "ASCII", nullptr));
	...
	if (binaryMode)
	{
		result = saveToBINFile(mesh, theFile, parameters);
	}
	else //if (msgBox.clickedButton() == asciiButton)
	{
		result = saveToASCIIFile(mesh, theFile, parameters);
	}

	return result;
}

CC_FILE_ERROR STLFilter::saveToBINFile(ccGenericMesh* mesh, QFile& theFile, const SaveParameters& parameters)
{
	...
	if (parameters.parentWidget)
	{
        ...
        pDlg->setMethodTitle(QCoreApplication::translate("PlyFilter", "Saving mesh", nullptr)
            + (mesh->getName().length() > 30 ? "[" + mesh->getName().left(30) + "...]" : "[" + mesh->getName() + "...]"));			pDlg->setInfo(QObject::tr("Number of facets: %1").arg(faceCount));
		pDlg->start();
	...
	//header
	{
		char header[80];
		memset(header, 0, 80);
		strcpy(header, "Binary STL file generated by FJD Trion Model!");
	...
	for (unsigned i = 0; i < faceCount; ++i)
	{
		...
		{
            theFile.close();
            QFileInfo FileInfo(theFile.fileName());
            if (FileInfo.isFile())//如果是文件
                QFile::remove(theFile.fileName());
			return CC_FERR_CANCELED_BY_USER;
		}
	}
...
CC_FILE_ERROR STLFilter::saveToASCIIFile(ccGenericMesh* mesh, QFile& theFile, const SaveParameters& parameters)
{
	assert(theFile.isOpen() && mesh && mesh->size() != 0);
	unsigned faceCount = mesh->size();
...
CC_FILE_ERROR STLFilter::loadFile(const QString& filename, ccHObject& container, LoadParameters& parameters)
{
	...
	if (error != CC_FERR_NO_ERROR && error != CC_FERR_CANCELED_BY_USER)
	{
		return CC_FERR_MALFORMED_FILE;
	}
    if (error == CC_FERR_CANCELED_BY_USER)
    {
        return CC_FERR_CANCELED_BY_USER;
    }
	...
	mesh->mergeDuplicatedVertices(ccMesh::DefaultMergeDuplicateVerticesLevel, nullptr);
