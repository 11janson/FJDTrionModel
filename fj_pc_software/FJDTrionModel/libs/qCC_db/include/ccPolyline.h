...
public:
	...
	**/
	...
	//设置点云真实坐标点
	void set3DPointData(std::vector<CCVector3d> pointData);

	//获取点云真实坐标点
	std::vector<CCVector3d> get3DPointData();

	//设置点坐标
	void setPointVec(unsigned Index, const CCVector3d & data);

    //设置等高线名称，名称位置，名称朝向
    void setContourNameParam(double name, CCVector3 namepos, CCVector3 nameheadpos);
...
protected:
	...
	//! Arrow length
	PointCoordinateType m_arrowLength = 0;
	//! Arrow index
	unsigned m_arrowIndex = 0;

	std::vector<CCVector3d> m_3DPointData;  //真实坐标

    double m_ContourName;//等高线数值

    CCVector3 m_ContourPosition;//等高线名称显示位置

    CCVector3 m_ContourHeadPosition;//等高线名称朝向位置

    bool m_isShowContourName = false;//是否显示等高线名称
};
