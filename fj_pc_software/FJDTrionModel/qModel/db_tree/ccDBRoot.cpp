...
private:
	...
	void	init()
	{
		...
		mIconList.append( { QIcon(QStringLiteral(":/CC/images/dbPolylineSymbol.png")),
                            QIcon(QStringLiteral(":/CC/images/dbPolylineSymbolLocked.png")) } );
...
ccDBRoot::ccDBRoot(ccCustomQTreeView* dbTreeWidget, QTreeView* propertiesTreeWidget, QObject* parent) : QAbstractItemModel(parent)
{
	...
	//m_expandBranch = new QAction(tr("Expand"), this);
	m_saveAction = new QAction(tr("Save File"), this);
	//m_collapseBranch = new QAction(tr("Folding"), this);
	m_gatherInformation = new QAction(tr("Information"), this);
	...
	m_addChildEntities = new QAction(tr("Import vector file"), this);
    m_exportTiff = new QAction(tr("Export tiff file"), this);


	m_toggleSelectedEntities = new QAction(tr("Toggle selected"), this);
	m_toggleSelectedEntitiesVisibility = new QAction(tr("Toggle visual"), this);
	...
	m_toggleSelectedEntitiesMat = new QAction(tr("Switch materials/textures"), this);
	...
	m_renameAction = new QAction(QApplication::translate("ccDBroot","Rename",nullptr), this);
	m_timer = new QTimer(this);
	connect(m_timer,&QTimer::timeout,this, &ccDBRoot::showDoubleClick);
	m_contextMenuPos = QPoint(-1,-1);
    QApplication::translate("cchobjectcontrol", "Ground", nullptr);
	//[!].分类菜单
	m_pClassifyMenu = new QMenu(m_dbTreeWidget);
	m_pClassifyMenu->setTitle(QApplication::translate("ccDBroot", "Toggle classification", nullptr));
	QAction *pWellAction = new QAction(QApplication::translate("ccDBroot","Wall",nullptr), this);
	pWellAction->setData(17);
	m_pClassifyMenu->addAction(pWellAction);
	QAction *pFloorAciton = new QAction(QApplication::translate("ccDBroot", "Floor", nullptr), this);
	pFloorAciton->setData(20);
	m_pClassifyMenu->addAction(pFloorAciton);
	QAction *pCeilingAciton = new QAction(QApplication::translate("ccDBroot", "Ceiling", nullptr), this);
	pCeilingAciton->setData(7);
	m_pClassifyMenu->addAction(pCeilingAciton);
	QAction *pOtherAciton = new QAction(QApplication::translate("ccDBroot", "Other", nullptr), this);
	pOtherAciton->setData(0);
	m_pClassifyMenu->addAction(pOtherAciton);

	//connect custom context menu actions
	connect(m_renameAction, &QAction::triggered, this, &ccDBRoot::renameSelectedEntities);
	...
	connect(m_saveAction,						&QAction::triggered,					this, &ccDBRoot::saveFile);
    ...
    connect(m_addChildEntities,                 &QAction::triggered,                    this, &ccDBRoot::addChildEntities);
    connect(m_exportTiff,                       &QAction::triggered,                    this, &ccDBRoot::exportTiffFiles);
	...
	connect(m_pClassifyMenu, &QMenu::triggered, this, &ccDBRoot::slotUIClassifyChanged);
	//Properties Tree
...
void ccDBRoot::addElement(ccHObject* object, bool autoExpand/*=true*/)
{
	...
	if (object->getClassID() ==(CC_TYPES::POINT_OCTREE))
	{
		return;
	}
...
bool ccDBRoot::getIsExpandElement(ccHObject* object)
{
    if (!object || !m_dbTreeWidget){
        return false;
    }

    return m_dbTreeWidget->isExpanded(index(object));
}
...
void ccDBRoot::showDoubleClick()
{
	QMouseEvent mEvnPress(QEvent::MouseButtonDblClick, m_contextMenuPos, m_dbTreeWidget->mapToGlobal(m_contextMenuPos),Qt::LeftButton, Qt::LeftButton, Qt::NoModifier);
	QWidget *w = qApp->widgetAt(m_dbTreeWidget->mapToGlobal(m_contextMenuPos));
	QApplication::sendEvent(w, &mEvnPress);
	m_timer->stop();
}

void ccDBRoot::renameSelectedEntities()
{
	m_timer->start(500);
}

void ccDBRoot::addChildEntities()
{
    ...
    if (1 == selCount)
    {
        ...
        {
            MainWindow::TheInstance()->doActionAddChildObj(obj);
        }
		
    }
}

void ccDBRoot::exportTiffFiles()
{
    QItemSelectionModel* qism = m_dbTreeWidget->selectionModel();
    QModelIndexList selectedIndexes = qism->selectedIndexes();
    if (selectedIndexes.empty())
    ...
    {
        ccHObject* obj = static_cast<ccHObject*>(selectedIndexes[0].internalPointer());
        if (obj && obj->isKindOf(CC_TYPES::MESH))
        {
            MainWindow::TheInstance()->doActionExportTiff();
        }
    }
}
...
void ccDBRoot::deleteSelectedEntities()
{
    ...
    if (!toBeDeleted.empty()) {
        //[!].删除实体信号
        emit CS::Model::ProjectModel::instance()->signalRemoveSelectedEntityChanged();
    }
	...
	while (!toBeDeleted.empty())
	{
        ...
        if (object->filterChildren(clouds, true, CC_TYPES::POINT_CLOUD) != 0)
        {
            for (ccHObject* pobj : clouds)
            {
                ccPointCloud* pCloud = ccHObjectCaster::ToPointCloud(pobj);
                if (pCloud)
                    CS::Model::ProjectModel::instance()->deletePointClouds(pCloud);
            }
        }

        ccPointCloud* pCloud = ccHObjectCaster::ToPointCloud(object);
        if (pCloud)
        {
            CS::Model::ProjectModel::instance()->deletePointClouds(pCloud);
        }
		...
		ccHObject* parent = object->getParent();
        ccHObject* parenttemp = object->getParent();
		int childPos = parent->getChildIndex(object);
		assert(childPos >= 0);
        int nodeNum = 1;
        /*nodeNum记录当前节点层级
        ...
        以后视情况根据nodeNum修改逻辑
        */
        while ("DB Tree" != parenttemp->getName())
        {
            ++nodeNum;
            if (parenttemp->getParent())
            {
		...
		parent->removeChild(childPos);
		endRemoveRows();
		auto iter = std::find(toBeDeleted.begin(), toBeDeleted.end(), parent);
        if (nodeNum == 2 )
        {
            if (parent->getChildrenNumber() == 0 && iter == toBeDeleted.end())
            {
                ccHObject* parentp = parent->getParent();
                int childPosp = parentp->getChildIndex(parent);
                assert(childPosp >= 0);
                beginRemoveRows(index(parent).parent(), childPosp, childPosp);
                parentp->removeChild(childPosp);
                endRemoveRows();
            }
        }
        MainWindow::TheInstance()->deleteDBTreeSelectObject(object);
        object = nullptr;
	}
   ...
   /* CS::Model::ProjectModel::instance()->clearSelectEntitys();
    emit CS::Model::ProjectModel::instance()->signalSelectedEntityChanged();*/
	MainWindow::RefreshAllGLWindow(false);
}
...
QVariant ccDBRoot::data(const QModelIndex& idx, int role) const
{
	...
	case Qt::DisplayRole:
	{
		...
		if (item->isA(CC_TYPES::LABEL_2D))
			baseName = QStringLiteral("Label: ")+baseName;
...
void ccDBRoot::setParentEnable(ccHObject *item, bool state)
{
    if (item && item->getParent() && item->getParent() != m_treeRoot && item->getParent()->getChildrenNumber() <= 1 && !item->isEnabled())
    {
        if (item->getParent()->isA(CC_TYPES::HIERARCHY_OBJECT) && !item->getParent()->isEnabled())
        {
            return;
        }
        item->getParent()->setEnabled(state);
        updateCCObject(item->getParent());
        setParentEnable(item->getParent(), state);
    }
    else if(item && item->getParent() && item->getParent() != m_treeRoot && item->getParent()->getChildrenNumber() > 1)
    {
        std::vector<ccHObject*> children;
        item->getParent()->filterChildren(children);
        std::vector<ccHObject*>::iterator it = std::find_if(children.begin(), children.end(), [&](ccHObject* temp) {
            return temp && temp->isEnabled();
        });
        if (!state)
        {
            if (it != children.end())
    ...
    if (item && item->getParent() && item->getParent() != m_treeRoot && item->isEnabled())
    {
        if (item->getParent()->isA(CC_TYPES::HIERARCHY_OBJECT) && item->getParent()->isEnabled())
        {
            return;
...
void ccDBRoot::setChildEnable(ccHObject *item, bool state)
{
    if (item)
    {
        for (int i = 0; i < item->getChildrenNumber(); i++)
        {
            ccHObject* pObj = item->getChild(i);
            if (pObj)
            {
                pObj->setEnabled(state);
                updateCCObject(pObj);
                setChildEnable(pObj, state);
            }
        }
    }
}
...
bool ccDBRoot::setData(const QModelIndex& idx, const QVariant& value, int role)
{
	if (idx.isValid())
	{
		if (role == Qt::EditRole)
		{
            ...
            if ( value.toString().contains("\\",Qt::CaseSensitive)|| value.toString().contains("/", Qt::CaseSensitive) || value.toString().contains("*", Qt::CaseSensitive) || value.toString().contains("|", Qt::CaseSensitive) || value.toString().contains("?", Qt::CaseSensitive) || value.toString().contains("<", Qt::CaseSensitive) || value.toString().contains(">", Qt::CaseSensitive) || value.toString().contains(":", Qt::CaseSensitive))
            {
				return false;
            }
		...
		else if (role == Qt::CheckStateRole)
		{
			...
			if (item)
			{
                ...
                bool checkd = (value == Qt::Checked) ? true : false;


                setParentEnable(item, checkd);
                setChildEnable(item, checkd);
...
void ccDBRoot::changeSelection(const QItemSelection & selected, const QItemSelection & deselected)
{
    ...
    getSelectedEntities(selectedEntities, CC_TYPES::OBJECT, &selInfo);
    CS::Model::ProjectModel::instance()->clearSelectEntitys();
    for (auto selectObject : selectedEntities)
    {
        CS::Model::ProjectModel::instance()->appendSelectEntity(selectObject);
    }
	updatePropertiesView();
...
bool ccDBRoot::getItemHoverSelectState()
{
    ccGui::ParamStruct parameters = ccGui::Parameters();
    return parameters.m_isHoverSelected;
}
...
void ccDBRoot::selectEntity(ccHObject* obj, bool forceAdditiveSelection/*=false*/)
{
    ...
    if (dynamic_cast<cc2DItemBase*>(obj))
    {
        additiveSelection = getItemHoverSelectState() ? false : true;
    }
...
void ccDBRoot::selectEntities(std::unordered_set<int> entIDs)
{
    ...
    ctrlPushed = getItemHoverSelectState() ? false : true;
...
void ccDBRoot::updateCCObject(ccHObject* object)
{
	...
	{
		emit dataChanged(idx, idx);
		emit refreashCombinationDisplayState();
	}
}
										...
										dbTreeSelectionInfo* info/*=nullptr*/ )
{
	...
	if (info)
	{
		...
		for (size_t i = 0; i < info->selCount; ++i)
		{
			...
			else if(obj->isKindOf(CC_TYPES::POINT_CLOUD))
			{
				...
				if (cloud)
				{
					...
					for (int i = 0; i < nsf; ++i)
					{
						...
						{
							info->hasScalarFieldAndContainIntensity = true;
						}
					}
			...
			{
                //janson 2022.8.30
                if(obj->getClassID() != CC_TYPES::CUBOIDS)
				    info->meshCount++;
...
Qt::ItemFlags ccDBRoot::flags(const QModelIndex& idx) const
{
	...
	if (item && !item->isLocked()) //locked items cannot be drag-dropped
	{
		if (item->isA(CC_TYPES::HIERARCHY_OBJECT)								||
			...
			item->isKindOf(CC_TYPES::SHOW_BOX)									||//janson.yang
...
void ccDBRoot::gatherRecursiveInformation()
{
	...
	{
		QStringList infoStr;
        QStringList infoStr1;

		infoStr << QString(tr("Point(s):"));
		infoStr << QString(tr("Triangle(s):"));
        infoStr << QString(tr("Scalar field(s):"));
        infoStr << QString(tr("Color(s):")) + "\t";
        infoStr << QString(tr("Cloud(s):")) + "\t";
        infoStr << QString(tr("Mesh(es):")) + "\t";

        infoStr1 << QString::number(info.pointCount);
        infoStr1 << QString::number(info.triangleCount);
        infoStr1 << QString::number(info.scalarFieldCount);
        infoStr1 << QString::number(info.colorCount);
        infoStr1 << QString::number(info.cloudCount);
        infoStr1 << QString::number(info.meshCount);

		//infoStr << separator;
		//if (info.colorCount)
		//	infoStr << QString(tr("Color(s):")) + "\t\t" + QString::number(info.colorCount);
		//if (info.normalCount)
		//	infoStr << QString(tr("Normal(s):")) + "\t\t" + QString::number(info.normalCount);
		//if (info.scalarFieldCount)
		//	infoStr << QString(tr("Scalar field(s):")) + "\t\t" + QString::number(info.scalarFieldCount);
		//if (info.materialCount)
		//	infoStr << QString(tr("Material(s):")) + "\t\t" + QString::number(info.materialCount);

		//infoStr << separator;
		//if (info.octreeCount)
		//	infoStr << QString(tr("Octree(s):")) + "\t\t" + QString::number(info.octreeCount);
		//if (info.imageCount)
		//	infoStr << QString(tr("Image(s):")) + "\t\t" + QString::number(info.imageCount);
		//if (info.labelCount)
		//	infoStr << QString(tr("Label(s):")) + "\t\t" + QString::number(info.labelCount);
		//if (info.sensorCount)
		//	infoStr << QString(tr("Sensor(s):")) + "\t\t" + QString::number(info.sensorCount);

		//display info box

        FjInformationShowDlg dlg(MainWindow::TheInstance());
		dlg.setWindowTitle(tr("Information"));
        dlg.setTopMessage(infoStr.join("\n\n"), infoStr1.join("\n\n"));
        dlg.exec();
	}
}
...
void ccDBRoot::toggleSelectedEntitiesProperty(TOGGLE_PROPERTY prop)
{
	...
	for (int i = 0; i < selCount; ++i)
	{
        ...
        ccPointCloud* cloudobj = ccHObjectCaster::ToPointCloud(item);
		switch (prop)
		{
		...
		case TG_COLOR: //color
            ...
            if (cloudobj)
            {
                cloudobj->scaleFieldColorChange();
            }
			break;
...
void ccDBRoot::showContextMenu(const QPoint& menuPos)
{
	...
	if (idx.isValid())
	{
		...
		if (selCount)
		{
			...
			bool bHasClassify = false;
			bool bDelete = true;
            bool bAddDxfChild = false;
            bool bForestryChild = false;
            bool hasvertices = false;
			bool bRenameDisabled = false;
            bool hasCubes = false;
			...
			for (int i = 0; i < selCount; ++i)
			{
                ...
                if (item->getMetaData("forestry_tree_split").toBool()) {
                    bExpandBranch = false;
                }
				...
				if (!item->isA(CC_TYPES::HIERARCHY_OBJECT))
				{
					...
					if (item->getMetaData("Classify").toBool()) {
						bHasClassify = true;
					}
					if (item->isLocked() && bDelete)
					{
						bDelete = false;
					}
                    if (item->getMetaData("is2dSurface").toBool() && !item->getMetaData("graphicsentitydxf").toBool())
                    {
                        bAddDxfChild = true;
                    }
                    if (item->getMetaData("forestry_tree_split_root").toBool())
                    {
                        bForestryChild = true;
                    }
					if (item->getMetaData("renamedisabled").toBool())
					{//[!].重命名使能
						bRenameDisabled = true;
					}
                ...
                if (item->isKindOf(CC_TYPES::POINT_CLOUD) && !CanDetachCloud(item))
                {
                    hasvertices = true;
                }
			}
			...
			if (toggleVisibility)
			{
				menu.addAction(m_toggleSelectedEntitiesVisibility);//janson.yang 2022.5.6 减少右键弹出信息
			}
			if (toggleOtherProperties)
			{
				//menu.addAction(m_toggleSelectedEntitiesColor);
				//menu.addAction(m_toggleSelectedEntitiesNormals);//janson.yang 2022.5.6 减少右键弹出信息
				//menu.addAction(m_toggleSelectedEntitiesSF);
			}
            
            //[!].林业单木风格
            if (bForestryChild && !toggleOtherProperties){
                //menu.addAction(m_toggleSelectedEntitiesColor);
            }
          
			//[!].分类
			if (bHasClassify) {
				menu.addMenu(m_pClassifyMenu);
			}
			if (toggleMaterials)
			{
			...
			//menu.addAction(m_toggleSelectedEntities3DName);//janson.yang 2022.5.6 减少右键弹出信息
			menu.addSeparator();


			if (bDelete && !hasvertices)
			{
				menu.addAction(m_deleteSelectedEntities);
			}
            //添加子节点
            if (selCount == 1 && toggleOtherProperties && bAddDxfChild)
            {
                menu.addAction(m_addChildEntities);
            }
            //mesh添加导出tiff菜单
            if (selCount == 1 && toggleOtherProperties && toggleMaterials && (!hasCubes))
            {
                menu.addAction(m_exportTiff);
            }
			...
			if (selCount == 1 && hasMoreThanOneChild)
			{
				...
				menu.addAction(m_sortChildrenType);*/ //janson.yang 2022.5.6 减少右键弹出信息
			}
			...
			if (selCount == 1 && !leafObject)
			{
				...
				menu.addSeparator();
				menu.addAction(m_addEmptyGroup);*/ //janson.yang 2022.5.6 减少右键弹出信息
			}
			...
			//[!].重命名
			if (bRenameDisabled){
				menu.removeAction(m_renameAction);
			}
		}
    ...
    if (idx.isValid())
    {
        ...
        if (selectedIndexes.size() == 1)
        {
            ...
            if (item && item->getMetaData("MeshType").toInt() == 1)
            {
                menu.removeAction(m_exportTiff);
            }
        }
    }
...
void ccDBRoot::slotUIClassifyChanged(QAction*pAction)
{
	if (!pAction) {
		qDebug() << "aciton is nullptr!";
		return;
	}
	qDebug() << QString("Classify name:%1, type:%2").arg(pAction->text()).arg(pAction->data().toInt());
	...
	for (unsigned i = 0; i < selCount; ++i)
	{
		ccHObject* obj = static_cast<ccHObject*>(selectedIndexes[i].internalPointer());
		if (!obj->getParent() || obj->isLocked()){
			qDebug() << QString("Object '%1' can't be deleted this way (locked)").arg(obj->getName());
			continue;
		}

		//[!].判断是否为可以切换的类型
		if (!obj->getMetaData("Classify").toBool()) {
			continue;
		}

		CS::ModelControl::CCHObjectControl control(obj);
		control.setClassifyTypeChange(nType);
		obj->prepareDisplayForRefresh();
		ccHObject* parent = obj->getParent()->getParent();
		if (parent)
		{
			expandElement(parent, false);
			expandElement(parent, true);
		}
	}
	MainWindow::RefreshAllGLWindow(false);
}
