...
void ccSectionExtractionTool::undo()
{
	...
	if (CS::Widgets::FramelessMessageBox::question(MainWindow::TheInstance(), "Undo", QString("Remove %1 polylines?").arg(m_sections.size() - count), QMessageBox::Yes, QMessageBox::No) == QMessageBox::No)
	{
		//restore undo stack!
...
bool ccSectionExtractionTool::reset(bool askForConfirmation/*=true*/)
{
	...
	if (askForConfirmation)
	{
		...
		for (auto & section : m_sections)
		{
			...
			{
				if (CS::Widgets::FramelessMessageBox::question(MainWindow::TheInstance(), "Reset", "You'll lose all manually defined polylines: are you sure?", QMessageBox::Yes, QMessageBox::No) == QMessageBox::No)
					return false;
				else
					break;
			}
...
void ccSectionExtractionTool::unfoldPoints()
{
	if (!m_selectedPoly)
	{
		if (QMessageBox::No == CS::Widgets::FramelessMessageBox::question(this, "No polyline selected", "Do you want to unfold all the polylines at once?"))
		{
			return;
		}
	}
