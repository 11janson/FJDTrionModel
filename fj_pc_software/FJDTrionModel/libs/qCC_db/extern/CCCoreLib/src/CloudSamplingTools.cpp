...
ReferenceCloud* CloudSamplingTools::subsampleCloudRandomly(	GenericIndexedCloudPersist* inputCloud,
															...
															GenericProgressCallback* progressCb/*=nullptr*/)
{
    qDebug() << "start subsampleCloudRandomly";
	...
	if (progressCb)
	{
		...
		{
			progressCb->setInfo(QCoreApplication::translate("CloudSamplingTools", "Random subsampling",nullptr));
		}
		progressCb->update(0);
		progressCb->start();
	}

    qDebug() << "start randomly remove inputCloud";
    ...
    qDebug() << "end randomly remove inputCloud";

	newCloud->resize(newNumberOfPoints); //always smaller, so it should be ok!

    qDebug() << "end subsampleCloudRandomly";
														   ...
														   GenericProgressCallback* progressCb/*=nullptr*/)
{
	...
	if (progressCb)
	{
		if ( nullptr )///progressCb->textCanBeEdited())
		{
												...
												GenericProgressCallback* progressCb/*=nullptr*/)
{
	...
	for (unsigned step = 0; step < 1; ++step) //fake loop for easy break
	{
		...
		//1st step: compute the average distance to the neighbors
		{
            ...
            QString title = QCoreApplication::translate("CloudSamplingTools","Processing",nullptr);
            progressCb->setMethodTitle(title.toStdString().c_str());
            QString info = QCoreApplication::translate("CloudSamplingTools", "Deleting outliers, please wait...", nullptr);
            progressCb->setInfo(info);
			if (octree->executeFunctionForAllCellsAtLevel(	octreeLevel,
															&applySORFilterAtLevel,
															additionalParameters,
															true,
															progressCb,title.toStdString().c_str())== 0)
			{
				//something went wrong
				break;
			}
