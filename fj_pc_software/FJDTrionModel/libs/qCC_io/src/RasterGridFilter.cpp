...
CC_FILE_ERROR RasterGridFilter::loadFile(const QString& filename, ccHObject& container, LoadParameters& parameters)
{
	...
	try
	{
		...
		if( poDataset != nullptr )
		{
            ...
            //读取旋转平移矩阵（针对点云形式）
            ccGLMatrix  rotate_matrix_inv;
            bool success_custom = false;
            const char* rotate_matrix_inv_char = poDataset->GetMetadataItem("transformMatrixCustom");
            if (rotate_matrix_inv_char != "")
            {    
                rotate_matrix_inv = ccGLMatrixTpl<float>::FromString(QString(rotate_matrix_inv_char), success_custom);
            }
			...
			if (colorBands >= 3)
			{
				...
				if (parameters.parentWidget && loadAsTexturedQuad) //otherwise it means we are in command line mode --> no popup
				{
					loadAsTexturedQuad = (	CS::Widgets::FramelessMessageBox::question(	parameters.parentWidget,
											"Result type",
			...
			else
			{
                ...
                if (success_custom)
                {
                    pc->setGLTransformation(rotate_matrix_inv);
                    pc->applyGLTransformation_recursive();
                }
				...
				else //colors
				{
					...
					else if (isScalar && !loadAsTexturedQuad)
					{
						...
						else
						{
							...
							const char* orisfName = poBand->GetMetadataItem("sfName");
							if (QString(orisfName).contains("intensity", Qt::CaseInsensitive))
							{
								sf->setColorScale(ccColorScalesManager::GetDefaultScale(ccColorScalesManager::GREY));
							}
							else
							{
								sf->setColorScale(ccColorScalesManager::GetDefaultScale(ccColorScalesManager::BGYR));
							}

							//将标量颜色转化到rgb显示
							if (pc->convertCurrentScalarFieldToColors(false))
							{
								pc->showColors(true);
								pc->showSF(false);
			...
			else if (pc)
			{
				...
				else if (validPoints.size() != 0 && validPoints.size() < pc->size())
				{
					...
					{
						result = (s_alwaysRemoveInvalidHeights ? QMessageBox::Yes : CS::Widgets::FramelessMessageBox::question(nullptr, "Remove invalid points?", "This raster has pixels with invalid heights. Shall we remove them?", QMessageBox::Yes| QMessageBox::YesToAll| QMessageBox::No));
					}
