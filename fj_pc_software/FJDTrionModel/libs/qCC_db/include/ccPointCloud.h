...
public:
    ...
    struct pointImage
    {
        CCVector3d m_trajectoryposition;
        CCVector3d m_imageposition;
        double keyframe_ema_qua_x;
        double keyframe_ema_qua_y;
        double keyframe_ema_qua_z;
        double keyframe_ema_qua_w;
        double deltaValue_x;
        double deltaValue_y;
        double deltaValue_z;
        double deltaValue_r;
        double deltaValue_p;
        double deltaValue_ya;
        QImage imageOrdIndex;
    };
...
public: //clone, copy, etc.
	...
	**/
    ...
    //[!]设置点云是否启动大小自适应
	inline void setPointSizeAdaptively(bool state = false) {
		m_cloudSizeAdaptively = state;
	}
    //[!]获取点云是否开启自适应大小
    inline bool getPointSizeAdaptively() { return m_cloudSizeAdaptively; }

    inline void setCloudBigPointSize(float size) {
        m_fBigAutoPointSize = size;
    }
    inline float getCloudBigPointSize() { return m_fBigAutoPointSize; }

    void importParametersPointCloudFrom(ccPointCloud* cloud);
...
public: //features deletion/clearing
	...
	**/
	...
	//第一个坐标小于10000时不偏移
	void adjustGlobalShift();

    //获取包围盒八个顶点
    void getCornerPoint(std::vector<CCVector3> & points);

    //设置图像融合数据
    void setImageSourse(const std::vector<ccPointCloud::pointImage> & imageSourse);

    //获取图像融合数据
    const std::vector<ccPointCloud::pointImage> & getImageSourse();
...
public: //features allocation/resize
	...
	**/
    ...
    //更新隐藏部分
    void visibilityArrayChange();

    //更新标量显示
    void scaleFieldColorChange();

    //更新坐标显示
    void cloudPositionChange();
...
public: //other methods
	...
	**/
    ...
    void drawUserDb(CC_DRAW_CONTEXT & context);
...
protected:
    ...
    //绘制点云新
    void drawDataNew(CC_DRAW_CONTEXT& context);

    //绘制点云旧
    void drawDataOld(CC_DRAW_CONTEXT& context);
...
protected: // VBO
    ...
    struct pointcloudVertexArrayManager
    {
        ...
        bool m_isInitVBO = false;
        bool m_isNeedUpdateColor = false;
        bool m_isNeedUpdateVertices = false;
        bool m_isNeedUpdateIndices = false;
        bool m_isNeedUpdateVisibilityArray = false;
        bool m_isShowSf = false;

        void reset()
        {
            m_isInitVBO = false;
            m_isNeedUpdateColor = false;
            m_isNeedUpdateVertices = false;
            m_isNeedUpdateIndices = false;
            m_isNeedUpdateVisibilityArray = false;
            colors.clear();
        }
    };

    //! Set of VBOs attached to this cloud
    pointcloudVertexArrayManager m_vertexArrayManager;
private:
    bool m_cloudSizeAdaptively = false;

	float m_fBigAutoPointSize = 0;

protected: //waveform (e.g. from airborne scanners)
    //点云图像融合数据
    std::vector<pointImage> m_imageSourse;
