...
template<class T> void PCLCloudToCCCloud(const PCLCloud& pclCloud, ccPointCloud& ccCloud)
{
    ...
    //Melone 20221219 PCD加载大坐标偏移
    unsigned int nbPointsRead = 0;
    CCVector3d PShift(0,0,0);
	for (size_t i = 0; i < pointCount; ++i)
	{
        //取第一个点判断大坐标系
        CCVector3d P(static_cast<PointCoordinateType>((int)pcl_cloud.at(i).x),
            static_cast<PointCoordinateType>((int)pcl_cloud.at(i).y),
            static_cast<PointCoordinateType>((int)pcl_cloud.at(i).z));
        if (nbPointsRead == 0)
        {
            //default scale
            double scale = 1.0;
            ...
            if (abs(P.x) >= 1.0e4 || abs(P.y) >= 1.0e4|| abs(P.z) >= 1.0e4)
            {
                needShift = true;
            }
            if (abs(diagonal*scale) > 1.0e12)
            {
                needRescale = true;
            }
            if (needShift || needRescale)
            {
                ...
                CCVector3d alreadyExistsShift(shiftsettings.value("x", true).toDouble(), shiftsettings.value("y", true).toDouble(), shiftsettings.value("z", true).toDouble());
                double alreadyExistsScale = shiftsettings.value("scale", true).toDouble();
                shiftsettings.endGroup();
                CCVector3d  Ptem = P * alreadyExistsScale + alreadyExistsShift;
                if (abs(Ptem.x) >= 1.0e4 || abs(Ptem.y) >= 1.0e4 || abs(Ptem.z) >= 1.0e4)
                {
                    needShift = true;
                }
                else
                ...
                if (abs(diagonal*alreadyExistsScale) > 1.0e12)
                {
                    needRescale = true;
                }
                else
                ...
                if (needShift || needRescale)
                {
                    CCVector3d PShift(std::abs(P.x) >= 1.0e4 ? -P.x : 0,
                        std::abs(P.y) >= 1.0e4 ? -P.y : 0,
                        std::abs(P.z) >= 1.0e4 ? -P.z : 0);

                    //round-off to the nearest hundred
                    PShift.x = static_cast<int>(PShift.x / 100) * 100.0;
                    PShift.y = static_cast<int>(PShift.y / 100) * 100.0;
                    PShift.z = static_cast<int>(PShift.z / 100) * 100.0;
                    double par = -static_cast<double>(ceil(log(diagonal / 1.0e12)));
                    double suggestedScale = diagonal < 1.0e12 ? 1.0 : pow(10.0, par);
                    scale = suggestedScale;
                    //判断两个偏移值距离，小于设定阈值，使用前一个
                    if ((PShift - alreadyExistsShift).norm() <= 100000
                        && std::abs(suggestedScale - alreadyExistsScale) <= CCCoreLib::ZERO_TOLERANCE_D)
                    {
                        PShift = alreadyExistsShift;
                        scale = alreadyExistsScale;
                    }
                    ccCloud.setGlobalShift(PShift);
                    ccCloud.setGlobalScale(scale);
                }
                else
                {
                    ccCloud.setGlobalShift(alreadyExistsShift);
                    ccCloud.setGlobalScale(alreadyExistsScale);
                }
            }
            nbPointsRead++;
        }
		CCVector3 pShift(pcl_cloud.at(i).x * ccCloud.getGlobalScale() + ccCloud.getGlobalShift().x,
            pcl_cloud.at(i).y * ccCloud.getGlobalScale() + ccCloud.getGlobalShift().y,
            pcl_cloud.at(i).z * ccCloud.getGlobalScale() + ccCloud.getGlobalShift().z);
		ccCloud.addPoint(pShift);
	}
}
