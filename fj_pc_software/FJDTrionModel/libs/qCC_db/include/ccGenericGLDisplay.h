...
struct ccGLCameraParameters
{
	...
	//计算平行投影下已知真实坐标z和屏幕点坐标，计算真实坐标,carl
	inline bool unprojectz(const CCVector3d& input2D, CCVector3d& output3D,const double & z) const { return ccGL::Unprojectz<double, double>(input2D, modelViewMat.data(), projectionMat.data(), viewport, output3D,z); }

	//计算平行投影下已知真实坐标y和屏幕点坐标，计算真实坐标,carl
	inline bool unprojecty(const CCVector3d& input2D, CCVector3d& output3D, const double & y) const { return ccGL::Unprojecty<double, double>(input2D, modelViewMat.data(), projectionMat.data(), viewport, output3D, y); }

	//计算平行投影下已知真实坐标x和屏幕点坐标，计算真实坐标,carl
	inline bool unprojectx(const CCVector3d& input2D, CCVector3d& output3D, const double & x) const { return ccGL::Unprojectx<double, double>(input2D, modelViewMat.data(), projectionMat.data(), viewport, output3D, x); }
...
public:
	...
	**/
	virtual void displayText(	QString text,
								...
								const QFont* font = nullptr, const ccColor::Rgba* backcolor = nullptr) = 0;
	...
	**/
    ...
    virtual QString windowName() { return QString(); }
};
