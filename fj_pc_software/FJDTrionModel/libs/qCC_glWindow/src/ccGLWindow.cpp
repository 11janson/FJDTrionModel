...
#include "libs/model/metahubclippingboxsegmentation.h"
//qCC_db
#include <cc2DLabel.h>
...
//Max moving duration for enabling picking mode (in ms)
constexpr int CC_MAX_MOVING_CLICK_DURATION_MS = 50;
    ...
    , m_strName("")
{
    ...
    m_deferredMovingTimer.setSingleShot(true);
    m_deferredMovingTimer.setInterval(50);

	//signal/slot connections
	connect(this, &ccGLWindow::itemPickedFast, this, &ccGLWindow::onItemPickedFast, Qt::DirectConnection);
    connect(&m_lodShowTimer, &QTimer::timeout, this, [=]() { 
        bool islod = m_isShowLodStyle;
        m_isShowLodStyle = false;
        m_lodShowTimer.stop(); 
        if (islod)
        {
            update();
	...
	connect(&m_deferredPickingTimer, &QTimer::timeout, CS::Model::ProjectModel::instance(), &CS::Model::ProjectModel::signalGLWindowsMouseReleasePicking);
...
void ccGLWindow::SetViewLock(bool islocked)
{
	m_Viewlock = islocked;
}
...
ccGLWindow::~ccGLWindow()
{
    ...
    QOpenGLFunctions_3_3_Core* glFunc3 = (context() ? context()->versionFunctions<QOpenGLFunctions_3_3_Core>() : nullptr);
    if (glFunc3)
    {
        glFunc3->glDeleteProgram(m_meshNormalShaderProgram);
        glFunc3->glDeleteProgram(m_pointcloudNormalShaderProgram);
        glFunc3->glDeleteProgram(m_imageMeshShaderProgram);
    }
	delete m_winDBRoot;
	m_winDBRoot = nullptr;
...
bool ccGLWindow::initialize()
{
                    ...
                    //加载着色器
                    {
                        ...
                        if (glFunc3)
                        {

                            {
                                ...
                                vertexShader = glFunc3->glCreateShader(GL_VERTEX_SHADER);
                                QString vertexShaderStr = ccGLUtils::readShaderFile(QStringLiteral("%1/ColorRamp/meshnormal_vertShader.glsl").arg(*s_shaderPath));
                                std::string verShaderString = vertexShaderStr.toStdString();
                                const char* vertexshaderchar = verShaderString.c_str();
                                glFunc3->glShaderSource(vertexShader, 1, &vertexshaderchar, NULL);
                                glFunc3->glCompileShader(vertexShader);

                                unsigned int geometryShader;
                                geometryShader = glFunc3->glCreateShader(GL_GEOMETRY_SHADER);
                                QString geometryShaderStr = ccGLUtils::readShaderFile(QStringLiteral("%1/ColorRamp/meshnormal_geometryShader.glsl").arg(*s_shaderPath));
                                std::string geometryShaderString = geometryShaderStr.toStdString();
                                const char* geometryshaderchar = geometryShaderString.c_str();
                                glFunc3->glShaderSource(geometryShader, 1, &geometryshaderchar, NULL);
                                glFunc3->glCompileShader(geometryShader);

                                unsigned int fragmentShader;
                                fragmentShader = glFunc3->glCreateShader(GL_FRAGMENT_SHADER);
                                QString fragmentShaderStr = ccGLUtils::readShaderFile(QStringLiteral("%1/ColorRamp/meshnormal_fragShader.glsl").arg(*s_shaderPath));
                                std::string fragmentShaderString = fragmentShaderStr.toStdString();
                                const char* fragmentshaderchar = fragmentShaderString.c_str();
                                glFunc3->glShaderSource(fragmentShader, 1, &fragmentshaderchar, NULL);
                                glFunc3->glCompileShader(fragmentShader);

                                m_meshNormalShaderProgram = glFunc3->glCreateProgram();
                                glFunc3->glAttachShader(m_meshNormalShaderProgram, vertexShader);
                                glFunc3->glAttachShader(m_meshNormalShaderProgram, geometryShader);
                                glFunc3->glAttachShader(m_meshNormalShaderProgram, fragmentShader);
                                glFunc3->glLinkProgram(m_meshNormalShaderProgram);

                                glFunc3->glDeleteShader(vertexShader);
                                glFunc3->glDeleteShader(geometryShader);
                                glFunc3->glDeleteShader(fragmentShader);
                            }
                            {
                                //点云带法向量
                                unsigned int vertexShader;
                                vertexShader = glFunc3->glCreateShader(GL_VERTEX_SHADER);
                                QString vertexShaderStr = ccGLUtils::readShaderFile(QStringLiteral("%1/ColorRamp/pointcloudnormal_vertShader.glsl").arg(*s_shaderPath));
                                std::string verShaderString = vertexShaderStr.toStdString();
                                const char* vertexshaderchar = verShaderString.c_str();
                                glFunc3->glShaderSource(vertexShader, 1, &vertexshaderchar, NULL);
                                glFunc3->glCompileShader(vertexShader);
                                ...
                                unsigned int fragmentShader;
                                fragmentShader = glFunc3->glCreateShader(GL_FRAGMENT_SHADER);
                                QString fragmentShaderStr = ccGLUtils::readShaderFile(QStringLiteral("%1/ColorRamp/pointcloudnormal_fragShader.glsl").arg(*s_shaderPath));
                                std::string fragmentShaderString = fragmentShaderStr.toStdString();
                                const char* fragmentshaderchar = fragmentShaderString.c_str();
                                glFunc3->glShaderSource(fragmentShader, 1, &fragmentshaderchar, NULL);
                                glFunc3->glCompileShader(fragmentShader);
                                ...
                                m_pointcloudNormalShaderProgram = glFunc3->glCreateProgram();
                                glFunc3->glAttachShader(m_pointcloudNormalShaderProgram, vertexShader);
                                glFunc3->glAttachShader(m_pointcloudNormalShaderProgram, fragmentShader);
                                glFunc3->glLinkProgram(m_pointcloudNormalShaderProgram);
                                glFunc3->glDeleteShader(vertexShader);
                                glFunc3->glDeleteShader(fragmentShader);
                            }
                            {
                                ...
                                unsigned int vertexShader;
                                vertexShader = glFunc3->glCreateShader(GL_VERTEX_SHADER);
                                QString vertexShaderStr = ccGLUtils::readShaderFile(QStringLiteral("%1/ColorRamp/imagemesh_vertShader.glsl").arg(*s_shaderPath));
                                std::string verShaderString = vertexShaderStr.toStdString();
                                const char* vertexshaderchar = verShaderString.c_str();
                                glFunc3->glShaderSource(vertexShader, 1, &vertexshaderchar, NULL);
                                glFunc3->glCompileShader(vertexShader);
                                int success;
                                glFunc3->glGetShaderiv(vertexShader, GL_COMPILE_STATUS, &success);
                                if (!success)
                                {
                                    int logLength;
                                    char message[1024];
                                    glFunc3->glGetShaderiv(vertexShader, GL_INFO_LOG_LENGTH, &logLength);
                                    if (logLength > 0)
                                    {
                                        glFunc3->glGetShaderInfoLog(vertexShader, logLength, NULL, message);
                                        auto mes = message;
                                        // 打印错误信息
                                    }
                                }
                                unsigned int fragmentShader;
                                fragmentShader = glFunc3->glCreateShader(GL_FRAGMENT_SHADER);
                                QString fragmentShaderStr = ccGLUtils::readShaderFile(QStringLiteral("%1/ColorRamp/imagemesh_fragShader.glsl").arg(*s_shaderPath));
                                std::string fragmentShaderString = fragmentShaderStr.toStdString();
                                const char* fragmentshaderchar = fragmentShaderString.c_str();
                                glFunc3->glShaderSource(fragmentShader, 1, &fragmentshaderchar, NULL);
                                glFunc3->glCompileShader(fragmentShader);
                                glFunc3->glGetShaderiv(fragmentShader, GL_COMPILE_STATUS, &success);
                                if (!success)
                                {
                                    int logLength;
                                    char message[1024];
                                    glFunc3->glGetShaderiv(fragmentShader, GL_INFO_LOG_LENGTH, &logLength);
                                    if (logLength > 0)
                                    {
                                        glFunc3->glGetShaderInfoLog(fragmentShader, logLength, NULL, message);
                                        auto mes = message;
                                        // 打印错误信息
                                    }
                                }

                                m_imageMeshShaderProgram = glFunc3->glCreateProgram();
                                glFunc3->glAttachShader(m_imageMeshShaderProgram, vertexShader);
                                glFunc3->glAttachShader(m_imageMeshShaderProgram, fragmentShader);
                                glFunc3->glLinkProgram(m_imageMeshShaderProgram);
                                glFunc3->glGetProgramiv(m_imageMeshShaderProgram, GL_LINK_STATUS, &success);
                                if (!success)
                                {
                                    int logLength;
                                    char message[1024];
                                    glFunc3->glGetProgramiv(m_imageMeshShaderProgram, GL_INFO_LOG_LENGTH, &logLength);
                                    if (logLength > 0)
                                    {
                                        glFunc3->glGetProgramInfoLog(m_imageMeshShaderProgram, logLength, NULL, message);
                                    }
                                }
                                glFunc3->glDeleteShader(vertexShader);
                                glFunc3->glDeleteShader(fragmentShader);
    ...
    PublicUtils::updateRGBColorBrightContrastValue(parameters.m_nBrightness, parameters.m_nContrast);
    PublicUtils::exeInitRGBColorBrightContrastValue(15, 60, std::byte(255));
...
{
	auto curtype = evt->type();
	if (evt->type() == QEvent::WindowActivate && ccGui::Parameters().m_isSectionAnalysis)
	{
		m_isSectionAnalysisFirstPressIgnore = true;
	}
	switch (evt->type())
	{
	//Gesture start/stop
...
void ccGLWindow::resizeGL(int w, int h)
{
    ...
    if (objectName().compare("2D View") != 0)
    {
        setLODEnabled(true, true);
    }
	m_currentLODState.level = 0;

	/*displayNewMessage(	QString(QApplication::translate("ccGLWindow","New size = %1 * %2 (px)",nullptr)).arg(glWidth()).arg(glHeight()),
						ccGLWindow::LOWER_LEFT_MESSAGE,
						false,
						2,
						SCREEN_SIZE_MESSAGE);*/
    m_lodShowTimer.stop();
    m_lodShowTimer.start(1000);
#ifdef CC_GL_WINDOW_USE_QWINDOW
	requestUpdate();
#else
	logGLError("ccGLWindow::resizeGL");
...
#endif
	emit signalPainteOnce();
}

void ccGLWindow::setIsNeedDrawSelectRect(bool isDraw)
{
	m_isNeedDrawSelectRect = isDraw;
}
...
void ccGLWindow::drawBackground(CC_DRAW_CONTEXT& CONTEXT, RenderingParams& renderingParams)
{
	...
	/****************************************/
	glFunc->glPointSize(std::clamp((m_viewportParams.defaultPointSize - 0.8), 0.2, double(m_viewportParams.defaultPointSize)));
	...
	//DGM: useless for now
	if (CONTEXT.display->windowName().compare("2D View") == 0 && m_bDrawBackgroundGridEnable)
	{
		int w = glWidth()*1.5;
		int h = glHeight()*1.5;
		double oneW = w / 80.0;
		...
		double value = acos(p.dot(q) / (p.norm()*q.norm()))*(180 / M_PI);
		//glFunc->glRotatef(value, 0.0, 0.0, 1.0);
		...
		for (int i = -80; i <= 80; i++)
		{
			...
			glFunc->glColor4f(0.45, 0.45, 0.45, 0.25);
			glFunc->glVertex2i(i*oneW, -h);
			glFunc->glVertex2i(i*oneW, h);
			glFunc->glEnd();
...
void ccGLWindow::fullRenderingPass(CC_DRAW_CONTEXT& CONTEXT, RenderingParams& renderingParams)
{
    ...
    if (currentFBO)
    {
        CONTEXT.activeFboid = currentFBO->getID();
    }
	/******************/
	/*** BACKGROUND ***/
	/******************/
...
void ccGLWindow::drawCrossRect(CC_DRAW_CONTEXT & context)
{
    std::vector<CCVector3d> pts = m_rect;
    if (pts.size() != 2)
        return;
    ...
    CCVector3d pt1 = pts[0];
    CCVector3d pt2 = pts[1];
    CCVector3d startPt, endPt;
    bool isDot = false;
    if (pt1.x > pt2.x ) {
        startPt = pt1;
        endPt = pt2;
    ...
    ccColor::Rgba fillColor = isDot == true? ccColor::blue :ccColor::green;
    fillColor.a = 0.2;

    ccColor::Rgba strokeColor(1, 1, 1, 1); // White stroke color with full opacity
    glFunc->glClear(GL_DEPTH_BUFFER_BIT);
    glFunc->glEnable(GL_COLOR_MATERIAL);
    glFunc->glColorMaterial(GL_FRONT_AND_BACK, GL_AMBIENT_AND_DIFFUSE);
    ...
    glFunc->glColor4f(fillColor.r, fillColor.g, fillColor.b, fillColor.a);
    glFunc->glBegin(GL_QUADS);
    glFunc->glVertex2f(startPt.x, startPt.y);
    glFunc->glVertex2f(startPt.x, endPt.y);
    glFunc->glVertex2f(endPt.x, endPt.y);
    glFunc->glVertex2f(endPt.x, startPt.y);
    glFunc->glEnd();

    // Draw stroke around rectangle
    glFunc->glLineWidth(1.0);
    glFunc->glColor4f(strokeColor.r, strokeColor.g, strokeColor.b, strokeColor.a);
...
void ccGLWindow::draw3D(CC_DRAW_CONTEXT& CONTEXT, RenderingParams& renderingParams)
{
    ...
    CONTEXT.meshNormalShaderProgram = m_meshNormalShaderProgram;
    CONTEXT.pointcloudNormalShaderProgram = m_pointcloudNormalShaderProgram;
    CONTEXT.imageMeshShaderProgram = m_imageMeshShaderProgram;
	...
	{
        if (windowName().compare("2D View") != 0)
        {
    ...
    //[!].绘制用户自己的数据
    drawUserDB(CONTEXT, renderingParams);
...
void ccGLWindow::drawForeground(CC_DRAW_CONTEXT& CONTEXT, RenderingParams& renderingParams)
{
	...
	/*** overlay entities ***/
	{
		...
		if (m_isNeedDrawSelectRect)
		{
			...
			glFunc->glBegin(GL_LINES);
			glFunc->glVertex3f(-(m_glViewport.width() / 2), (m_glViewport.height() / 2), 0.0f);
			glFunc->glVertex3f(-(m_glViewport.width() / 2), -(m_glViewport.height() / 2), 0.0f);
			glFunc->glVertex3f(-(m_glViewport.width() / 2), -(m_glViewport.height() / 2), 0.0f);
			glFunc->glVertex3f((m_glViewport.width() / 2), -(m_glViewport.height() / 2), 0.0f);
			glFunc->glVertex3f((m_glViewport.width() / 2), -(m_glViewport.height() / 2), 0.0f);
		...
		if (m_isDrawTitle)
		{
			...
			renderText(-(m_glViewport.width() / 2) + 10, (m_glViewport.height() / 2) - 30, 1, titleName, font);
		}
		...
		if (!m_captureMode.enabled)
		{
			...
			showGLFilterRibbon = false;//janson 2022.7.1
...
void ccGLWindow::drawUserDB(CC_DRAW_CONTEXT & context, RenderingParams & params)
{
    if (!m_pUserDBRoot){
        return;
    }

    ccPointCloud *pC = ccHObjectCaster::ToPointCloud(m_pUserDBRoot);
    if (!pC) {
        return;
    }

    //[!].绘制自己的数据
    pC->drawUserDb(context);
}
...
void ccGLWindow::addToOwnDB(ccHObject* obj, bool noDependency/*=true*/)
{
	...
	if (m_winDBRoot)
	{
		...
		//obj->setDisplay(this);
        obj->setDisplay_recursive(this); //将显示窗口传给子节点 janson 2022.8.2
	}
	else
	{
...
void ccGLWindow::setDrawUserDB(ccHObject * pEntity)
{
    m_pUserDBRoot = pEntity;
}

void ccGLWindow::zoomGlobal(bool isopen)
{
	if (!isopen)
	{
		return;
	}
    emit signalExitFovLocked();
...
void ccGLWindow::updateConstellationCenterAndZoom(const ccBBox* boundingBox/*=nullptr*/)
{
    ...
    pos.y = P.y;
    pos.z = v.z;
    emit signalCramerInitialStatePos(pos);
	//just in case
	invalidateViewport();
	invalidateVisualization();
	deprecate3DLayer();
...
void ccGLWindow::moveCamera(CCVector3d& v)
{
    ...
    auto camearpos = m_viewportParams.getCameraCenter();
								...
								bool verbose/*=false*/)
{
	...
	if (verbose)
	{
		...
		displayNewMessage(QString(QApplication::translate("ccGLWindow","Point (%1 ; %2 ; %3) set as rotation center",nullptr))
...
void ccGLWindow::calcPlottingScale()
{
    ...
    if (windowName().compare("2D View") == 0 && textEquivalentWidth <= 10e-4)
        m_bWheelEnable = false;
    else
        m_bWheelEnable = true;
	m_viewportParams.m_dViewEquivalentWidth = textEquivalentWidth;
	calculatePointSizeByZoomRatio();
}
...
void ccGLWindow::drawTrihedron()
{
	...
	// now display the X, Y and Z axis labels
	{
		...
		if ((objectName() == "viewer3d" || objectName() == "viewer3D" || objectName() == "viewer2D") && m_isShowZAxis)
		{
			glFunc->glColor3f(0.0f, 0.7f, 1.0f);
			{
...
ccGLMatrixd ccGLWindow::computeProjectionMatrix(bool withGLfeatures, ProjectionMetrics* metrics/*=nullptr*/, double* eyeOffset/*=nullptr*/) const
{
	...
	//we deduce zFar
	double zNear = cameraCenterToRotationCentertDist - rotationCenterToFarthestObjectDist * 2.0;
	double zFar = cameraCenterToRotationCentertDist + rotationCenterToFarthestObjectDist * 2.0;//扩大zFar, 减小zNear以增加远近裁剪面范围
...
void ccGLWindow::getContext(CC_DRAW_CONTEXT& CONTEXT)
{
    ...
    CONTEXT.labelOriginalMarkerSize = static_cast<float>(guiParams.labelMarkerSize);
	...
	CONTEXT.lengthunit = guiParams.lengthunit;
	CONTEXT.diameterunit = guiParams.diameterunit;
	CONTEXT.angleunit = guiParams.angleunit;
	CONTEXT.areaunit = guiParams.areaunit;
	CONTEXT.volumnunit = guiParams.volumnunit;
	CONTEXT.isshowUnitlabel = guiParams.isshowUnitlabel;
    CONTEXT.visibleEnable = m_bVisibleEnable;
}
...
{
	ccGui::ParamStruct parameters = ccGui::Parameters();
	if (parameters.m_bGraphicsElement){
		return;
	}
...
void ccGLWindow::updateActiveItemsList(int x, int y, bool extendToSelectedLabels/*=false*/)
{
	...
	if (m_activeItems.size() == 1)
	{
        ...
        //[!].如果是注释
        ccNote* pNote = dynamic_cast<ccNote*>(pickedObj);
        if (pNote)
        {
            if (m_pickingMode != NOTE_PICKING) {
                emit entitySelectionChanged(NULL);
            }


            ccHObject::Container labels;
            if (m_globalDBRoot)
                m_globalDBRoot->filterChildren(labels, true, CC_TYPES::NOTE_2D);
            if (m_winDBRoot)
                m_winDBRoot->filterChildren(labels, true, CC_TYPES::NOTE_2D);

            for (auto & label : labels)
            {
                if (label->isA(CC_TYPES::NOTE_2D) && label->isVisible()) //Warning: cc2DViewportLabel is also a kind of 'CC_TYPES::LABEL_2D'!
                {
                    ccNote* l = static_cast<ccNote*>(label);
                    label->setSelected(false);
                }
            }

            if (!pNote->isSelected() || !extendToSelectedLabels)
            {

                if (m_pickingMode != NOTE_PICKING) {
                    emit entitySelectionChanged(pNote);
	...
	//选中剖面carl20221117
	{
		...
		if (m_globalDBRoot)
			m_globalDBRoot->filterChildren(rects, true, CC_TYPES::RECT_3D);

		for (auto & rect : rects)
		{
			if (rect->isA(CC_TYPES::RECT_3D) && rect->isVisible()) //Warning: cc2DViewportLabel is also a kind of 'CC_TYPES::LABEL_2D'!
			{
				ccSectionAnalysisLine* l = static_cast<ccSectionAnalysisLine*>(rect);
				if (l)
				{
					ccGLCameraParameters camera;
					getGLCameraParameters(camera);
					if (l->pointPicking(toCenteredGLCoordinates(x, y), camera))
					{
						m_activeItems.push_back(l);
						//setView(CC_TOP_VIEW);
					}
...
void ccGLWindow::onItemPickedFast(ccHObject* pickedEntity, int pickedItemIndex, int x, int y)
{
	...
	if (pickedEntity->getMetaData("attachedentity").toBool()){
		ccInteractor* pItem = dynamic_cast<ccInteractor*>(pickedEntity);
		if (pItem){
			m_activeItems.push_back(pItem);
		}
	}
	...
	else if (pickedEntity->isA(CC_TYPES::NOTE_2D))
	{
        ccNote* label = static_cast<ccNote*>(pickedEntity);
        m_activeItems.push_back(label);
	}

    else if (pickedEntity->isA(CC_TYPES::STATUC_LABEL)) {
        ccStaticLabel *pStaticLabel = static_cast<ccStaticLabel*>(pickedEntity);
        if (pStaticLabel) {
            emit CS::Model::ProjectModel::instance()->signalCurrentSelectStaticLableChanged(pStaticLabel);
        }
    }
}
...
{
    ccGui::ParamStruct parameters = ccGui::Parameters();
    parameters.m_CuttingBoxPickingOpen = false;
	...
	//[!].更新鼠标位置，信息
	QVector3D pos;
	updateGLWindowsMousePos(event->x(), event->y(), pos);

	emit activeSelectWindow();
	...
	{
		m_leftClickPos = event->posF();
		m_lastClickTime_ticks = m_timer.elapsed(); //in msec
        m_lastGraphicsPos = CCVector2d(event->x(), event->y());

		//left click = rotation
		ccGui::ParamStruct parameters = ccGui::Parameters();
		if ((m_interactionFlags & INTERACT_ROTATE) && !parameters.m_bGraphicsElement)
		{
        ...
        if (m_interactionFlags & INTERACT_2D_ITEMS)
		{
          ...
          emit CS::Model::ProjectModel::instance()->signalGLWindowsMousePressed(pos);
        }
        //updateCrossSelect(CCVector3d(event->x(), event->y(), 0), false);       
	}
...
void ccGLWindow::mouseDoubleClickEvent(QMouseEvent *event)
{
    ...
    ccGui::ParamStruct parameters = ccGui::Parameters();
    //parameters.m_enableCrossSelect = false;
    ccGui::Set(parameters);
    ...
    if (event->button() == Qt::LeftButton){
        qDebug() << "update activate is  enter";
        emit leftButtonDoubleClicked(event->x(), event->y());
        updateActiveItemsList(event->x(), event->y(), false);
        qDebug() << "update activate is  end";
    }
    ...
    if (!m_activeItems.empty()){
        ...
        m_activeItems.clear();
        ccHObject* pickedEntity = dynamic_cast<ccHObject*>(item);
        if (!pickedEntity){
            return;
        }

        if (pickedEntity->isA(CC_TYPES::NOTE_2D)){
           ccNote* pNote = dynamic_cast<ccNote*>(pickedEntity);
           if (pNote) {
               CS::Model::ProjectModel::instance()->setActivatedNote(pNote);
               return;
           }
        }
        ...
        //[!].没有选择
        qDebug() << "getClick3DPos is  enter";
        CCVector3d P;
        bool bValid = false;
        if (getClick3DPos(x, y, P, false) && parameters.m_canChangePivotPoint) {
            setPivotPoint(P, true, true);
            bValid = true;
        }
        qDebug() << "getClick3DPos is  end";

        //[!].鼠标左键 双击选中实体上
        if (getClick3DPos(x, y, P, true)){
            if ((event->button() == Qt::LeftButton) && bValid) {
                emit signalMouseLeftDoubleClickEntity(CCVector3d(x, y, 0));
            }
        }
...
void ccGLWindow::refreshCuttingBoxPickingStatus(QMouseEvent* event)
{
    PickingParameters params(FAST_PICKING, event->x(), event->y(), 2, 2);
    std::unordered_set<int> selectedIDs;
    int pickedItemIndex = -1;
    int selectedID = -1;
    ccClipBox* pBBox = nullptr;
    ccHObject* pickedEntity = getPickingEntity(params, selectedIDs, pickedItemIndex, selectedID);
    if (!pickedEntity) {
        std::vector<ccHObject*> allObject;
        m_winDBRoot->filterChildren(allObject, true, CC_TYPES::CLIPPING_BOX);
        for (ccHObject* node : allObject) {
            pBBox = dynamic_cast<ccClipBox*>(node);
            if (pBBox) {
                if (pBBox->getActiveComponent() != 0) {
                    pBBox->setActiveComponent(0);
    ...
    {
        pBBox = dynamic_cast<ccClipBox*>(pickedEntity);
        if (pBBox) {
            if (pBBox->getActiveComponent() != pickedItemIndex) {
                pBBox->setActiveComponent(pickedItemIndex);
                emit CS::Model::ProjectModel::instance()->signalRedrawAllWindows();
            }
        }
    }
...
{
    ccGui::ParamStruct parameters = ccGui::Parameters();
    if (parameters.m_CuttingBoxisOpen && parameters.m_CuttingBoxPickingOpen) {
        refreshCuttingBoxPickingStatus(event);
    }
	//过滤无效左键移动carl20221124
	if ((event->buttons() & Qt::LeftButton))
	{
		if (m_leftClickPos == QPointF(-1, -1))
		{
			return;
		}
		QPointF offset = event->pos() - m_leftClickPos;
...
//#define DEBUG_MOUSE_MOVE_FREQ
	...
	const int x = event->x();
	const int y = event->y();
	if (m_interactionFlags & INTERACT_SIG_MOUSE_MOVED || parameters.m_isTwoPointCuttingOn)
	{
		emit mouseMoved(x, y, event->buttons());
		event->accept();
	}
	CCVector3d currentP3d;
	covertScreenCoord3DPos(QPoint(event->x(), event->y()), currentP3d);
	
	//no button pressed
	if (event->buttons() == Qt::NoButton)
	{
		emit CS::Model::ProjectModel::instance()->signalGLWindowsMouseMoveEfficiency(currentP3d);
        //处理按住鼠标左键移动到主窗口其他控件也显示坐标的问题
        emit sigTransScreenPosTo3DPos(currentP3d, QPointF(event->x(), event->y()));

     

        CC_DRAW_CONTEXT context;
        getContext(context);
        //if (context.display->windowName().compare("2D View") == 0 && !stGraphicsParams.m_isPaintMode)
        {
            //鼠标hover事件选中二维矢量图元
            m_lastMouseMovePos = event->pos();
            m_lastMoveTime_ticks = m_timerMove.elapsed(); //in msec

            if (m_timerMove.elapsed() < m_lastMoveTime_ticks + CC_MAX_MOVING_CLICK_DURATION_MS) //in msec
            {
                int x = m_lastMouseMovePos.x();
                int y = m_lastMouseMovePos.y();

                //first test if the user has clicked on a particular item on the screen
                if (!processClickableItems(x, y))
                {
                    m_lastMouseMovePos = event->pos(); //just in case (it should be already at this position)
                    const ccGui::ParamStruct& displayParams = getDisplayParameters();
                    if ((displayParams.singleClickPicking && m_pickingMode != NO_PICKING))
                    {
                        if (!m_isSectionAnalysisFirstPressIgnore)
                        {
                            ccGui::ParamStruct params = ccGui::Parameters();
                            ccGui::Set(params);
                            m_deferredMovingTimer.start();
                        }
                    }
                }
            }
		...
		//display the 3D coordinates of the pixel below the mouse cursor (if possible)
		{
			...
			m_bMosuePickupMoveEnable = parameters.m_bGraphicsElement ? m_bMosuePickupMoveEnable : false;

		}
		if (parameters.m_bGraphicsElement) {

			emit CS::Model::ProjectModel::instance()->signalGLWindowsMouseMove(pos);
		}
		
        updateCrossSelect(CCVector3d(event->x(), event->y(), 0), true, false);
		...
		)
	{
		...
		if (m_interactionFlags & INTERACT_PAN && !parameters.m_isImageMeshOnlyShow)
		{
			//displacement vector (in "3D")
			double pixSize = computeActualPixelSize();
			if (m_viewportParams.perspectiveView)
			{
				//此处修改是为了解决放大点云后右键拖拽反向或无作用的问题
				pixSize = std::abs(pixSize);
				if (pixSize < 0.001)
				{
					pixSize += 0.001;
				}
			...
			emit segmentPositionChanged(event->x(),event->y());
		} //if (m_interactionFlags & INTERACT_PAN)
	}
	else if (event->buttons() & Qt::LeftButton) //rotation
	{
		...
		if (m_interactionFlags & INTERACT_2D_ITEMS)
		{
			...
			{
				CCVector3d mousePos;
                if (parameters.m_canChangePivotPoint && covertScreenCoordNear3DPos(QPoint(x, y), mousePos)&& parameters.m_bViewRotateEanble) {
                    setPivotPoint(mousePos, true, true);
                }
				...
				if (//m_pickingMode != NO_PICKING
					...
					(QApplication::keyboardModifiers() == Qt::NoModifier
						||	QApplication::keyboardModifiers() == Qt::ControlModifier) && !parameters.m_bGraphicsElement)
				{
					updateActiveItemsList(m_lastMousePos.x(), m_lastMousePos.y(), true);
				}
				else {
					updateActiveGraphics(pos);
				}
		...
		if (!m_activeItems.empty())
		{
			...
			{
				//控制拖拽2D标签不超出主窗口范围
				if (x>=5 && x<= (glWidth()-5) && y>=5 && y <= (glHeight() - 5))
				{
			...
			ccGLCameraParameters camera;
			getGLCameraParameters(camera);
			for (auto curRect : m_activeItems)
			{
				ccSectionAnalysisLine* SectionRect = dynamic_cast<ccSectionAnalysisLine*>(curRect);
				if (SectionRect)
				{
					if (SectionRect->isA(CC_TYPES::RECT_3D) && SectionRect->isVisible())
					{
						SectionRect->changePosition(toCenteredGLCoordinates(x, y), camera);
						deprecate3DLayer();
					}
				}
			}
		...
		else
		{
			...
			ccGui::ParamStruct params = ccGui::Parameters();
			if (m_allowRectangularEntityPicking && params.m_enableCrossSelect
			...
			{
				//[!].是否旋转
				if (!parameters.m_bViewRotateEanble) {
					return;
				}
				...
				case BubbleViewMode:
				{
					QPoint posDelta = event->pos() - m_lastMousePos;
				...
				case StandardMode:
				{
                    //[!].支持两种模式
                    ccGui::ParamStruct parameters = ccGui::Parameters();
                    bool bBubbleRotationModeOn = parameters.m_bBubbleRotationModeOn;
                    if (m_strName.compare("3D View") == 0 && bBubbleRotationModeOn){
                        QPoint posDelta = event->pos() - m_lastMousePos;
                        if (parameters.m_isImageMeshOnlyShow)
                        {
                            posDelta = m_lastMousePos - event->pos();
                        }
                        if (std::abs(posDelta.x()) != 0){
                            double delta_deg = (posDelta.x() * static_cast<double>(m_bubbleViewFov_deg)) / height();
                            //rotation about the sensor Z axis
                        ...
                        if (QApplication::keyboardModifiers() & Qt::ShiftModifier){
                            ...
                            if (parameters.m_isImageMeshOnlyShow)
                            {
                                rotMat = ccGLMatrixd::FromToRotation(currentMouseOrientation, s_lastMouseOrientation);
                            }
                        }
				...
				case LockedAxisMode:
				{
					...
					if (topView)
					{
						...
						if (u_norm > 1.0e-6)
						{
							...
							//determine the rotation direction
							if (u.z * m_lockedRotationAxis.z > 0 
                                && (abs(m_lockedRotationAxis.x) + abs(m_lockedRotationAxis.y) + abs(m_lockedRotationAxis.z) - 1.0) < EPSINON)
							{
								sin_angle = -sin_angle;
							}
							if (((m_lockedRotationAxis.x > 0 && u.z > 0) || (m_lockedRotationAxis.y > 0 && u.z < 0))
                                && (abs(m_lockedRotationAxis.x) + abs(m_lockedRotationAxis.y) + abs(m_lockedRotationAxis.z) - 1.0) < EPSINON)
							{
								sin_angle = -sin_angle;
							}
                            //处理任意轴逆时针方向旋转
                            if (u.z > 0 && (abs(m_lockedRotationAxis.x) + abs(m_lockedRotationAxis.y) + abs(m_lockedRotationAxis.z) - 1.0) > EPSINON)
                            {
                                sin_angle = -sin_angle;
                            }
	...
	else if ((event->buttons() & Qt::MiddleButton)) // zoom
	{
		...
		//ccGLWindow起zoom作用的函数 janson
		if ((m_interactionFlags & INTERACT_ZOOM_CAMERA) == INTERACT_ZOOM_CAMERA)
		{
	...
	//[!].计算移动距离防止误触发
	CCVector3d dl = (CCVector3d(event->pos().x(), event->pos().y(), 0) - CCVector3d(m_lastMousePos.x(), m_lastMousePos.y(), 0));
	if (dl.norm() > 5){//默认按照大于5px,移动移动
		m_mouseMoved = true;
	}
	m_lastMousePos = event->pos();
...
//框选计算有问题，会出现全部选中情况，暂不使用
void ccGLWindow::updateCrossSelect(CCVector3d pt, bool isMoveing, bool bClear)
{
	
    const ccGui::ParamStruct& params = ccGui::Parameters();
    if (!params.m_enableCrossSelect || !(QApplication::keyboardModifiers() & Qt::ControlModifier) || bClear)
    {
		if (!m_rect.empty()){
			m_rect.clear();
			redraw(true, false);
    ...
    if (!isMoveing)
    {
        ...
        if (m_rect.size() == 2)
        {
           /* m_rect[0].y = glHeight() - 1 - m_rect[0].y;
            m_rect[1].y = glHeight() - 1 - m_rect[1].y;
            int x = 0.5*(m_rect[0] + m_rect[1]).x;
            int y = 0.5*(m_rect[0] + m_rect[1]).y;
            int w = fabs((m_rect[0] - m_rect[1]).x);
            int h = fabs((m_rect[0] - m_rect[1]).y);
            qDebug() << "w = " << w << "h = " << h;
            //异常值导致框选矩阵为整个视口
            if (w <= 1 || h <= 1)
            {
            ...
            PickingParameters params(FAST_PICKING, x, y, w, h);
            const int retinaScale = devicePixelRatio();
...
{
	m_leftClickPos = QPointF(-1, -1);
    ccGui::ParamStruct parameters = ccGui::Parameters();
    parameters.singleClickPicking = true;
    parameters.m_CuttingBoxPickingOpen = true;
    ccGui::Set(parameters);
	...
	if (((m_interactionFlags & INTERACT_SIG_BUTTON_RELEASED) || parameters.m_isManualClassificationOpen) && (event->button() == Qt::LeftButton) && !mouseHasMoved)
	{
		if (!m_isSectionAnalysisFirstPressIgnore)
		{
			emit leftButtonNoMovedRelease(event->x(), event->y());
		}
	}
    m_isSectionAnalysisFirstPressIgnore = false;
	if ((m_interactionFlags & INTERACT_SIG_BUTTON_RELEASED) && !(event->button() == Qt::RightButton))
	{
		emit buttonNoRightMovedRelease();
        if (parameters.m_ignoreSelectPoint)
        {
            return;
        }
	}
		...
		)
	{
		...
		else if ((m_interactionFlags & INTERACT_2D_ITEMS) && !parameters.m_bGraphicsElement)
		{
        ...
        if (!mouseHasMoved){
            ...
            if (parameters.m_bGraphicsElement)
            {
                ...
                emit CS::Model::ProjectModel::instance()->signalGLWindowsRightMouseClicked();

            }
			//分割时右键释放事件,carl
			if (m_interactionFlags & INTERACT_SIG_BUTTON_RELEASED || parameters.m_isMeasureOn)
			{
				emit rightButtonNoMovedRelease(event->x(), event->y());
                return;
			}
        }
	...
	else if (event->button() == Qt::LeftButton)
	{
		...
		if (mouseHasMoved)
		{
			...
			if (m_rectPickingPoly)
			{
				...
				//此处修改为了处理大点云点击卡顿
				m_isNeedUpdatePickingMode = false;
				
				if (QApplication::keyboardModifiers() & Qt::ShiftModifier && ccGui::Parameters().m_openShiftMeasure)
				{
					m_cashPickingMode = m_pickingMode;
					m_isNeedUpdatePickingMode = true;
				...
				if (m_isNeedUpdatePickingMode)
				{
					m_pickingMode = m_cashPickingMode;
					m_isNeedUpdatePickingMode = false;
				}
			}

			event->accept();
			//分割选点屏幕禁止旋转时，左键拖拽不重绘carl20221109
			if (!(m_interactionFlags & INTERACT_SIG_BUTTON_RELEASED))
			{
				toBeRefreshed();
			}
			//[!].开启二维矢量绘图
			const ccGui::ParamStruct& stGraphicsParams = ccGui::Parameters();
			if (stGraphicsParams.m_bGraphicsElement && !m_activeItems.empty()) {//鼠标移动编辑
				emit CS::Model::ProjectModel::instance()->signalGLWindowMouseLeftMovedRelease();
			}
		...
		else
		{
            //[!]此处增加的判断仅限二维矢量开启时
            ccGui::ParamStruct params = ccGui::Parameters();
			if (m_timer.elapsed() < m_lastClickTime_ticks + CC_MAX_PICKING_CLICK_DURATION_MS || !params.m_isPaintPageActive) //in msec
			{
		...
		ccGui::ParamStruct parameters = ccGui::Parameters();
		if (m_rect.size() >= 2 && parameters.m_enableCrossSelect && (QApplication::keyboardModifiers() & Qt::ControlModifier)) {
			emit CS::Model::ProjectModel::instance()->signalGLWindowsRectSelect(m_rect.at(0), m_rect.at(1));
			m_rect.clear();
		}
		m_activeItems.clear();
	}
...
void ccGLWindow::doPicking()
{
    ...
    {
        if (m_interactionFlags & INTERACT_2D_ITEMS /*&& m_pickingMode != NOTE_PICKING*/)
        {
            //label selection
            updateActiveItemsList(x, y, false);
            if (!m_activeItems.empty())
            {
                if (m_activeItems.size() == 1 && m_pickingMode != NOTE_PICKING)
                {
                    ccInteractor* pickedObj = m_activeItems.front();
                    cc2DLabel* label = dynamic_cast<cc2DLabel*>(pickedObj);
		...
		if (m_activeItems.empty() 
            && m_pickingMode != NO_PICKING 
            && m_pickingMode != NOTE_PICKING 
            && m_pickingMode != POINT_PICKING_ENTITY )
		{
			//perform standard picking
			PICKING_MODE pickingMode = m_pickingMode;

			//shift+click = point/triangle picking
			if (pickingMode == ENTITY_PICKING && (QApplication::keyboardModifiers() & Qt::ShiftModifier) && ccGui::Parameters().m_openShiftMeasure)
			{
        ...
        else if (/*m_activeItems.empty() && */(m_pickingMode == NOTE_PICKING || m_pickingMode == POINT_PICKING_ENTITY ))
        {
            PICKING_MODE pickingMode = m_pickingMode;
            PickingParameters params(pickingMode, x, y, m_pickRadius, m_pickRadius);
            startPicking(params);
...
void ccGLWindow::wheelEvent(QWheelEvent* event)
{
    ...
    if (parameters.m_isImageMeshOnlyShow && m_viewportParams.perspectiveView)
    {
        event->accept();
	...
	else if (m_interactionFlags & INTERACT_ZOOM_CAMERA)
	{
		...
		if (windowName().compare("2D View") == 0 && !m_bWheelEnable && wheelDelta_deg>=0)
		{
			return;
		}
		else if(windowName().compare("2D View") == 0)
		{
			m_bWheelEnable = true;
		}
	...
	if ((!(keyboardModifiers & Qt::ShiftModifier))  && m_viewportParams.perspectiveView && !parameters.m_isImageMeshOnlyShow)
	{
		if (15.0f != m_viewportParams.fov_deg)
		{
			setFov(15.0f);
			doRedraw = true;
		}
...
void ccGLWindow::calculatePointSizeByZoomRatio()
{
	double minScale = 0.1;
	double maxScale = 8;
	double minPointSize = 0.1;
	double maxPointSize = 7;

    double cameraToPointCloudDistance = m_viewportParams.m_dViewEquivalentWidth;
	double clampedScale = std::max(minScale, std::min(maxScale, cameraToPointCloudDistance));

	double pointSize = minPointSize + (maxScale - clampedScale) * (maxPointSize - minPointSize) / (maxScale - minScale);
	bool redrawState = false;
    if (m_viewportParams.m_fViewAutoPointSize != static_cast<float>(pointSize)){
		redrawState = true;
    }
	m_viewportParams.m_fViewAutoPointSize =static_cast<float>(pointSize);
	if (redrawState) {
        redraw();
    }
}

void ccGLWindow::enterEvent(QEvent *event)
{
    if (getDisplayParameters().m_bShowCapacityPoint 
        || getDisplayParameters().m_bNotePickGLEnable
		|| getDisplayParameters().m_bGraphicsPickStatus
		)
	{
        setPickCursor(true);
    }
...
void ccGLWindow::leaveEvent(QEvent * event)
{
...
void ccGLWindow::onWheelEvent(float wheelDelta_deg)
{
	...
	else
	{
		...
		if (m_viewportParams.objectCenteredView)
		{
			...
			if (abs(delta) < 0.8 && m_viewportParams.perspectiveView)
			{
				delta += (delta > 0.0 ? 0.5 : -0.5);
			}
		}
		else
		{
		...
		if (delta == 0 && parameters.m_isImageMeshOnlyShow && !m_viewportParams.perspectiveView)
		{
			double currentznum = m_viewportParams.getCameraCenter().z;
			double znum = (wheelDelta_deg < 0.0 ? -0.1:0.1);
			v.z = currentznum+ znum;
		}
		moveCamera(v);
        emit signalCramerCurrentPos(getViewportParameters().getCameraCenter());
	}
...
void ccGLWindow::startPicking(PickingParameters& params)
{
	...
	if (	params.mode == POINT_OR_TRIANGLE_PICKING
        ...
        ||  params.mode == NOTE_PICKING
        ||  params.mode == POINT_PICKING_ENTITY// = spawn a label on the clicked point or triangle
        )
	{
										...
										const std::unordered_set<int>* selectedIDs/*=nullptr*/)
{
	...
	else if (params.mode == LABEL_PICKING)
	{
		if (m_globalDBRoot && pickedEntity && pickedItemIndex >= 0)
		{
			...
			if (label)
			{
				...
				label->setMetaData("measuretype", 0);
    ...
    else if (params.mode == NOTE_PICKING)
    {
        if (m_globalDBRoot && pickedEntity && pickedItemIndex >= 0)
        {
            ...
            if (pickedEntity->isKindOf(CC_TYPES::POINT_CLOUD)){
                ...
                //[!].位置
                QPointF pose;
                pose.setX(static_cast<float>(params.centerX + 20) / glWidth());
                pose.setY(static_cast<float>(params.centerY + 20) / glHeight());

                if (!CS::Model::ProjectModel::instance()->getSelectedEntitys().empty()) {
                    ccNote *pNote = new ccNote();
                    cc2DLabel::PickedPoint pp;
                    pp._cloud = ccHObjectCaster::ToGenericPointCloud(pickedEntity);
                    pp.index = pickedItemIndex;
                    pp.entityCenterPoint = false;

                    //[!].新添加类型
                    pNote->setMetaData("notetypeadd", true);
                    pNote->setPickedPoint(pp);
                    pNote->setVisible(true);
                    pNote->setEnabled(true);
                    pNote->setTitle(QString::null);
                    pNote->setPosition(pose.x(), pose.y());
                    CS::Model::ProjectModel::instance()->setActivatedNote(pNote);
                }              
            }
        }
    }
    else if (params.mode == POINT_PICKING_ENTITY)
    {
        if (m_globalDBRoot && pickedEntity && pickedItemIndex >= 0)
        {
            ...
            if (pickedEntity->isKindOf(CC_TYPES::POINT_CLOUD)) 
            {
                //[!].位置
                QVector3D pose;
                ccGui::ParamStruct currentParameters = ccGui::Parameters();
                pose.setX(QString::number(nearestPoint->x ,'f', currentParameters.displayedNumPrecision).toFloat());
                pose.setY(QString::number(nearestPoint->y, 'f', currentParameters.displayedNumPrecision).toFloat());
                pose.setZ(QString::number(nearestPoint->z, 'f', currentParameters.displayedNumPrecision).toFloat());
                CS::Model::ProjectModel::instance()->setPickedEntityPointPos(pose, CCVector2d(params.centerX, params.centerY));
				CS::Model::ProjectModel::instance()->setPickedScreenPos(CCVector2d(params.centerX, params.centerY));
            }
        }
    }
}

ccHObject* ccGLWindow::getPickingEntity(const PickingParameters& params,
                                        std::unordered_set<int>& selectedIDs,
                                        int& pickedItemIndex,
                                        int& selectedID,
                                        bool is_movingPick)
...
void ccGLWindow::startOpenGLMovingPicking(const PickingParameters& params, CCVector2d pos2d, bool needSelect)
{
    std::unordered_set<int> selectedIDs;
    int pickedItemIndex = -1;
    int selectedID = -1;
    ccHObject* pEntity = getPickingEntity(params, selectedIDs, pickedItemIndex, selectedID, true);
		...
		while (!toProcess.empty())
		{
			...
			if (ent->isDisplayedIn(this))
			{
				if (ent->isKindOf(CC_TYPES::POINT_CLOUD))
				{
                    ...
                    const ccGui::ParamStruct& stparams = ccGui::Parameters();
					int nearestPointIndex = -1;
					double nearestSquareDist = 0.0;
					auto totalSize = cloud->size();
					if (cloud->pointPicking(clickedPos,
						...
						params.pickWidth,
						params.pickHeight,
						autoComputeOctree && cloud->size() > MIN_POINTS_FOR_OCTREE_COMPUTATION, stparams.m_isPlaneCut))
					{
                ...
                else if (ent->isA(CC_TYPES::NOTE_2D))
				{
...
void ccGLWindow::setPointSize(float size, bool silent/*=false*/)
{
	...
	if (m_viewportParams.defaultPointSize != newSize)
	{
		...
		if (!silent)
		{
			//displayNewMessage(	QString(QCoreApplication::translate("ccGLWindow", "New default point size: %1", nullptr)).arg(newSize),
			//					ccGLWindow::LOWER_LEFT_MESSAGE,
			//					false,
			//					2,
...
{
    //cc原有设置点setpointsize 不支持缩放切换。
	QFont font = m_font;
	font.setPixelSize(getLabelFontPointSize());
	return font;
}
...
void ccGLWindow::setSunLight(bool state)
{
	...
	/*displayNewMessage(	state ? QApplication::translate("ccGLWindow", "Sun light ON", nullptr) : QApplication::translate("ccGLWindow", "Sun light OFF", nullptr) ,
						ccGLWindow::LOWER_LEFT_MESSAGE,
						false,
						2,
...
void ccGLWindow::setCustomLight(bool state)
{
	...
	//displayNewMessage(	state ? QApplication::translate("ccGLWindow", "Custom light ON", nullptr)  : QApplication::translate("ccGLWindow", "Custom light OFF", nullptr),
	//					ccGLWindow::LOWER_LEFT_MESSAGE,
	//					false,
	//					2,
...
void ccGLWindow::drawPivot()
{
	...
	if (m_pivotGLList == GL_INVALID_LIST_ID)
	{
		...
		//draw a small sphere
		{
            ...
            sphere.setIsNeedReleas(false);
			sphere.showColors(true);
            sphere.showNormals(false);
		...
		//glDrawUnitCircle(context(), 0);//janson.yang 2022.6.16
		glFunc->glBegin(GL_LINES);
		...
		//glDrawUnitCircle(context(), 1);//janson.yang 2022.6.16
		glFunc->glBegin(GL_LINES);
		...
		//glDrawUnitCircle(context(), 2);//janson.yang 2022.6.16
		glFunc->glBegin(GL_LINES);
...
void ccGLWindow::setPerspectiveState(bool state, bool objectCenteredView)
{
	...
	/*	displayNewMessage(QApplication::translate("ccGLWindow", "Perspective OFF", nullptr),
			ccGLWindow::LOWER_LEFT_MESSAGE,
			false,
			2,
			PERSPECTIVE_STATE_MESSAGE);*/
    ...
    auto newcamerapos = m_viewportParams.getPivotPoint() + cameraCenterToPivot;
    bool ispers = m_viewportParams.perspectiveView;
    if (newcamerapos.z < 30 && !m_viewportParams.perspectiveView)
    {
        newcamerapos.z = 30;
    }
	setCameraPos(newcamerapos);
	ccGui::ParamStruct parameters = ccGui::Parameters();
	if (!m_viewportParams.perspectiveView && parameters.m_isImageMeshOnlyShow)
	{
		CCVector3d v(0, 0, 1);
		moveCamera(v);
	}
...
void ccGLWindow::setView(CCVector3d eyePos, CCVector3d upPos, bool forceRedraw/*=true*/)
{
	bool wasViewerBased = !m_viewportParams.objectCenteredView;
	if (wasViewerBased)
		setPerspectiveState(m_viewportParams.perspectiveView, true);

	m_viewportParams.viewMat = ccGLUtils::GenerateViewMatByEyePos(eyePos, upPos);
...
void ccGLWindow::setviewer_for_trans(ccGLMatrixd currentMat, ccGLMatrixd& previous_mat)
{
	previous_mat = m_viewportParams.viewMat;
	m_viewportParams.viewMat = currentMat;
}
...
void ccGLWindow::displayText(	QString text,
								...
								const ccColor::Rgba* backcolor/*=nullptr*/)
{
	ccQOpenGLFunctions* glFunc = functions();
	assert(glFunc);
	...
	if (align != ALIGN_DEFAULT || bkgAlpha != 0.0f)
	{
		...
		if (bkgAlpha != 0.0f)
		{
			...
			float invertedCol[4] = {	(255 - rgba[0]) / 255.0f,
											(255 - rgba[1]) / 255.0f,
											(255 - rgba[2]) / 255.0f,
											bkgAlpha };
	...
	renderText(x2 + 1, glHeight() - 1 - y2, text, 0, textFont, backcolor); // x2 + 1 --> empirical
}
...
bool ccGLWindow::enableStereoMode(const StereoParams& params)
{
	...
	if (params.glassType == StereoParams::OCULUS)
	{
		...
		if (!s_oculus.session)
		{
			...
			{
				CS::Widgets::FramelessMessageBox::critical(asWidget(), "Oculus", "Failed to initialize the Oculus SDK (ovr_Initialize)");
				return false;
			}
			...
			if (OVR_FAILURE(result))
			{
				CS::Widgets::FramelessMessageBox::critical(asWidget(), "Oculus", "Failed to initialize the Oculus SDK (ovr_Create)");
				ovr_Shutdown();
				return false;
			}
		...
		{
			CS::Widgets::FramelessMessageBox::critical(asWidget(), "Oculus", "Failed to initialize the swap texture set (ovr_CreateSwapTextureSetGL)");
			s_oculus.stop(true);
			return false;
		}
		...
		CS::Widgets::FramelessMessageBox::critical(asWidget(), "Oculus", "The Oculus device is not supported by this version\n(use the 'Stereo' version)");
		return false;
	...
	else if (params.glassType == StereoParams::NVIDIA_VISION || params.glassType == StereoParams::GENERIC_STEREO_DISPLAY)
	{
		...
		{
			CS::Widgets::FramelessMessageBox::critical(asWidget(), "Stereo", "Quad buffering not supported!");
			return false;
		}
		...
		if (m_initialized)
		{
			...
			{
				CS::Widgets::FramelessMessageBox::critical(asWidget(), "Stereo", "OpenGL stereo mode not supported/enabled!");
				return false;
			}
		}
...
void ccGLWindow::renderText(int x, int y, const QString & str, uint16_t uniqueID/*=0*/, const QFont & pFont/*=QFont()*/, const ccColor::Rgba* backcolor/*=QFont()*/)
{   
	if (m_activeFbo)
	{
		m_activeFbo->start();
...
GLfloat ccGLWindow::getGLDepth(int x, int y, bool extendToNeighbors/*=false*/, bool usePBO/*=false*/)
{
	...
	if (usePBO && m_pickingPBO.glBuffer)
	{
		...
		if (diff_ms < 100)
		{
			...
			{
				qDebug() <<"Failed to read the picking PBO contents. We won't use it anymore";
				if (m_pickingPBO.glBuffer != nullptr)
				{
	...
	if (usePBO && m_pickingPBO.glBuffer)
	{
		if (!bufferRestored)
		{
			...
			{
				qDebug() << "Failed to map the picking PBO contents. We won't use it anymore";
...
void ccGLWindow::setPickCursor(bool bEable)
{
    PICKING_MODE type = getPickingMode();
	ccGui::ParamStruct parameters = ccGui::Parameters();
	if (parameters.m_bGraphicsElement || parameters.m_bCustomMouseStyle) {
		return;
	}
    if (bEable)
    {
        if (NOTE_PICKING == type)
        {
            QPixmap pix(CS::Core::ICore::resourceThemeImagePath("notemouse.png"));
            QSize size(26, 26);
            pix = pix.scaled(size, Qt::KeepAspectRatio);
            this->setCursor(QCursor(pix, 0, pix.width()-1));
        }
		
		
        else if (POINT_PICKING_ENTITY == type || FAST_PICKING == type)
        {
            setCursor(QCursor(Qt::PointingHandCursor));
        }
    }
...
bool ccGLWindow::covertScreenCoord3DPos(const QPoint & point, CCVector3d & p3d)
{
    int x = point.x();
    int y = point.y();

    //[!].获取视图显示配置参数
    ccViewportParameters stViewParameters =  getViewportParameters();

    //[!].获取深度值， 如果深度值无效设置深度值，为最远平面位置
    y = glHeight() - 1 - y;
    GLfloat glDepth = getGLDepth(x, y, false, false);
    if (glDepth == INVALID_DEPTH){//[!].深度超出了设置为最远平面值
       //glDepth = stViewParameters.zFar;
        return false;
    }
...
bool ccGLWindow::covertScreenCoordNear3DPos(
    const QPoint & point, 
    CCVector3d & p3d)
{

    bool extendToNeighbors = true;
    bool usePBO = false;
    int x = point.x();
    int y = point.y();
    y = glHeight() - 1 - y;
    ...
    //[!].反投影计算
    if (minZ == INVALID_DEPTH) {
        return false;
    }

    //[!].计算深度值
    CCVector3d P2D(x, y, minZ);
    ccGLCameraParameters camera;
    getGLCameraParameters(camera);
    return camera.unproject(P2D, p3d);
}

void ccGLWindow::updateGLWindowsMousePos(const int &x, const int &y, QVector3D &pos)
{
	//[!].计算鼠标3D位置
	int fcx = x;
	int fcy = y;
	QVector3D mouse2d(x, y, 0);
	fcy = glHeight() - 1 - fcy;
	GLfloat glDepth = getGLDepth(fcx, fcy, false, false);
	CCVector3d P2D(fcx, fcy, glDepth);
	CCVector3d P3D;
	ccGLCameraParameters camera;
	getGLCameraParameters(camera);
	camera.unproject(P2D, P3D);
	QVector3D pos3d(P3D.x, P3D.y, P3D.z);
	QVector3D pos2d(P2D.x, P2D.y, glDepth);
	pos = pos3d;
	return;
}

void ccGLWindow::updateActiveGraphics(const QVector3D &pos)
{
	const ccGui::ParamStruct& stGraphicsParams = ccGui::Parameters();
	if (stGraphicsParams.m_bGraphicsElement) {
		ccHObject* pOwnAttacheEntity = CS::Model::ProjectModel::instance()->getAttachedGLOwnEntity();
		if (pOwnAttacheEntity) {
...
void ccGLWindow::setDrawBackgroundGridEnable(bool bEnable)
{
	m_bDrawBackgroundGridEnable = bEnable;
	return;
}
...
void ccGLWindow::setIsDrawTitle(bool isDraw)
{
	m_isDrawTitle = isDraw;
}


void ccGLWindow::deleteVertexArrays(unsigned int id)
{
    QOpenGLFunctions_3_3_Core* glFunc3 = (context() ? context()->versionFunctions<QOpenGLFunctions_3_3_Core>() : nullptr);
    if (glFunc3)
    {
        makeCurrent();
        glFunc3->glDeleteVertexArrays(1, &id);
        doneCurrent();
    }
}


void ccGLWindow::deleteBuffers(unsigned int id)
{
    QOpenGLFunctions_3_3_Core* glFunc3 = (context() ? context()->versionFunctions<QOpenGLFunctions_3_3_Core>() : nullptr);
    if (glFunc3)
    {
        makeCurrent();
        glFunc3->glDeleteBuffers(1, &id);
        doneCurrent();
    }
}