...
int SARibbonPannelPrivate::s_pannelTitleHeight = 26;
    ...
    : QWidget(parent), m_d(new SARibbonPannelPrivate(this))
{
	...
	QSize titleSize = fm.size(Qt::TextShowMnemonic, name);
	if (m_d->m_layout) {
		m_d->m_layout->setLeftSpace(titleSize.width()+12);
	}
	setPannelLayoutMode(ThreeRowMode);
	setPannelName(name);
	setObjectName(name);
...
SARibbonToolButton* SARibbonPannel::addAction(QAction* action, SARibbonPannelItem::RowProportion rp)
{
    ...
    setActionRowProportionProperty(action, rp);
    addAction(action);
	SARibbonToolButton * lastobj = m_d->lastAddActionButton();
	lastobj->setObjectName(objname);
    return (lastobj);
}
...
SARibbonToolButton* SARibbonPannel::addMenu(QMenu* menu, SARibbonPannelItem::RowProportion rp, QToolButton::ToolButtonPopupMode popMode)
{
	...
	QString tip = menu->toolTip();
    addAction(action, rp);
    SARibbonToolButton* btn = m_d->lastAddActionButton();
	btn->setToolTip(tip);
	btn->setStyleSheet("QToolButton::menu-indicator:image{subcontrol-position: right center;}");
...
void SARibbonPannel::paintEvent(QPaintEvent* event)
{
    ...
    if (ThreeRowMode == pannelLayoutMode()) {
        ...
        if (m_d->m_optionActionButton) {
            ...
            p.drawText(6, height() - th-3, width()-1, th-3, Qt::AlignCenter, windowTitle());
#ifdef SA_RIBBON_DEBUG_HELP_DRAW
...
QSize SARibbonPannel::sizeHint() const
{
    ...
    if (ThreeRowMode == pannelLayoutMode()) {
        ...
        maxWidth = qMax(maxWidth, titleSize.width()+12);
    }
...
void SARibbonPannel::setActionsEnabled(bool state, QString actionObjectName, QString ignoreObjectName)
{
	for (auto action : actions())
	{
		if (action->objectName() == ignoreObjectName)
			continue;

		if (actionObjectName == "" || action->objectName() == actionObjectName)
