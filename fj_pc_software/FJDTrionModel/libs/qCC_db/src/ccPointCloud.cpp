	...
	, m_fwfData(nullptr)
{
	...
	showSF(false);
	//addChild(dynamic_cast<ccHObject*>(&m_Surface));
}
	...
	if (n)
	{
		...
		if (hasColors())
		{
			if (result->reserveTheRGBTable())
			{
				for (unsigned i = 0; i < n; i++)
				{
					unsigned index = selection->getPointGlobalIndex(i);
					if (m_rgbaColors && index < m_rgbaColors->currentSize()) {
						result->addColor(getPointColor(index));
					}
...
{
	clear();
    FJStyleManager::Instance()->removePointCloudObject(getUniqueIDForDisplay());
}

void ccPointCloud::importParametersPointCloudFrom(ccPointCloud* cloud)
{
	if (!cloud) {
		return;
	}
    this->setPointSizeAdaptively(cloud->getPointSizeAdaptively());
	ccGenericPointCloud* ccGenCloud = dynamic_cast<ccGenericPointCloud*>(cloud);
	if (ccGenCloud) {
        this->importParametersFrom(ccGenCloud);
	}
}
	...
	if (hasColors() || addedCloud->hasColors())
	{
		...
		else //otherwise
		{
			...
			if (hasColors() && m_rgbaColors->currentSize() == pointCountBefore)
			{
				for (unsigned i = 0; i < addedPoints; i++)
				{
					if (i < addedCloud->m_rgbaColors->size()){
...
//aric.tang_2022.12.8_bug correct
static float               s_rgbBuffer4f[MAX_POINT_COUNT_PER_LOD_RENDER_PASS * 4];
...
void ccPointCloud::drawUserDb(CC_DRAW_CONTEXT & context)
{
    drawDataOld(context);
}

void ccPointCloud::getCornerPoint(std::vector<CCVector3> & points)
{
    CCVector3 bbMin = m_bbox.minCorner();
    CCVector3 bbMax = m_bbox.maxCorner();
    double dx = std::abs(bbMax.x - bbMin.x);
    double dy = std::abs(bbMax.y - bbMin.y);
    double dz = std::abs(bbMax.z - bbMin.z);
    points.push_back(bbMin);
    points.push_back(bbMin + CCVector3(dx,0,0));
    points.push_back(bbMin + CCVector3(0, dy, 0));
    points.push_back(bbMin + CCVector3(dx, dy, 0));
    points.push_back(bbMin + CCVector3(dx, 0, dz));
    points.push_back(bbMin + CCVector3(0, dy, dz));
    points.push_back(bbMin + CCVector3(0, 0, dz));
    points.push_back(bbMax);
}


void ccPointCloud::drawDataNew(CC_DRAW_CONTEXT& context)
{
            ...
            )
        {
            ...
            if (!toDisplay.indexMap)
            {
                ...
                if (context.minLODPointCount && toDisplay.count > context.minLODPointCount)
                {
                    ...
                    double miny = 9999999;
                    CCVector3d output2D;
                    for (auto curcorner : cornerpoints)
                    {
                        camera.project(curcorner, output2D);
                        maxx = std::max(maxx, output2D.x);
                        maxy = std::max(maxy, output2D.y);
                        minx = std::min(minx, output2D.x);
                        miny = std::min(miny, output2D.y);
                    }
                    if (std::abs(maxx - minx) > ((camera.viewport)[2] * 12.0) || std::abs(maxy - miny) > ((camera.viewport)[3] * 12.0))
                    {
                        isInLodRange = false;
                    }
                    if (!context.isShowLodState || !isInLodRange)
                    {
                        toDisplay.decimStep = 1;
                    }
                }
            }
        }
    }
    int associatedCloudSize = size();
    const ccGenericPointCloud::VisibilityTableType& verticesVisibility = getTheVisibilityArray();
    bool visFiltering = (verticesVisibility.size() >= associatedCloudSize);
    if (!m_vertexArrayManager.m_isInitVBO)
    { 
        m_vertexArrayManager.colors.clear();
        try
        {
            m_vertexArrayManager.colors.resize(associatedCloudSize * 4);
        }
        catch (const std::bad_alloc&)
        {
            if (pushName)
            ...
            setEnabled(false);
			qDebug() << "error! draw pointcloud is enough memory " << this->getName();
            FJStyleManager::Instance()->showErrorMessage("Not enough chash");
            return;
        }
        if (glParams.showSF)
        {
            m_vertexArrayManager.m_isShowSf = true;
            if (getCurrentCombinationMode() == ALTITUDEANDINTENSITY)
            {
#if defined(_OPENMP)
#pragma omp parallel for
#endif
                for (int j = 0; j < associatedCloudSize; j++)
                {
                    const ccColor::Rgb* sfrgb = m_currentDisplayedScalarField->getValueColorHigh(j);
                    m_vertexArrayManager.colors[j*4] = sfrgb->r / 255.0;
                    m_vertexArrayManager.colors[j * 4+1] = sfrgb->g / 255.0;
                    m_vertexArrayManager.colors[j * 4+2] = sfrgb->b / 255.0;
                    m_vertexArrayManager.colors[j * 4+3] = 1 + j % 200;
                }
            }
            else
            {
...
#pragma omp parallel for
                ...
                for (int j = 0; j < associatedCloudSize; j++)
                {
                    const ccColor::Rgb* sfrgb = m_currentDisplayedScalarField->getValueColor(j);
                    m_vertexArrayManager.colors[j * 4] = sfrgb->r / 255.0;
                    m_vertexArrayManager.colors[j * 4 + 1] = sfrgb->g / 255.0;
                    m_vertexArrayManager.colors[j * 4 + 2] = sfrgb->b / 255.0;
                    m_vertexArrayManager.colors[j * 4 + 3] = 1 + j % 200;
				...
				if (getCurrentCombinationMode() == CLASSIFICATION)
				{
#if defined(_OPENMP)
#pragma omp parallel for
#endif
					for (int j = 0; j < associatedCloudSize; j++)
					{
						...
						if (m_classficationNotShowData.find(int(sf)) != m_classficationNotShowData.end())
						{
							m_vertexArrayManager.colors[j * 4 + 3] = 0;
						}
					}
				}
            }
...
#pragma omp parallel for
                ...
                for (int j = 0; j < associatedCloudSize; j++)
                {
                    m_vertexArrayManager.colors[j * 4] = ((*m_rgbaColors)[j].r) / 255.0;
                    m_vertexArrayManager.colors[j * 4 + 1] = ((*m_rgbaColors)[j].g) / 255.0;
                    m_vertexArrayManager.colors[j * 4 + 2] = ((*m_rgbaColors)[j].b) / 255.0;
                    m_vertexArrayManager.colors[j * 4 + 3] = ((*m_rgbaColors)[j].a > 0 ? (1 + j % 200) : 0);
                }
            }
            else
            {
                const ccColor::Rgba& tempColor = getTempColor();
#if defined(_OPENMP)
#pragma omp parallel for
#endif
                for (int j = 0; j < associatedCloudSize; j++)
                {
                    m_vertexArrayManager.colors[j * 4] = tempColor.r / 255.0;
                    m_vertexArrayManager.colors[j * 4 + 1] = tempColor.g / 255.0;
                    m_vertexArrayManager.colors[j * 4 + 2] = tempColor.b / 255.0;
                    m_vertexArrayManager.colors[j * 4 + 3] = 1 + j % 200;
                }
            }

        }
		if (verticesVisibility.size() == associatedCloudSize)
		{
#if defined(_OPENMP)
#pragma omp parallel for
#endif
			for (int j = 0; j < associatedCloudSize; j++)
			{
				if (verticesVisibility[j] == CCCoreLib::POINT_VISIBLE)
				{
					m_vertexArrayManager.colors[j * 4 + 3] = 1 + j % 200;
				}
				else
        ...
        glFunc3->glGenVertexArrays(1, &m_vertexArrayManager.VAO);
        glFunc3->glGenBuffers(1, &m_vertexArrayManager.VBO);
        //glFunc3->glGenBuffers(1, &m_vertexArrayManager.EBO);
        glFunc3->glGenBuffers(1, &m_vertexArrayManager.CBO);


        glFunc3->glBindVertexArray(m_vertexArrayManager.VAO);


        glFunc3->glBindBuffer(GL_ARRAY_BUFFER, m_vertexArrayManager.CBO);
        glFunc3->glBufferData(GL_ARRAY_BUFFER, m_vertexArrayManager.colors.size() * sizeof(float), m_vertexArrayManager.colors.data(), GL_STATIC_DRAW);
        if (!glFunc3->glCheckFramebufferStatus(GL_FRAMEBUFFER) == GL_FRAMEBUFFER_COMPLETE) {
            if (pushName)
            {
                glFunc->glPopName();
            }
        ...
        // 关联颜色信息到第二个顶点属性
        glFunc3->glVertexAttribPointer(1, 4, GL_FLOAT, GL_FALSE, 4 * sizeof(float), (void*)0);
        glFunc3->glEnableVertexAttribArray(1);

        m_vertexArrayManager.colors.clear();
        m_vertexArrayManager.colors.shrink_to_fit();

        glFunc3->glBindBuffer(GL_ARRAY_BUFFER, m_vertexArrayManager.VBO);
        glFunc3->glBufferData(GL_ARRAY_BUFFER, associatedCloudSize * 3 * sizeof(float), m_points.data(), GL_STATIC_DRAW);
        if (!glFunc3->glCheckFramebufferStatus(GL_FRAMEBUFFER) == GL_FRAMEBUFFER_COMPLETE) {
            if (pushName)
            {
                glFunc->glPopName();
        ...
        bool is_vao_created = (glFunc3->glIsVertexArray(m_vertexArrayManager.VAO) == GL_TRUE);
        bool is_vbo_created = (glFunc3->glIsBuffer(m_vertexArrayManager.VBO) == GL_TRUE);
        //bool is_ebo_created = (glFunc3->glIsBuffer(m_vertexArrayManager.EBO) == GL_TRUE);
        bool is_cbo_created = (glFunc3->glIsBuffer(m_vertexArrayManager.CBO) == GL_TRUE);
        glFunc3->glBindVertexArray(0);
        m_vertexArrayManager.m_isInitVBO = true;
    }
    if (m_vertexArrayManager.m_isNeedUpdateVertices)
    {
        glFunc3->glBindBuffer(GL_ARRAY_BUFFER, m_vertexArrayManager.VBO);
        glFunc3->glBufferSubData(GL_ARRAY_BUFFER, 0, m_points.size() * 3 * sizeof(float), m_points.data());
        m_vertexArrayManager.m_isNeedUpdateVertices = false;
        //m_vertexArrayManager.vertices.clear();
        //m_vertexArrayManager.vertices.shrink_to_fit();
    }

  
    if (m_vertexArrayManager.m_isNeedUpdateColor || m_vertexArrayManager.m_isShowSf != glParams.showSF || m_vertexArrayManager.m_isNeedUpdateVisibilityArray)
    {
        m_vertexArrayManager.colors.clear();
        try
        {
...
#pragma omp parallel for
        ...
        glFunc3->glBindBuffer(GL_ARRAY_BUFFER, m_vertexArrayManager.CBO);
        glFunc3->glBufferSubData(GL_ARRAY_BUFFER, 0, m_vertexArrayManager.colors.size() * sizeof(float), m_vertexArrayManager.colors.data());
        m_vertexArrayManager.m_isNeedUpdateColor = false;
		m_vertexArrayManager.m_isNeedUpdateVisibilityArray = false;
        m_vertexArrayManager.m_isShowSf = glParams.showSF;
        m_vertexArrayManager.colors.clear();
        m_vertexArrayManager.colors.shrink_to_fit();
    }
    ...
    //[!]设置点大小
    m_cloudSizeAdaptively ? glFunc->glEnable(GL_PROGRAM_POINT_SIZE) : glFunc->glDisable(GL_PROGRAM_POINT_SIZE);
    glFunc->glPointSize(static_cast<float>(m_pointSize) ? m_pointSize + m_fBigAutoPointSize : viewParams.defaultPointSize + m_fBigAutoPointSize);
    //rounded points
	if (context.drawRoundedPoints) {
		glFunc->glPushAttrib(GL_POINT_BIT);
		glFunc->glEnable(GL_POINT_SMOOTH);

		//// 1.启用点平滑 2.启用点大小衰减, 3.设置点平滑的提示为最高图像质量
    ...
    for (int i = 0; i < maxLightCount; ++i)
    {
        if (glFunc->glIsEnabled(GL_LIGHT0 + i))
        {
            ...
            glFunc->glGetLightfv(GL_LIGHT0 + i, GL_POSITION, position);

            break;
        }
    }

    glFunc3->glUseProgram(context.pointcloudNormalShaderProgram);

	//[!].对比度亮度值
	float fBright = PublicUtils::getRGBColorBright();
	float fContrast = PublicUtils::getRGBColorContrast();
    ...
    int diffuseColorLoc = -1;
    int diffuseCoefficientLoc = -1;
    int modelviewLoc = -1;
    int showNormalLoc = -1;
    int useclipboxLoc = -1;
    int boxmincornerLoc = -1;
    int boxmaxcornerLoc = -1;
    int boxMatrixLoc = -1;
    int decimStepLoc = -1;
	int colorBrightloaction = -1;
	int colorContrastloaction = -1;
    int pointCloudAutonSize = -1;
   
   
    pointCloudAutonSize = glFunc3->glGetUniformLocation(context.pointcloudNormalShaderProgram, "adaptivePointSize");
    viewLoc = glFunc3->glGetUniformLocation(context.pointcloudNormalShaderProgram, "view");
    projectionLoc = glFunc3->glGetUniformLocation(context.pointcloudNormalShaderProgram, "projection");
    lightPositionLoc = glFunc3->glGetUniformLocation(context.pointcloudNormalShaderProgram, "lightPosition");
    ambientColorLoc = glFunc3->glGetUniformLocation(context.pointcloudNormalShaderProgram, "ambientColor");
    diffuseColorLoc = glFunc3->glGetUniformLocation(context.pointcloudNormalShaderProgram, "diffuseColor");
    diffuseCoefficientLoc = glFunc3->glGetUniformLocation(context.pointcloudNormalShaderProgram, "diffuseCoefficient");
    modelviewLoc = glFunc3->glGetUniformLocation(context.pointcloudNormalShaderProgram, "modelview");
    showNormalLoc = glFunc3->glGetUniformLocation(context.pointcloudNormalShaderProgram, "showNormal");
    useclipboxLoc = glFunc3->glGetUniformLocation(context.pointcloudNormalShaderProgram, "useclipbox");
    boxmincornerLoc = glFunc3->glGetUniformLocation(context.pointcloudNormalShaderProgram, "boxmincorner");
    boxmaxcornerLoc = glFunc3->glGetUniformLocation(context.pointcloudNormalShaderProgram, "boxmaxcorner");
    boxMatrixLoc = glFunc3->glGetUniformLocation(context.pointcloudNormalShaderProgram, "boxMatrix");
    decimStepLoc = glFunc3->glGetUniformLocation(context.pointcloudNormalShaderProgram, "decimStep");
	colorBrightloaction = glFunc3->glGetUniformLocation(context.pointcloudNormalShaderProgram, "colorBright");
	colorContrastloaction = glFunc3->glGetUniformLocation(context.pointcloudNormalShaderProgram, "colorContrast");
    ...
    glFunc3->glUniformMatrix4fv(modelviewLoc, 1, GL_FALSE, mvparams2);
    glFunc3->glUniformMatrix4fv(projectionLoc, 1, GL_FALSE, projectionparams2);
    //glFunc3->glUniform3f(lightPositionLoc, position[0], position[1], position[2]);
    glFunc3->glUniform3f(lightPositionLoc, 10000, 10000, 10000);
    //glFunc3->glUniform3f(ambientColorLoc, ambiant[0], ambiant[1], ambiant[2]);
    glFunc3->glUniform3f(ambientColorLoc, 0.6f, 0.6f, 0.6f);//环境光
    //glFunc3->glUniform3f(diffuseColorLoc, diffuse[0], diffuse[1], diffuse[2]);
    glFunc3->glUniform3f(diffuseColorLoc, 0.7f, 0.7f, 0.7f);//漫反射光
    glFunc3->glUniform1f(diffuseCoefficientLoc, 0.8f);
    glFunc3->glUniform1i(showNormalLoc, glParams.showNorms);
    glFunc3->glUniform1i(useclipboxLoc, !m_clipPlanes.empty());
    glFunc3->glUniform1i(decimStepLoc, toDisplay.decimStep);
	glFunc3->glUniform1f(colorBrightloaction, fBright);
	glFunc3->glUniform1f(colorContrastloaction, fContrast);
    glFunc3->glUniform1f(pointCloudAutonSize, viewParams.getAutoPointSize() + m_fBigAutoPointSize);

    if (!m_clipPlanes.empty())
    {
        ccHObject *ccbox = getClippingBox();
        if (ccbox)
        {
            ccClipBox* box = static_cast<ccClipBox*>(ccbox);
            if (box)
            {
                const ccBBox& boxcorner = box->getBox();
                glFunc3->glUniform3f(boxmincornerLoc, boxcorner.minCorner().x, boxcorner.minCorner().y, boxcorner.minCorner().z);
                glFunc3->glUniform3f(boxmaxcornerLoc, boxcorner.maxCorner().x, boxcorner.maxCorner().y, boxcorner.maxCorner().z);
                const ccGLMatrix& boxGLMatrix = box->getGLTransformation();
                ccGLMatrix transMat = boxGLMatrix.inverse();
                glFunc3->glUniformMatrix4fv(boxMatrixLoc, 1, GL_FALSE, transMat.data());
            }
        }
    }
    ...
    glFunc3->glBindVertexArray(m_vertexArrayManager.VAO);
    //glFunc3->glBindBuffer(GL_ARRAY_BUFFER, m_vertexArrayManager.EBO);
    //glFunc3->glVertexAttribPointer(3, 1, GL_UNSIGNED_INT, GL_FALSE, sizeof(unsigned int), 0);
    //glFunc3->glVertexAttribDivisor(3, 10);
    //glFunc3->glDrawArraysInstanced(GL_POINTS, 0, m_vertexArrayManager.vertices.size(), 1);
    //glFunc3->glDrawElementsInstanced(GL_POINTS, m_vertexArrayManager.indices.size(), GL_UNSIGNED_INT, 0, 1);
    glFunc3->glDrawArrays(GL_POINTS, 0,associatedCloudSize);
    //glFunc3->glDrawElements(GL_POINTS, m_vertexArrayManager.indices.size(), GL_UNSIGNED_INT, 0);
    delete[] mvparams;
    delete[] projectionparams;
    delete[] mvparams2;
    delete[] projectionparams2;
    int success;
    glFunc3->glGetProgramiv(context.pointcloudNormalShaderProgram, GL_LINK_STATUS, &success);
    if (!success)
    {
        int logLength;
        char message[1024];
        glFunc3->glGetProgramiv(context.pointcloudNormalShaderProgram, GL_INFO_LOG_LENGTH, &logLength);
        if (logLength > 0)
        {
            glFunc3->glGetProgramInfoLog(context.pointcloudNormalShaderProgram, logLength, NULL, message);
        }
    }
...
void ccPointCloud::drawDataOld(CC_DRAW_CONTEXT& context)
{
    if (m_points.empty())
        return;
    ...
    if (MACRO_Draw3D(context))
    {
        ...
        //[!]旧版绘制方式增加点大小自适应
        {
            ...
            {
                float size = viewParams.getAutoPointSize();
                glFunc->glPointSize(size + m_fBigAutoPointSize);
            }
            else {
                glFunc->glPointSize(static_cast<float>(m_pointSize) ? m_pointSize + m_fBigAutoPointSize : viewParams.defaultPointSize + m_fBigAutoPointSize);
            }
        ...
        //main display procedure
        {
            ...
            if (isVisibilityTableInstantiated())
            {
                ...
                CCVector3d globalshift = getGlobalShift();
                ...
                {
                    if (context.visibleEnable) {
                        if ((m_pointsVisibility[j] != CCCoreLib::POINT_VISIBLE)) {
                            continue;
                        }
                    }
                    ...
                    if (glParams.showSF)
                    {
                        ...
                        if (getCurrentCombinationMode() == ALTITUDEANDINTENSITY)
                        {
                            ScalarType _sf = m_currentDisplayedScalarField->getValue(j);
                            const ccColor::Rgb* color = m_currentDisplayedScalarField->getValueColor(j);
                            QColor unique_color;
                            unique_color.setRed(color->r);
                            unique_color.setGreen(color->g);
                            unique_color.setBlue(color->b);
                            unique_color.setAlpha(255);
                            int h_, s_, v_, a_;
                            int *h = &h_;
                            int *s = &s_;
                            int *v = &v_;
                            ...
                            float val_pre = _sf;
                            int val_int = (int)(val_pre * 1000000.0);
                            int vIndex;
                            if ((val_int % 1000) < 5 && (val_int % 1000) > -5)
                            {
                                vIndex = 0;
                            }
                            else
                            {
                                vIndex = abs(val_int % 1000);
                            }

                            ccColor::Rgb col1 = ccColor::Convert::hsv2rgb((float)h_, ((float)s_ / 255.0), ((float)vIndex / 255.0));
                            col = &col1;
                        }
                        else
                        {
                            col = m_currentDisplayedScalarField->getValueColor(j);
                        }
                    ...
                    {
                        ccColor::Rgbub c;
                        c.r = m_rgbaColors->getValue(j).r;
                        c.g = m_rgbaColors->getValue(j).g;
                        c.b = m_rgbaColors->getValue(j).b;
                        glFunc->glColor3ubv(c.rgb);
                    }
                    if (glParams.showNorms)
                    {
                        ccGL::Normal3v(glFunc, compressedNormals->getNormal(m_normals->getValue(j)).u);
                    }
                    //glFunc->glVertex3dv((m_points[j].toDouble() - globalshift).u);
                    ccGL::Vertex3v(glFunc, m_points[j].u);
                }
            ...
            else if (glParams.showSF) //no visibility table enabled + scalar field
            {
                ...
                else //potentially hidden points
                {
                    ...
                    else //potentially hidden points without normals (a bit faster)
                    {
                        if (colorRampShader)
                        {
                            std::set<int> notShowData = getCurrentNotShowClassficationSet();
                            if (!m_currentDisplayedScalarField->symmetricalScale())
                            {
                                for (unsigned j = toDisplay.startIndex; j < toDisplay.endIndex; j += toDisplay.decimStep)
                                {
                                    ...
                                    if (getCurrentCombinationMode() == CLASSIFICATION)
                                    {
                                        if (notShowData.find(int(sf)) == notShowData.end())
                                        {
                                            glFunc->glColor3f(GetNormalizedValue(sf, sfDisplayRange), 1.0f, 1.0f);
                                            ccGL::Vertex3v(glFunc, m_points[pointIndex].u);
                                        }
                        ...
                        else
                        {
                            for (unsigned j = toDisplay.startIndex; j < toDisplay.endIndex; j += toDisplay.decimStep)
                            {
                                ...
                                if (getCurrentCombinationMode() == ALTITUDEANDINTENSITY)
                                {
                                    col = m_currentDisplayedScalarField->getValueColorHigh(pointIndex);
                                }
                                else
                                {
                                    col = m_currentDisplayedScalarField->getValueColor(pointIndex);
                                }
            ...
            else //no visibility table enabled, no scalar field
            {
...

                {
                    ...
                    for (size_t k = 0; k < chunkCount; ++k)
                    {
                        ...
                        if (glParams.showColors) {
                            //[!].对比度亮度调整
                            ccColor::Rgba* pColorsTable = ccChunk::Start(*m_rgbaColors, chunkSize);
                            if (pColorsTable) {
                                ColorCompType* _sfColors = s_rgbBuffer4ub;
                                for (unsigned j = 0; j < chunkSize; j += toDisplay.decimStep, _sfColors += 4) {

									ccColor::Rgb rgb;
									rgb.r = (*m_rgbaColors)[j + index].r;
									rgb.g = (*m_rgbaColors)[j + index].g;
									rgb.b = (*m_rgbaColors)[j + index].b;
                                    //[!].曝光度
                                    //exeRGBExposure(rgb, 1.125);
                                    _sfColors[0] = rgb.r;
                                    _sfColors[1] = rgb.g;
                                    _sfColors[2] = rgb.b;
                                    _sfColors[3] = (*m_rgbaColors)[j + index].a;
                                }
                                glFunc->glColorPointer(4, GL_UNSIGNED_BYTE, 1 * 4 * sizeof(ColorCompType), s_rgbBuffer4ub);
                            }
                            index += chunkSize;
                        }
...
ccGenericPointCloud* ccPointCloud::createNewCloudFromVisibilitySelection(bool removeSelectedPoints/*=false*/, VisibilityTableType* visTable/*=nullptr*/, bool silent/*=false*/)
{
	...
	result->setCurrentCombinationMode(getCurrentCombinationMode());
						...
						//copy SF colors in static array
						{
                            ...
                            if (getCurrentCombinationMode() == ALTITUDEANDINTENSITY)
                            {
                                for (int j = 0; j < chunkSize; j++, _sf++)
                                {
                                    ...
                                    //aric.tang_2022.12.5
                                    const ccColor::Rgb* color = _sf ? m_vboManager.sourceSF->getColor(*_sf) : nullptr;
                                    QColor unique_color;
                                    unique_color.setRed(color->r);
                                    unique_color.setGreen(color->g);
                                    unique_color.setBlue(color->b);
...
void ccPointCloud::setImageSourse(const std::vector<pointImage> & imageSourse)
{
    m_imageSourse = imageSourse;
}

const std::vector<ccPointCloud::pointImage> & ccPointCloud::getImageSourse()
{
...
void ccPointCloud::adjustGlobalShift()
{
	if (m_points.size() > 0)
	{
		CCVector3d firstPoint = m_points[0].toDouble() - getGlobalShift();
		if (std::abs(firstPoint.x) < 10000 && std::abs(firstPoint.y) < 10000 && std::abs(firstPoint.z) < 10000)
		{
			addGlobalShift(getGlobalShift());
			setGlobalShift(CCVector3d(0, 0, 0));
		}
	}
}

void ccPointCloud::visibilityArrayChange()
{
    m_vertexArrayManager.m_isNeedUpdateVisibilityArray = true;
}

void ccPointCloud::scaleFieldColorChange()
{
    m_vertexArrayManager.m_isNeedUpdateColor = true;
}

void ccPointCloud::deleteVertexArrays()
{
    if (m_vertexArrayManager.m_isInitVBO)
    {
        FJStyleManager::Instance()->deleteVertexArrays(m_vertexArrayManager.VAO);
        FJStyleManager::Instance()->deleteBuffers(m_vertexArrayManager.VBO);
        //FJStyleManager::Instance()->deleteBuffers(m_vertexArrayManager.EBO);
        FJStyleManager::Instance()->deleteBuffers(m_vertexArrayManager.CBO);
        m_vertexArrayManager.m_isInitVBO = false;
    }
}

void ccPointCloud::cloudPositionChange()
{
    m_vertexArrayManager.m_isNeedUpdateVertices = true;
}
