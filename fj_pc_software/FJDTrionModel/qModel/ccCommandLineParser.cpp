...
int ccCommandLineParser::Parse(int nargs, char** args, ccPluginInterfaceList& plugins)
{
	...
	consoleDlg->setWindowFlags(Qt::Window | Qt::FramelessWindowHint);
	...
	if (!parser->silentMode())
	{
		if (result == EXIT_SUCCESS)
			CS::Widgets::FramelessMessageBox::information(consoleDlg.data(), "Processed finished", "Job done");
		else
			CS::Widgets::FramelessMessageBox::warning(consoleDlg.data(), "Processed finished", "An error occurred! Check console");
	}
