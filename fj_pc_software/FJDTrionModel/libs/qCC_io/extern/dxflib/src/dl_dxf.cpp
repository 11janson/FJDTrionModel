...
bool DL_Dxf::in(const std::string& file,
    ...
    if (fp) {
		...
		{
			//加载cad dxf时区分普通dxf和cad dxf
			if (!bDxfFilter && breakloop)
					break;
		}
...
bool DL_Dxf::readDxfGroups(FILE *fp, DL_CreationInterface* creationInterface) {
    ...
    if (DL_Dxf::getStrippedLine(groupCodeTmp, DL_DXF_MAXLINE, fp) &&
        ...
        if (fabs(atof(groupValue.c_str()) - 0.9713096022) < 0.01)
        {
            int nbreak = 0;
        }
		//cad dxf通过识别码加载切面类型
...
bool DL_Dxf::processDXFGroup(DL_CreationInterface* creationInterface,
	...
	//读取写入识别XRECORD数据，区分cad dxf和dxf
	bool hasXRecord = handleXRecordData(creationInterface);
...
{
	//通过handle区分多段线、三点矩形、对角矩形
	attrib = creationInterface->getAttributes();
	std::string name = attrib.getLayer(); 
	creationInterface->addXDataString(attrib.getHandle(), name);

    DL_PolylineData pd(/*maxVertices*/getIntValue(66, 1), getIntValue(71, 0), getIntValue(72, 0), getIntValue(70, 0), getRealValue(38, 0));
    ...
    //if (currentObjectType==DL_ENTITY_LWPOLYLINE) {
	//类型错误，导致折线解析错误
	if (currentObjectType == DL_ENTITY_POLYLINE) {
...
void DL_Dxf::addArc(DL_CreationInterface* creationInterface) {
	...
	std::string name = attrib.getLayer();
	creationInterface->addXDataString(attrib.getHandle(), name);
	creationInterface->addXDataReal(attrib.getHandle(), attrib.getLinetypeScale());
