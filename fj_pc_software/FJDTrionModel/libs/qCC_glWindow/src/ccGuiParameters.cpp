...
void ccGui::ParamStruct::reset()
{
	...
	displayedNumPrecision		= 3;
	labelOpacity				= 30;

	zoomSpeed					= 1.0;

	autoComputeOctree			= NEVER;
	...
	volumnunit                  = CUBICMETER;
	isshowUnitlabel             = false;
    m_bShowCapacityPoint        = false;
    m_isPaintPageActive         = false;
	int m_nBrightness			= 10;
	int m_nContrast				= 25;
    m_canChangePivotPoint = true;
}
...
void ccGui::ParamStruct::fromPersistentSettings()
{
	...
	drawRoundedPoints			=                                      settings.value("drawRoundedPoints_newBegin110",		false).toBool();
	...
	//labelMarkerSize				= static_cast<unsigned>(std::max(0,    settings.value("labelMarkerSize",         8    ).toInt()));
	...
	defaultFontSize				= static_cast<unsigned>(std::max(0,    settings.value("defaultFontSize",          16  ).toInt()));
	//labelFontSize				= static_cast<unsigned>(std::max(0,    settings.value("labelFontSize",            16  ).toInt()));
	displayedNumPrecision		= static_cast<unsigned>(std::max(0,    settings.value("displayedNumPrecision",    3   ).toInt()));
	//labelOpacity				= static_cast<unsigned>(std::max(0,    settings.value("labelOpacity",             25  ).toInt()));
	zoomSpeed					=                                      settings.value("zoomSpeed",                1.0 ).toDouble();
	//autoComputeOctree			= static_cast<ComputeOctreeForPicking>(settings.value("autoComputeOctree", NEVER).toInt());
	autoComputeOctree = NEVER;
	singleClickPicking			=									   settings.value("singleClickPicking",		 true ).toBool();
	lengthunit                  = static_cast<Lengthparametertype>(    settings.value("lengthunit", RICE).toInt());
	diameterunit                = static_cast<Lengthparametertype>(    settings.value("diameterunit", RICE).toInt());
	angleunit                   = static_cast<Angleparametertype>(     settings.value("angleunit", DEGREE).toInt());
	areaunit                    = static_cast<Areaparametertype>(      settings.value("areaunit", SQUAREMETER).toInt());
	volumnunit                  = static_cast<Volumnparametertype>(    settings.value("volumnunit", CUBICMETER).toInt());
	isshowUnitlabel             =                                      settings.value("isshowUnitlabel", false).toBool();
	m_globalpointsize			=									   settings.value("m_globalpointsize", 1).toInt();
	settings.endGroup();
}
...
void ccGui::ParamStruct::toPersistentSettings() const
{
	...
	settings.setValue("lengthunit",               static_cast<int>(lengthunit));
	settings.setValue("diameterunit",             static_cast<int>(diameterunit));
	settings.setValue("angleunit",                static_cast<int>(angleunit));
	settings.setValue("areaunit",                 static_cast<int>(areaunit));
	settings.setValue("volumnunit",               static_cast<int>(volumnunit));
	settings.setValue("isshowUnitlabel",          isshowUnitlabel);
    settings.setValue("m_globalpointsize",			m_globalpointsize);
	settings.endGroup();
}
