																	...
																	GenericProgressCallback* progressCb/*=nullptr*/)
{
	...
	DataCloud data;
	{
		...
		else if (inputDataCloud->size() > 1000000 && inputModelCloud->size() > 1000000)
		{
            ...
            data.cloud = CloudSamplingTools::resampleCloudSpatially(inputDataCloud, 0.2, modParams);
			if (!data.cloud)
			{
	...
	else /*if (inputModelCloud)*/
	{
		...
		else if (inputModelCloud->size() > 1000000 && inputDataCloud->size() > 1000000)
		{
			//we resample the cloud if it's too big (speed increase)
			//ReferenceCloud* subModelCloud = CloudSamplingTools::subsampleCloudRandomly(inputModelCloud, params.samplingLimit);
            CCCoreLib::CloudSamplingTools::SFModulationParams modParams(false);
            ReferenceCloud* subModelCloud = CloudSamplingTools::resampleCloudSpatially(inputModelCloud, 0.2, modParams);
			if (!subModelCloud)
			{
				//not enough memory
				return ICP_ERROR_NOT_ENOUGH_MEMORY;
		...
		//(now that we have selected the points that will be used for registration!)
		{
			...
			if (iteration == 0)
			{
				...
				if (progressCb)
				{
					...
					{
                        progressCb->setMethodTitle(nullptr);
						//progressCb->setInfo(QCoreApplication::translate("MainWindowUI", "Registration", nullptr));
						char buffer[256];
			...
			else
			{
				...
				if (progressCb)
				{
					...
					else
					{
                        ...
                        if (lastStepRMSRecod > deltaRMS)
                        {
                            progressCb->update(progressPercent);
                            lastStepRMSRecod = deltaRMS;
                        }
					}
				}
			}
...
{
	return RegistrationProcedureNew(lCloud, rCloud, trans);
}

bool HornRegistrationTools::FindPlaneAbsoluteOrientation(GenericCloud* lCloud,
    GenericCloud* rCloud,
    ScaledTransformation& trans,
    bool fixedScale/*=false*/)
{
    return PlaneCoordinateTrans(lCloud, rCloud, trans);
}
...
bool RegistrationTools::RegistrationProcedureNew(GenericCloud* P,
    GenericCloud* X,
    ScaledTransformation& trans)
{
    trans.R.invalidate();
    ...
    trans.s = 1.0;
    //取目标点云坐标系坐标
    Eigen::MatrixXd oldXYZ(P->size(), 3);
    P->placeIteratorAtBeginning();
    for (int i = 0; i < P->size(); ++i)
    {
        const CCVector3* pp = P->getNextPoint();
        oldXYZ(i, 0) = pp->x;
        oldXYZ(i, 1) = pp->y;
        oldXYZ(i, 2) = pp->z;
    }
    //取源点云坐标系坐标
    Eigen::MatrixXd newXYZ(P->size(), 3);
    X->placeIteratorAtBeginning();
    for (int i = 0; i < P->size(); ++i)
    {
        const CCVector3* xx = X->getNextPoint();
        newXYZ(i, 0) = xx->x;
        newXYZ(i, 1) = xx->y;
        newXYZ(i, 2) = xx->z;
    }
    int PN = oldXYZ.rows();
    Eigen::MatrixXd TT;//平移
    TT.resize(3, 1);
    TT.setZero();
    Eigen::Matrix3d RR;//旋转
    double k = 1.0;//缩放
    ...
    //迭代，选择平移参数迭代改正数最大值小于1e - 8），跳出循环
    //定义迭代次数为10000，当迭代最大值或者满足精度时，跳出循环，防止迭代发散
    ...
    while (1)
    {
        ...
        RR.setZero();
        Eigen::Matrix3d EPlusS = E + S;
        Eigen::Matrix3d EMinusSinv = (E - S).inverse();
        RR = EPlusS * EMinusSinv;
        //构造B矩阵(3oldXYZ.size() ，7)
        //构造L矩阵（3oldXYZ.size()，1)
        //SE = npmulti(S, -1) + E
        Eigen::Matrix3d SE;
        SE.setIdentity();
        SE = -1 * S + E;
        //B1 = np.zeros((3 * m, 3))
        Eigen::MatrixXd Bone(3 * PN, 3);
        Bone.setZero();
        //B1[0:m, :] = SE[0, :]
        for (int i = 0; i < PN; ++i)
        {
            for (int j = 0; j < Bone.cols(); ++j)
            {
                Bone(i, j) = SE(0, j);
            }
        }
        ...
        for (int i = 0; i < PN; ++i)
        {
            for (int j = 0; j < Bone.cols(); ++j)
            {
                Bone(i + 2 * PN, j) = SE(2, j);
            }
        }
        //B2 = np.zeros((3 * m, 4))
        Eigen::MatrixXd Btwo(3 * PN, 4);
        Btwo.setZero();
        //L = np.zeros((3 * m, 1))
        Eigen::MatrixXd  L(3 * PN, 1);
        L.setZero();
        //for i in range(0, m) :
        for (int i = 0; i < PN; ++i)
        {
            Btwo(i, 1) = -k * oldXYZ(i, 2) - newXYZ(i, 2) + TT(2, 0);
            Btwo(i, 2) = -k * oldXYZ(i, 1) - newXYZ(i, 1) + TT(1, 0);
            Btwo(i, 3) = oldXYZ(i, 0) - c * oldXYZ(i, 1) - b * oldXYZ(i, 2);
            Btwo(PN + i, 0) = -k * oldXYZ(i, 2) - newXYZ(i, 2) + TT(2, 0);
            Btwo(PN + i, 2) = k * oldXYZ(i, 0) + newXYZ(i, 0) - TT(0, 0);
            Btwo(PN + i, 3) = c * oldXYZ(i, 0) + oldXYZ(i, 1) - a * oldXYZ(i, 2);
            Btwo(2 * PN + i, 0) = k * oldXYZ(i, 1) + newXYZ(i, 1) - TT(1, 0);
            Btwo(2 * PN + i, 1) = k * oldXYZ(i, 0) + newXYZ(i, 0) - TT(0, 0);
            Btwo(2 * PN + i, 3) = b * oldXYZ(i, 0) + a * oldXYZ(i, 1) + oldXYZ(i, 2);

            L(i, 0) = -(k * oldXYZ(i, 0) - k * c * oldXYZ(i, 1) - k * b * oldXYZ(i, 2) - newXYZ(i, 0) + TT(0, 0) - c * newXYZ(i, 1) + c * TT(1, 0) - b * newXYZ(i, 2) + b * TT(2, 0));
            L(PN + i, 0) = -(k * c * oldXYZ(i, 0) + k * oldXYZ(i, 1) - k * a * oldXYZ(i, 2) + c * newXYZ(i, 0) - c * TT(0, 0) - newXYZ(i, 1) + TT(1, 0) - a * newXYZ(i, 2) + a * TT(2, 0));
            L(2 * PN + i, 0) = -(k * b * oldXYZ(i, 0) + k * a * oldXYZ(i, 1) + k * oldXYZ(i, 2) + b * newXYZ(i, 0) - b * TT(0, 0) + a * newXYZ(i, 1) - a * TT(1, 0) - newXYZ(i, 2) + TT(2, 0));
        }

        //矩阵拼接
        //B = np.c_[B1, B2]
        Eigen::MatrixXd B(Bone.rows(), Bone.cols() + Btwo.cols());
        B << Bone, Btwo;

        //计算法方程x = (Bnn - 1)*W
        //Nbb = (B.T * P * B)
        Eigen::MatrixXd Nbb(7, 7);
        Nbb.setZero();
        Nbb = B.transpose() * PW * B;

        Eigen::MatrixXd W(7, 1);
        W.setZero();
        W = B.transpose() * PW * L;

        Eigen::MatrixXd XX(7, 1);
        XX.setZero();
        XX = Nbb.inverse() * W;
        ...
        //待求量的改正量足够小或者迭代次数足够多，跳出迭代。
        Eigen::MatrixXd V = B * XX - L;
        //计算单位权中误差
        double alpha0 = 0;
        alpha0 = sqrt((V.transpose() * PW * V)(0, 0) / (3 * PN - 7));
        if ((XX.array().abs()).maxCoeff() < 0.0000000001 || iterationCounter >= 10000)
        {
            //if enter the wrong coordinates, judge the illegal number
            for (int i = 0; i < 3; ++i)
            {
                for (int j = 0; j < 3; ++j)
                {
                    //if enter the wrong coordinates, judge the illegal number
                    if (isnan(RR(i, j)) || isinf(RR(i, j)) || isnan(TT(i, 0)) || isinf(TT(i, 0)))
                    {
                        return false;
                    }
                }
            ...
            if (k < 0)
            {
                RegistrationProcedure(P, X, trans, 1);
            }
            else
            {
                SquareMatrix temR(3);
                for (int i = 0; i < 3; ++i)
                {
                    for (int j = 0; j < 3; ++j)
                    {
                        temR.m_values[i][j] = RR(i, j);
                    }
                }
                trans.R = temR;
                trans.T = CCVector3d(TT(0, 0), TT(1, 0), TT(2, 0));
                trans.s = k;
            }
            break;
        }
...
bool RegistrationTools::PlaneCoordinateTrans(GenericCloud* P,
    GenericCloud* X,
    ScaledTransformation& transMat)
{
    ...
    int number = 0;//计算次数
    const int parNum = 4;
    const int formulNum = 2;
    int lineNum = X->size();
    std::vector<double> fourPar(4);
    std::vector<double> RMS(lineNum);
    //系数阵
    Eigen::MatrixXd bmatrix(lineNum*formulNum, parNum);
    bmatrix.setZero();

    Eigen::VectorXd lmatrix(lineNum*formulNum);
    lmatrix.setZero();

    Eigen::MatrixXd pmatrix(lineNum*formulNum, lineNum*formulNum);
    pmatrix.setIdentity();

    std::vector<double> paraTrans{ 0,0,0,0 };
    double sumX = 0;
    double sumY = 0;

    //目标点云
    Eigen::MatrixXd newm(lineNum, 2);
    X->placeIteratorAtBeginning();
    for (int i = 0; i < lineNum; ++i)
    {
        const CCVector3* pp = X->getNextPoint();
        newm(i, 0) = pp->x;
        newm(i, 1) = pp->y;
    }

    //源点云
    Eigen::MatrixXd orim(lineNum, 2);
    P->placeIteratorAtBeginning();
    for (int i = 0; i < lineNum; ++i)
    {
        const CCVector3* xx = P->getNextPoint();
        orim(i, 0) = xx->x;
        orim(i, 1) = xx->y;
    }
    ...
    for (int i = 0; i < lineNum; i++)
    {
        sumX += newm(i, 0) - orim(i, 0);
        sumY += newm(i, 1) - orim(i, 1);
    }
    double rawPar[4] = { sumX / lineNum,sumY / lineNum,0,0 };//x y a k

    //迭代计算
    while (iscompute)
    {
        int countNum = 0;
        double cosp3 = cos(rawPar[3]);
        double sinp3 = sin(rawPar[3]);

        for (int i = 0; i < lineNum; i++)
        {
            ////先旋转再平移
            bmatrix(countNum, 0) = cosp3 * orim(i, 0) - sinp3 * orim(i, 1);
            bmatrix(countNum, 1) = (1.0 + rawPar[2])*(-sinp3 * orim(i, 0) - cosp3 * orim(i, 1));
            bmatrix(countNum, 2) = 1;
            bmatrix(countNum, 3) = 0;

            lmatrix(countNum) = -((1.0 + rawPar[2])* (cosp3 * orim(i, 0) - sinp3 * orim(i, 1)) + rawPar[0] - newm(i, 0));
            countNum++;

            bmatrix(countNum, 0) = sinp3 * orim(i, 0) + cosp3 * orim(i, 1);
            bmatrix(countNum, 1) = (1.0 + rawPar[2])*(cosp3 * orim(i, 0) - sinp3 * orim(i, 1));
            bmatrix(countNum, 2) = 0;
            bmatrix(countNum, 3) = 1;

            lmatrix(countNum) = -((1.0 + rawPar[2]) * (sinp3 * orim(i, 0) + cosp3 * orim(i, 1)) + rawPar[1] - newm(i, 1));
            countNum++;

        }

        //计算结果
        Eigen::MatrixXd btran = bmatrix.transpose();
        Eigen::VectorXd ResultOffSetMatrix = (btran*pmatrix*bmatrix).inverse()*btran*pmatrix*lmatrix;
        rawPar[0] = rawPar[0] + ResultOffSetMatrix(2);// k a x y
        rawPar[1] = rawPar[1] + ResultOffSetMatrix(3);
        rawPar[2] = rawPar[2] + ResultOffSetMatrix(0);
        rawPar[3] = rawPar[3] + ResultOffSetMatrix(1);
        number++;
        if ((fabs(ResultOffSetMatrix(0)) < 0.0001) && (fabs(ResultOffSetMatrix(1)) < 0.0001))
        {
            iscompute = false;
            //残差计算
            fourPar[0] = rawPar[0];// DX:
            fourPar[1] = rawPar[1];// DY
            fourPar[2] = rawPar[3];// T
            fourPar[3] = 1.0 + rawPar[2];// K
            cosp3 = cos(rawPar[3]);
            sinp3 = sin(rawPar[3]);
            paraTrans[0] = fourPar[0];
            paraTrans[1] = fourPar[1];
            paraTrans[2] = fourPar[2];
            paraTrans[3] = abs(fourPar[3]);
            double averageRMS = 0.0;// 用平均值代替第一个rms值 janson
            for (int i = 0; i < lineNum; i++)
            {
                //先旋转后平移
                double x = (abs(fourPar[3]) * cosp3) * orim(i, 0) - (abs(fourPar[3]) * sinp3) * orim(i, 1) + fourPar[0];
                double y = (abs(fourPar[3]) * sinp3) * orim(i, 0) + (abs(fourPar[3]) * cosp3) * orim(i, 1) + fourPar[1];
                RMS[i] = sqrt((x - newm(i, 0))*(x - newm(i, 0)) + (y - newm(i, 1))*(y - newm(i, 1)));
                averageRMS += abs(RMS[i]);
            }
            averageRMS /= lineNum;
            if (averageRMS > 1.0)  //调整角度方向，防止倒转Z轴   //阈值由0.5改为1.0 janson
            {
                sinp3 = -sin(rawPar[3]);
                cosp3 = -cos(rawPar[3]);
                for (int i = 0; i < lineNum; i++)
                ...
                paraTrans[2] = asin(sinp3);
            }
            //判断旋转角度（0，2π），顺时针；若需要可归算到（-π，π）
            if (asin(sinp3) >= 0)
            {
                if (cosp3 < 0 && sinp3 > 0)
                {
                    double rotateAngle = 3.1415926 - asin(sinp3);
                    paraTrans[2] = rotateAngle;
                }
            }
            else
            {
                ...
                {
                    double rotateAngle = 3.1415926 * 2 + asin(sinp3);
                    paraTrans[2] = rotateAngle;
                }
            }
            //旋转矩阵 3*3；现改为4*4，需要可放开，注意顺序，原始为单位阵。
            trans(0, 0) = cos(paraTrans[2]);
            trans(0, 1) = -sin(paraTrans[2]);
            trans(1, 0) = sin(paraTrans[2]);
            trans(1, 1) = cos(paraTrans[2]);
            //trans(0, 2) = paraTrans[0];    //平移X
            //trans(1,2) = paraTrans[1];   //平移Y
            //trans(2, 2) = abs(paraTrans[3]);
            ...
            for (int i = 0; i < 3; ++i)
            {
                ...
                {
                    //if enter the wrong coordinates, judge the illegal number
                    if (isnan(trans(i, j)) || isinf(trans(i, j)) || isnan(paraTrans[0]) || isinf(paraTrans[0]))
                    {
                        return false;
                    }
                    else
                    {
                        temR.m_values[i][j] = trans(i, j);
                    }
                }
            }
            ...
            //四维转换矩阵
            transMat.R = temR;
            transMat.T = CCVector3d(paraTrans[0], paraTrans[1], 0);
            transMat.s = abs(paraTrans[3]);
...
CCVector3d RegistrationTools::PointCloudTrans(const CCVector3d oriPoint,
    Eigen::Matrix3d  trans)
{
    CCVector3d temPoint(0, 0, 0);
    temPoint.x = trans(2, 2) * trans(0, 0) * oriPoint.x + trans(2, 2) * trans(0, 1) * oriPoint.y + trans(0, 2);
    temPoint.y = trans(2, 2) * trans(1, 0) * oriPoint.x + trans(2, 2) * trans(1, 1) * oriPoint.y + trans(1, 2);
    temPoint.z = oriPoint.z;
    return temPoint;
}
