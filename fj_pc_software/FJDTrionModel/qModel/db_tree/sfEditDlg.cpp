...
{
    qDebug() << "sfEditDlginitstart";
	m_ui->setupUi(this);
    qDebug() << "sfEditDlginitstart2";
	//histogram window
	{
    ...
    m_ui->minValSpinBox->setDecimals(8);
    m_ui->maxValSpinBox->setDecimals(8);
    m_ui->minSatSpinBox->setDecimals(8);
    m_ui->maxSatSpinBox->setDecimals(8);
    m_ui->nanInGreyCheckBox->setChecked(true);
...
void sfEditDlg::fillDialogWith(ccScalarField* sf)
{
	...
	//options (checkboxes)
	{
		...
		if (logScale)
			...
			m_ui->satLabel->setText(QApplication::translate("SFEditDlg","saturation",nullptr));
	}
