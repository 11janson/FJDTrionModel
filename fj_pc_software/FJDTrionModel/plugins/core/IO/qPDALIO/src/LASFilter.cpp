...
//pdal
//#include <pdal/io/private/las/Utils.hpp>//2022.3.28yjj
...
//using namespace pdal::las;//2022.3.28yjj
...
{
	//! Default constructor
	//更新构造函数，提供修改LasField类型的接口
	ExtraLasField(QString name, Id id, LAS_FIELDS fieldType = LAS_EXTRA, double defaultVal = 0.0, double min = 0.0, double max = -1.0)
		: LasField(fieldType, defaultVal, min, max)
...
CC_FILE_ERROR LASFilter::saveToFile(ccHObject* entity, const QString& filename, const SaveParameters& parameters)
{
    ...
    {
        CS::Widgets::FramelessMessageBox::critical(nullptr, QObject::tr("Error"), QObject::tr("Save failed.Please ensure that the file name does not contain the # symbol."));
	...
	if (theCloud->isA(CC_TYPES::POINT_CLOUD))
	{
		...
		for (unsigned i = 0; i < pc->getNumberOfScalarFields(); ++i)
		{
			...
			{
				//支持平面切割节点二次打开，加载额外字段，直接进行绘制
				if (QString(sf->getName()).compare("is2dSurface") == 0)
				{
					ExtraLasField::Shared extraField(new ExtraLasField(QString(sf->getName()), Id::Unknown, IS2DSURFACE));
					extraFields.emplace_back(extraField);
					extraFields.back()->sf = sf;
				}
				else if (QString(sf->getName()).compare("AXIS_X") == 0)
				{
					ExtraLasField::Shared extraField(new ExtraLasField(QString(sf->getName()), Id::Unknown, AXIS_X));
					extraFields.emplace_back(extraField);
					extraFields.back()->sf = sf;
				}
				else if (QString(sf->getName()).compare("AXIS_Y") == 0)
				{
					ExtraLasField::Shared extraField(new ExtraLasField(QString(sf->getName()), Id::Unknown, AXIS_Y));
					extraFields.emplace_back(extraField);
					extraFields.back()->sf = sf;
				}
				else if (QString(sf->getName()).compare("AXIS_Z") == 0)
				{
					ExtraLasField::Shared extraField(new ExtraLasField(QString(sf->getName()), Id::Unknown, AXIS_Z));
					extraFields.emplace_back(extraField);
					extraFields.back()->sf = sf;
				}
				else
	...
	if (parameters.parentWidget)
	{
		...
		QString m_str = QCoreApplication::translate("LASFilter", "Saving LAS file...", nullptr);
		if (filename.endsWith("laz", Qt::CaseInsensitive))
		{
			m_str = QCoreApplication::translate("LASFilter", "Saving LAZ file...", nullptr);
		}
		std::string str = m_str.toStdString();
		const char* ch = str.c_str();
		pDlg->setMethodTitle(ch);
	...
	try
	{
		...
		for (const ExtraLasField::Shared extraField : extraFields)
		{
			...
			if (QString(dimName.c_str()).compare("is2dSurface") != 0 && QString(dimName.c_str()).compare("AXIS_X") != 0
				&& QString(dimName.c_str()).compare("AXIS_Y") != 0 && QString(dimName.c_str()).compare("AXIS_Z") != 0 && QString(dimName.c_str()).compare("HasColor") != 0)
			{
				continue;
			}
			// All extra scalar fields are written as double.
...
struct LasCloudChunk
{
	...
	void createFieldsToLoad(IdList extraFieldsToLoad, StringList extraNamesToLoad,const std::vector<bool> & param)
	{
		if (param.size() < 5)
		{
			return;
...
std::vector<ExtraDim> ExtraBytesIf::toExtraDims()
{
	...
	if (m_type == Dimension::Type::None)
	{
		...
		//ExtraDim ed(m_name, Dimension::Type::None, m_scale[0]);//2022.3.28yjj
		ed.m_size = m_size;
		eds.push_back(ed);
	}
...
CC_FILE_ERROR LASFilter::loadFile(const QString& filename, ccHObject& container, LoadParameters& parameters)
{
	try
	{
		...
		bool hasis2dSurface = false;
		bool hasAXIS_X = false;
		bool hasAXIS_Y = false;
		bool hasAXIS_Z = false;
		for (const auto & cursfname : extraDims)
		{
			if (cursfname.m_name == "is2dSurface")
			{
				hasis2dSurface = true;
			}
			if (cursfname.m_name == "AXIS_X")
			{
				hasAXIS_X = true;
			}
			if (cursfname.m_name == "AXIS_Y")
			{
				hasAXIS_Y = true;
			}
			if (cursfname.m_name == "AXIS_Z")
			{
				hasAXIS_Z = true;
			}
		}
		...
		bool isloadredcolor = false;
		bool isloadgreencolor = false;
		bool isloadbluecolor = false;
		bool isloadgpstime = false;
		bool isloadintensity = false;
		bool isloadclassfication = false;
		bool isloadcloudsourseid = false;
		bool isuserdata = false;
		for (auto curdimdata : file_info.m_dimNames)
		{
			if (QString(curdimdata.c_str()).toUpper() == QString(LAS_FIELD_NAMES[LAS_RED]).toUpper())
			{
				isloadredcolor = true;
			}
			if (QString(curdimdata.c_str()).toUpper() == QString(LAS_FIELD_NAMES[LAS_GREEN]).toUpper())
			{
				isloadgreencolor = true;
			}
			if (QString(curdimdata.c_str()).toUpper() == QString(LAS_FIELD_NAMES[LAS_BLUE]).toUpper())
			{
				isloadbluecolor = true;
			}
			if (QString(curdimdata.c_str()).toUpper() == QString(LAS_FIELD_NAMES[LAS_TIME]).toUpper())
			{
				isloadgpstime = true;
			}
			if (QString(curdimdata.c_str()).toUpper() == QString(LAS_FIELD_NAMES[LAS_INTENSITY]).toUpper())
			{
				isloadintensity = true;
			}
			if (QString(curdimdata.c_str()).toUpper() == QString(LAS_FIELD_NAMES[LAS_CLASSIFICATION]).toUpper())
			{
				isloadclassfication = true;
			}
			if (QString(curdimdata.c_str()).toUpper() == QString(LAS_FIELD_NAMES[LAS_POINT_SOURCE_ID]).toUpper())
			{
				isloadcloudsourseid = true;
			}
			if (QString(curdimdata.c_str()).toUpper() == QString(LAS_FIELD_NAMES[LAS_USER_DATA]).toUpper())
			{
				isuserdata = true;
			}
		}
		std::vector<bool> paramdata;
		paramdata.push_back(isloadclassfication);
		paramdata.push_back(isloadintensity);
		paramdata.push_back(isloadgpstime);
		paramdata.push_back(isuserdata);
		paramdata.push_back(isloadcloudsourseid);
		...
		{
			if (extraDims[i].m_name != "is2dSurface" && extraDims[i].m_name != "AXIS_X" && extraDims[i].m_name != "AXIS_Y" && extraDims[i].m_name != "AXIS_Z")
			{
		...
		if (parameters.parentWidget)
		{
			...
			QString methonname = QObject::tr("Opening LAS file");
			if (filename.endsWith("laz", Qt::CaseInsensitive))
			{
				methonname = QObject::tr("Opening LAZ file");
			}
			npDlg->setAutoClose(false);
			npDlg->setAutoReset(false);
			npDlg->setMethodTitle(methonname);
			npDlg->setInfo(QObject::tr("Points: %L1").arg(nbOfPoints));
			npDlg->start();
		}

		CCCoreLib::NormalizedProgress nProgress(npDlg, nbOfPoints+1);
		...
		{
			if (npDlg && npDlg->isCancelRequested())
			{
				callbackError = CC_FERR_CANCELED_BY_USER;
			...
			if (pointChunk.getLoadedCloud() == nullptr)
			{
				...
				pointChunk.createFieldsToLoad(extraDimensionsIds, extraNamesToLoad, paramdata);
			}
		...
		for (auto &chunk : chunks)
		{
			...
			if (hasis2dSurface)
			{
				chunk.loadedCloud->setMetaData("is2dSurface", true);
				chunk.loadedCloud->addScalarField("is2dSurface");
			}
			CCVector3d lastAxisvalue(0, 0, 0);
			if (hasAXIS_X)
			{
				lastAxisvalue = CCVector3d(1, 0, 0);
				chunk.loadedCloud->addScalarField("AXIS_X");
			}
			else if (hasAXIS_Y)
			{
				lastAxisvalue = CCVector3d(0, 1, 0);
				chunk.loadedCloud->addScalarField("AXIS_Y");
			}
			else if (hasAXIS_Z)
			{
				lastAxisvalue = CCVector3d(0, 0, 1);
				chunk.loadedCloud->addScalarField("AXIS_Z");
			}
			QVector3D axis(lastAxisvalue.x, lastAxisvalue.y, lastAxisvalue.z);
			chunk.loadedCloud->setMetaData("axistype", QVariant(axis));
