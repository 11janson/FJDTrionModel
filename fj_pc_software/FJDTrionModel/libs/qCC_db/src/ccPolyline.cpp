...
bool ccPolyline::initWith(ccPointCloud*& vertices, const ccPolyline& poly)
{
	...
	if (vertices)
	{
		...
		//addChild(vertices);//janson.yang 2022.5.17 ²»ÏÔÊ¾¶¥µã
...
void ccPolyline::drawMeOnly(CC_DRAW_CONTEXT& context)
{
    ...
    if (m_isShowContourName)
    {
        QFont titleFont(context.display->getLabelDisplayFont());
        ccColor::Rgba bgkcolor = context.labelDefaultBkgCol;
        ...
        CCVector3d P2D;
        camera.project(m_ContourPosition, P2D);
        context.display->displayText(QString::number(m_ContourName, 'f', context.dispNumberPrecision),
            static_cast<int>(P2D.x) + context.labelMarkerTextShift_pix,
            static_cast<int>(P2D.y) + context.labelMarkerTextShift_pix,
...
double ccPolyline::getLineLenght()
{
    double lenght = 0;
    unsigned vertCount = size();
    for (unsigned i = 0; i < vertCount-1; ++i)
    {
        CCVector3 firstpoint;
        CCVector3 secondpoint;
        getPoint(i, firstpoint);
        getPoint(i+1, secondpoint);
        lenght += (firstpoint - secondpoint).normd();
    }
    return lenght;
}
...
bool ccPolyline::toFile_MeOnly(QFile& out) const
{
    ...
    outStream << m_ContourName;
    outStream << m_isShowContourName;
    outStream << m_ContourPosition.x;
    outStream << m_ContourPosition.y;
    outStream << m_ContourPosition.z;
...
bool ccPolyline::fromFile_MeOnly(QFile& in, short dataVersion, int flags, LoadedIDMap& oldToNewIDMap)
{
	...
	*(uint32_t*)(&m_theAssociatedCloud) = vertUniqueID;
    ...
    inStream >> m_ContourName;
    inStream >> m_isShowContourName;
    inStream >> m_ContourPosition.x;
    inStream >> m_ContourPosition.y;
    inStream >> m_ContourPosition.z;
	return true;
}
...
void ccPolyline::set3DPointData(std::vector<CCVector3d> pointData)
{
	m_3DPointData = pointData;
}

std::vector<CCVector3d> ccPolyline::get3DPointData()
{
	return m_3DPointData;
}

void ccPolyline::setPointVec(unsigned Index, const CCVector3d & data)
{
	ccPointCloud* cloud = dynamic_cast<ccPointCloud*>(m_theAssociatedCloud);
	if (cloud)
	{
		cloud->setPointVec(Index, CCVector3(data.x, data.y, data.z));
	}
}

void ccPolyline::setContourNameParam(double name, CCVector3 namepos, CCVector3 nameheadpos)
{
    m_ContourName = name;
    m_ContourPosition = namepos;
    m_ContourHeadPosition = nameheadpos;
    m_isShowContourName = true;
}