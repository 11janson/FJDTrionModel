...
CSUTILS_EXPORT QString winErrorMessage(unsigned long error);

// Determine a DLL version
enum WinDLLVersionType { WinDLLFileVersion, WinDLLProductVersion };
CSUTILS_EXPORT QString winGetDLLVersion(WinDLLVersionType t,
                                                const QString &name,
                                                QString *errorMessage);

CSUTILS_EXPORT bool is64BitWindowsSystem();

// Check for a 64bit binary.
CSUTILS_EXPORT bool is64BitWindowsBinary(const QString &binary);
...
//
class CSUTILS_EXPORT WindowsCrashDialogBlocker {
public:
