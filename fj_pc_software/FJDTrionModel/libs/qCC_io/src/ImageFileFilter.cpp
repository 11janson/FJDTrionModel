...
QString ImageFileFilter::GetSaveFilename(const QString& dialogTitle, const QString& baseName, const QString& imageSavePath, QWidget* parentWidget/*=nullptr*/)
{
	...
	for (int i = 0; i < formats.size(); ++i)
	{
        ...
        if (ext.compare("TIFF") != 0 && ext.compare("TIF") != 0)
        {
            continue;
        }
        ...
        //find TIFF by default
        if (tiffFilter.isEmpty() && ext == "TIFF")
        {
            tiffFilter = filter;
        }
	}
    ...
    QString outputFilename = CS::Widgets::FramelessFileDialog::getSaveFileName(parentWidget,
        dialogTitle,
        imageSavePath, fileFilters.join(s_fileFilterSeparator), &selectedFilter);


    QString strSuffix = QFileInfo(outputFilename).suffix();
    int index = outputFilename.lastIndexOf(".");
    if (index != -1)
    {
        outputFilename = outputFilename.remove(index, strSuffix.length() + 1);
    }
    ...
    if (list.size() > 1)
    {
        ...
        outputFilename += QString("%1").arg(suffixstr);
    }
...
QString ImageFileFilter::GetLoadFilename(const QString& dialogTitle, const QString& imageLoadPath, QWidget* parentWidget/*=nullptr*/)
{
	...
	return CS::Widgets::FramelessFileDialog::getOpenFileName(	parentWidget,
											dialogTitle,
											imageLoadPath,
											imageFilter);
}
