...
namespace CCCoreLib
{
	...
	public:
        ...
        std::vector<CCVector3>& getBindPointCould(void)
        {
            return m_points;
        }
		...
		**/
		...
		void addGlobalShift(CCVector3d shift)
		{
			for (auto & curpoint : m_points)
			{
				curpoint.x = curpoint.x - shift.x;
				curpoint.y = curpoint.y - shift.y;
				curpoint.z = curpoint.z - shift.z;
			}
		}

		void setPointVec(unsigned Index, const CCVector3 &P)
		{
			//NaN coordinates check
		...
		**/
		...
		void setScalarFieldName(int index, char* name)
		{
			if (index >= 0 && index < static_cast<int>(m_scalarFields.size()))
			{
				m_scalarFields[index]->setName(name);
			}
		}
		...
		**/
		int getScalarFieldIndexByName(const char* name) const
		{
			...
			for (std::size_t i = 0; i < sfCount; ++i)
			{
                ...
                if (stricmp(m_scalarFields[i]->getName(), name) == 0) // janson 2022.10.25 不区分大小写
