...
void ccRasterizeTool::generateRaster() const
{
	...
	while (true)
	{
		...
		{
			if (CS::Widgets::FramelessMessageBox::warning(	nullptr,
										"Mixed raster",
	...
	QString outputFilename;
	{
		...
		outputFilename = CS::Widgets::FramelessFileDialog::getSaveFileName(	const_cast<ccRasterizeTool*>(this),
                                    ...
                                    std::string rotate_matrix_inv_Str/* = ""*/)
{
    ...
    //添加自定义元数据
    poDstDS->SetMetadataItem("transformMatrixCustom", rotate_matrix_inv_Str.c_str());
	...
	if (exportBands.allSFs || (exportBands.visibleSF && visibleSfIndex >= 0))
	{
		for (size_t k = 0; k < grid.scalarFields.size(); ++k)
		{
			...
			if (exportBands.allSFs || (exportBands.visibleSF && visibleSfIndex == static_cast<int>(k)))
			{
				...
				assert(poBand);
				poBand->SetColorInterpretation(GCI_Undefined);
				poBand->SetMetadataItem("sfName", grid.scalarFieledsName.at(k).toStdString().c_str());
...
void ccRasterizeTool::exportTiffImage()
{
    if (!m_cloud)
    {
        assert(false);
    ...
    m_UI->dimensionComboBox->setCurrentIndex(2);
    ...
    bool exportRGB = false;//(m_UI->activeLayerComboBox->currentData().toInt() == LAYER_RGB);
    ...
    if (!outputImage.isNull())
    {
        ...
        //open file saving dialog
        {
            ...
            //QString outputFilename = CS::Widgets::FramelessFileDialog::getSaveFileName(this, "Save raster as image",
            //    imageSavePath,
            //    "*.tiff *.tif");
            ...
            QString outputFilename = ImageFileFilter::GetSaveFilename("Save raster as image",
                "image",
                imageSavePath,
                MainWindow::TheInstance()/*const_cast<ccRasterizeTool*>(this)*/);

            if (!outputFilename.isNull())
            {
                qDebug() << "path = " << QFileInfo(outputFilename).absoluteFilePath() << "\n";
                settings.setValue("savePathImage", QFileInfo(outputFilename).absoluteFilePath());
                settings.endGroup();

                if (outputImage.save(QFileInfo(outputFilename).absoluteFilePath()))
                {
...
void ccRasterizeTool::generateASCIIMatrix() const
{
	...
	QString outputFilename = CS::Widgets::FramelessFileDialog::getSaveFileName(nullptr, "Save grid as ASCII file", asciiGridSavePath + QString("/raster_matrix.txt"), filter);
	if (outputFilename.isNull())
		return;
