...
public: //base members access
	...
	**/
	...
	//储存设置的单色
	void setUniqueColor(QColor uniqueColor)
	{
		m_uniqueColor = uniqueColor;
	}

	//获取储存设置的单色
	QColor getUniqueColor()
	{
		return m_uniqueColor;
	}

	//获取当前组合渲染方式
	CurrentCombinationMode getCurrentCombinationMode()
	{
		return m_CurrentCombinationMode;
	}

	//设置当前组合渲染方式
	void setCurrentCombinationMode(CurrentCombinationMode mode)
	{
		m_CurrentCombinationMode = mode;
	}

	//获取类别不显示的id
	std::set<int> getCurrentNotShowClassficationSet()
	{
		return m_classficationNotShowData;
	}

	//设置类别不显示的id
	void setCurrentNotShowClassficationSet(std::set<int> data)
	{
		m_classficationNotShowData.clear();
		m_classficationNotShowData = data;
	}

	//设置类别组合显示时另一个标量范围
	void setClassficationCombineSingleSFRange(double min,double max)
	{
		m_sfMin = min;
		m_sfMax = max;
	}
	...
	//获取裁剪框
	ccHObject * getClippingBox() { return m_ClippingBox; }

	//设置裁剪框
	void setClippingBox(ccHObject * box) { m_ClippingBox = box; }
...
protected:
	...
	**/
	...
	QColor m_uniqueColor = Qt::white;

	CurrentCombinationMode m_CurrentCombinationMode = RGBCOLOR;

	std::set<int> m_classficationNotShowData;

	double m_sfMin = 0;
	double m_sfMax = 1.0;

	ccHObject * m_ClippingBox = nullptr;
