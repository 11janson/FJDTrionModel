shared {
    DEFINES += CSUTILS_LIBRARY
} else {
    DEFINES += CSUTILS_STATIC_LIB
}

!win32:{
    isEmpty(IDE_LIBEXEC_PATH) | isEmpty(IDE_BIN_PATH): {
        warning("using utils_lib.pri without IDE_LIBEXEC_PATH or IDE_BIN_PATH results in empty CHISHINE_REL_TOOLS_PATH")
        DEFINES += CHISHINE_REL_TOOLS_PATH=$$shell_quote(\"\")
    } else {
        DEFINES += CHISHINE_REL_TOOLS_PATH=$$shell_quote(\"$$relative_path($$IDE_LIBEXEC_PATH, $$IDE_BIN_PATH)\")
    }
}
