	...
	for (CLCloudDesc& cloudDesc : cmd.clouds())
	{
		...
		if (gridWidth * gridHeight > (1 << 26)) //64 million of cells
		{
			...
			else
			{
				...
				if (s_firstTime && CS::Widgets::FramelessMessageBox::warning(cmd.widgetParent(), "Raster grid", "Grid size is huge. Are you sure you want to proceed?\n(you can avoid this message by running in SILENT mode)", QMessageBox::Yes, QMessageBox::No) == QMessageBox::No)
				{
...
bool CommandVolume25D::process(ccCommandLineInterface &cmd)
{
	...
	if (gridWidth * gridHeight > (1 << 26)) //64 million of cells
	{
		...
		{
			static bool s_firstTime = true;
			if (s_firstTime && CS::Widgets::FramelessMessageBox::warning(cmd.widgetParent(), "Volume grid", "Grid size is huge. Are you sure you want to proceed?\n(you can avoid this message by running in SILENT mode)", QMessageBox::Yes, QMessageBox::No) == QMessageBox::No)
			{
				return ccLog::Warning("Process cancelled");
			}
			s_firstTime = false;
