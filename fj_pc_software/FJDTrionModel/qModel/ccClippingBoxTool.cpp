...
namespace
{
    ...
    bool staticMeshInorOutCorp = true;
}
...
{
	setupUi(this);
    thickXLabel->setText(QCoreApplication::translate("ClippingBoxDlg", "X Axis", nullptr));
    thickYLabel->setText(QCoreApplication::translate("ClippingBoxDlg", "Y Axis", nullptr));
    thickZLabel->setText(QCoreApplication::translate("ClippingBoxDlg", "Z Axis", nullptr));
    thickXDoubleSpinBox->setSingleStep(0.005);
    thickYDoubleSpinBox->setSingleStep(0.005);
    thickZDoubleSpinBox->setSingleStep(0.005);
    thickXDoubleSpinBox->setDecimals(3);
    thickYDoubleSpinBox->setDecimals(3);
    thickZDoubleSpinBox->setDecimals(3);
    thickXDoubleSpinBox->setMinimum(0.1);
    thickYDoubleSpinBox->setMinimum(0.1);
    thickZDoubleSpinBox->setMinimum(0.1);
	...
	connect(thickXDoubleSpinBox,			qOverload<double>(&QDoubleSpinBox::valueChanged),	this, &ccClippingBoxTool::thicknessChanged, Qt::UniqueConnection);
	connect(thickYDoubleSpinBox,			qOverload<double>(&QDoubleSpinBox::valueChanged),	this, &ccClippingBoxTool::thicknessChanged, Qt::UniqueConnection);
	connect(thickZDoubleSpinBox,			qOverload<double>(&QDoubleSpinBox::valueChanged),	this, &ccClippingBoxTool::thicknessChanged, Qt::UniqueConnection);
    ...
    connect(thicknesstoolbutton, &QToolButton::clicked, this, &ccClippingBoxTool::slotThicknesstoolbutton);

//    thicknesstoolbutton
    //setUIstyleSheet(showBoxToolButton, "box");
    //setUIstyleSheet(showInteractorsToolButton, "clippingmove");
    //setUIstyleSheet(minusXShiftToolButton, "X-");
    //setUIstyleSheet(plusXShiftToolButton, "X+");
    //setUIstyleSheet(minusYShiftToolButton, "Y-");
    //setUIstyleSheet(plusYShiftToolButton, "Y+");
    //setUIstyleSheet(minusZShiftToolButton, "Z-");
    //setUIstyleSheet(plusZShiftToolButton, "Z+");
    //setUIstyleSheet(resetButton, "reset");
    //setUIstyleSheet(sectioninToolButton, "clippingin");
    //setUIstyleSheet(sectionoutToolButton, "clippingout");
    //setUIstyleSheet(createButton, "clippingcreate");
    //setUIstyleSheet(closeButton, "clippingClose");
    

    //[!]新增盒内外按钮
    connect(sectioninToolButton, &QToolButton::clicked, this, &ccClippingBoxTool::slotSectionIn);
    connect(sectionoutToolButton, &QToolButton::clicked, this, &ccClippingBoxTool::slotSectionOut);
    connect(createButton, &QToolButton::clicked, this, &ccClippingBoxTool::slotCreate);
    sectioninToolButton->setEnabled(false);
    sectionoutToolButton->setEnabled(false);

    viewButtonsFrame_2->hide();
	s_maxEnvelopeEdgeLength = -1.0;
	//s_lastContourUniqueIDs.clear();
	removeLastContourToolButton->setEnabled(false);
    this->setFixedHeight(48);
    setUIstyleSheet(thicknesstoolbutton, "displaycappingspinboxon");
}
...
void ccClippingBoxTool::addClippingBoxToPointCloud()
{
	for (auto hobj : m_selectedEntities)
	{
		hobj->setClippingBox(m_clipBox);
	}
}


void ccClippingBoxTool::clearClippingBoxFromPointCloud()
{
	for (auto hobj : m_selectedEntities)
	{
		hobj->setClippingBox(nullptr);
	}
}
...
bool ccClippingBoxTool::linkWith(ccGLWindow* win)
{
	...
	if (win)
	{
		if (!m_clipBox)
		{
            ...
            connect(m_clipBox, &ccClipBox::siganlSetResetToolButtonEnabel, this, &ccClippingBoxTool::slotSetResetToolButtonEnabel);
		}
		m_associatedWin->addToOwnDB(m_clipBox);
	}
    connect(MainWindow::TheInstance()->getActiveGLWindow(), &ccGLWindow::activeSelectWindow, this, &ccClippingBoxTool::slotSetCloseDoubleBox, Qt::UniqueConnection);

    slotSetResetToolButtonEnabel(false);
...
bool ccClippingBoxTool::start()
{
    ...
    parameters.m_CuttingBoxisOpen = true;
	...
	m_associatedWin->redraw();
	addClippingBoxToPointCloud();
	return ccOverlayDialog::start();
}

void ccClippingBoxTool::stop(bool state)
{
	clearClippingBoxFromPointCloud();
	if (m_clipBox)
	{
    ...
    ccGui::ParamStruct parameters = ccGui::Parameters();
    parameters.m_CuttingBoxisOpen = false;
    ccGui::Set(parameters);
	if (m_associatedWin)
	{
...
ccHObject* GetSlice(ccHObject* obj, ccClipBox* clipBox, bool silent)
{
	...
	else if (obj->isKindOf(CC_TYPES::MESH))
	{
        ...
        bool type = staticMeshInorOutCorp;
		ccHObject* mesh = ccCropTool::Crop(obj, cropBox, type, _transformation);
		if (!mesh)
		{
			if (!silent)
        ...
        //[!]删除mesh子节点
        for (int i = 0; i < mesh->getChildrenNumber(); i++)
        {
            ccHObject *meshson = mesh->getChild(i);
            mesh->removeDependencyWith(meshson);
            mesh->removeChild(meshson);
        }
		return mesh;
	}
...
//内部预览
void ccClippingBoxTool::slotSectionIn()
{
    if (m_clipBox)
    {
        disPlayPointName = true;
        m_clipBox->m_bDisPlayInorOutpoint = true;
    }
    ...
    for (unsigned ci = 0; ci != m_clipBox->getContainer().getChildrenNumber(); ++ci)
    {
        ...
        staticMeshInorOutCorp = true;
        ccHObject * tempHobject = GetSlice(obj, m_clipBox, false);
        if (tempHobject == nullptr)
        {
            continue;
        }
        m_pContainerPointObject.insert(std::pair(obj->getUniqueID(), tempHobject));
        MainWindow::GetGLWindow("3D View")->addToOwnDB(tempHobject);
    }
    if (m_pContainerPointObject.empty())
    {
        CS::Widgets::FramelessMessageBox::critical(MainWindow::TheInstance(),
            QApplication::translate("ccClippingBoxTool", "Error", nullptr),
            QApplication::translate("ccClippingBoxTool", "Clipping failed. No points inside the box.", nullptr));
        return;
    }

    if (m_associatedWin)
        m_associatedWin->redraw();
    setSectionButtonStatus(false);
    if (!m_bCurrentCombination)
    {
        createButton->setEnabled(true);
    }
    showInteractorsToolButton->setChecked(false);
    showBoxToolButton->setChecked(false);
	emit updateSegmentState(false);
}

//外部预览
void ccClippingBoxTool::slotSectionOut()
{
    if (m_clipBox)
    {
        disPlayPointName = false;
        m_clipBox->m_bDisPlayInorOutpoint = false;
    }
    ...
    for (unsigned ci = 0; ci != m_clipBox->getContainer().getChildrenNumber(); ++ci)
    {
        ...
        m_pContainerPointObject.insert(std::pair(obj->getUniqueID(), tempHobject));
        MainWindow::GetGLWindow("3D View")->addToOwnDB(tempHobject);
        m_selectedEntities.at(ci)->setVisible(false);
    }
    m_clipBox->m_bDisPlayInorOutpoint = true;
    if (m_pContainerPointObject.empty())
    {
        CS::Widgets::FramelessMessageBox::critical(MainWindow::TheInstance(),
            QApplication::translate("ccClippingBoxTool", "Error", nullptr),
            QApplication::translate("ccClippingBoxTool", "Clipping failed. No points outside the box.", nullptr));
        return;
    }
    if (m_associatedWin)
        m_associatedWin->redraw();
...
//保存
void ccClippingBoxTool::slotCreate()
{
    if (!m_clipBox || !MainWindow::TheInstance())
        return;
    ...
    for (unsigned ci = 0; ci != m_clipBox->getContainer().getChildrenNumber(); ++ci)
    {
        ...
        //[!]遍历当前选中父节点下所有子节点的名称，计算section_的个数
        for (int i = 0; i < obj->getParent()->getChildrenNumber(); i++)
        {
             strInName = parentName + QString(".section_in%1").arg(nameInNumber);
             strOutName = parentName + QString(".section_out%1").arg(nameOutNumber);
            for (int i = 0; i < obj->getParent()->getChildrenNumber(); i++)
            {
                if (obj->getParent()->getChild(i)->getName().contains(strInName))
                {
                    nameInNumber++;
                    break;
                }
                else if (obj->getParent()->getChild(i)->getName().contains(strOutName))
                {
                    nameOutNumber++;
                    break;
                }
        ...
        auto findIter = m_pContainerPointObject.find(obj->getUniqueID());
        if (findIter == m_pContainerPointObject.end())
        {
            continue;
        }
        ccHObject * clapObj = findIter->second;
        if (clapObj)
        {
            //[!]清除GL预览，将点云加载进树
            MainWindow::GetGLWindow("3D View")->removeFromOwnDB(ccHObjectCaster::ToPointCloud(clapObj));
            //[!]设置名称
            if (disPlayPointName) {
                clapObj->setName(parentName + QString(".section_in") + QString::number(nameInNumber));
            }
            else {
                clapObj->setName(parentName + QString(".section_out")+ QString::number(nameOutNumber));
            }
            clapObj->setDisplay(obj->getDisplay());
            clapObj->prepareDisplayForRefresh();
            if (obj->getParent())
                obj->getParent()->addChild(clapObj);
            MainWindow::TheInstance()->addToDB(clapObj);
            m_selectedEntities.at(ci)->setEnabled(false);
            MainWindow::TheInstance()->setDBTreeSelectObject(clapObj);
            if (clapObj->isKindOf(CC_TYPES::MESH) && clapObj->getParent())
            {
                clapObj->getParent()->setEnabled(false);
            }
            m_selectedEntities.at(ci)->setVisible(true);
        }
    }

    m_pContainerPointObject.clear();
    setSectionButtonStatus(true);
    showInteractorsToolButton->setChecked(true);
    showBoxToolButton->setChecked(true);
    stop(true);
    MainWindow::TheInstance()->setGlobalZoom();
    MainWindow::GetGLWindow("3D View")->redraw();
}
void ccClippingBoxTool::closeDialog()
{
    ...
    for (auto ContainerPoint : m_pContainerPointObject)
    {
        MainWindow::GetGLWindow("3D View")->removeFromOwnDB(ContainerPoint.second);
    }
    m_pContainerPointObject.clear();
    stop(true);
}
void ccClippingBoxTool::reset()
{
    ...
    {
        addAssociatedEntity(m_selectedEntities.at(i));
    }
    m_pContainerPointObject.clear();

    if (m_associatedWin)
    {
        m_associatedWin->zoomGlobal(true);
        m_associatedWin->redraw();
    }
    ...
    slotSetResetToolButtonEnabel(false);
    createButton->setEnabled(false);
    m_bPointCloudStateChanges = false;
    m_bCloseCombinationCropButton = false;
	emit updateSegmentState(true);
...
void ccClippingBoxTool::extractSlicesAndContours(bool singleSliceMode)
{
	...
	if (sliceCount != 0)
	{
		CS::Widgets::FramelessMessageBox::warning(nullptr, tr("Process finished"), tr("%1 slices have been generated.\n(you may have to close the tool and hide the initial cloud to see them...)").arg(sliceCount));
	}
	else if (s_extractSliceCloudsOrMeshes)
	{
		CS::Widgets::FramelessMessageBox::warning(nullptr, tr("Process finished"), tr("The process has generated no output"));
	}
}
...
void ccClippingBoxTool::thicknessChanged(double)
{
    ...
    m_bPointCloudStateChanges = true;
    m_bCloseCombinationCropButton = true;
    if (!m_bCurrentCombination)
    {
        sectionoutToolButton->setEnabled(true);
        sectioninToolButton->setEnabled(true);
    }
    resetButton->setEnabled(true);
}
...
void ccClippingBoxTool::setUIstyleSheet(QToolButton *button,QString Pix)
{
    button->setFixedSize(29, 28);
    QIcon pIcon(FJStyleManager::Instance()->Getcurrentthemepath() + "qssimage/" + Pix + "@2x.png");
    pIcon.addPixmap(QPixmap(FJStyleManager::Instance()->Getcurrentthemepath() + "qssimage/" + Pix + "@2x-clicked.png"), QIcon::Active, QIcon::Off);
    pIcon.addPixmap(QPixmap(FJStyleManager::Instance()->Getcurrentthemepath() + "qssimage/" + Pix + "@2x-active.png"), QIcon::Disabled, QIcon::Off);
    button->setIconSize(QSize(29, 28));
    button->setIcon(pIcon);
}

void ccClippingBoxTool::slotSetResetToolButtonEnabel(bool status)
{
    m_bPointCloudStateChanges = true;
    m_bCloseCombinationCropButton = true;
    resetButton->setEnabled(status);
    m_bPointCloudStateChanges = status;
    m_bCloseCombinationCropButton = status;
    if (!m_bCurrentCombination)
    {
        sectioninToolButton->setEnabled(status);
        sectionoutToolButton->setEnabled(status);
    }
}
void ccClippingBoxTool::setSectionButtonStatus(bool status)
{
    showBoxToolButton->setEnabled(status);
    showInteractorsToolButton->setEnabled(status);
    minusXShiftToolButton->setEnabled(status);
    plusXShiftToolButton->setEnabled(status);
    minusYShiftToolButton->setEnabled(status);
    plusYShiftToolButton->setEnabled(status);
    minusZShiftToolButton->setEnabled(status);
    plusZShiftToolButton->setEnabled(status);
    m_bPointCloudStateChanges = status;
    m_bCloseCombinationCropButton = status;
    thicknesstoolbutton->setEnabled(status);
    if (this->height() == 98)
    {
        this->setFixedHeight(48);
    }
...
void ccClippingBoxTool::setCurrentButtonStatus(bool status)
{
    m_bCurrentCombination = status;
    if (m_bPointCloudStateChanges)
    {
        if (!m_bCloseCombinationCropButton)
        {
            sectioninToolButton->setEnabled(status);
            sectionoutToolButton->setEnabled(status);
        }
...
void ccClippingBoxTool::setCombineOpen()
{
	sectioninToolButton->setEnabled(false);
	sectionoutToolButton->setEnabled(false);
}

void ccClippingBoxTool::slotThicknesstoolbutton()
{
    if (height()== 98)
    {
        this->setFixedHeight(48);
    ...
    else
    {
        ...
        setUIstyleSheet(thicknesstoolbutton, "displaycappingspinbox");
    }
}


bool ccClippingBoxTool::refreshData(ccHObject::Container ccHobjcets)
{
    if (m_selectedEntities.size() > 0)
    {
        m_selectedEntities.clear();
    }
    if (m_pContainerPointObject.size() > 0)
    {
        m_pContainerPointObject.clear();

    }

    for (auto obj : ccHobjcets)
    {
        obj->m_displayBoundingBox = false;
        addAssociatedEntity(obj);
    }
...
void ccClippingBoxTool::slotSetCloseDoubleBox()
{
    if (height() == 98)
    {
        this->setFixedHeight(48);
