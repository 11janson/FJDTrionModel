...
{
    std::string  str = filename.toLocal8Bit().data();
	if (!root || str.empty())
	...
	//	pDlg->setMethodTitle(QApplication::translate("BinFilter","Save document",nullptr));
	//	pDlg->setInfo(QApplication::translate("BinFilter", "Please wait... saving in progress", nullptr));
	//	pDlg->setRange(0, 0);
	//	pDlg->setModal(true);
	//	pDlg->start();
	//}
...
CC_FILE_ERROR BinFilter::SaveFileV2(QFile& out, ccHObject* object)
{
	...
	while (!toCheck.empty())
	{
        ...
        else if (currentObject->isA(CC_TYPES::NOTE_2D))
        {
            ccNote* label = static_cast<ccNote*>(currentObject);
            const cc2DLabel::PickedPoint& pp = label->getPickedPoint();
            if (pp._cloud)
                dependencies.insert(pp._cloud);
            else if (pp._mesh)
                dependencies.insert(pp._mesh);
...
CC_FILE_ERROR BinFilter::loadFile(const QString& filename, ccHObject& container, LoadParameters& parameters)
{
	...
	else
	{
		...
		//{
		//	CS::Widgets::FramelessMessageBox::information(nullptr, QString("Wrong version"), QString("This file has been generated with the standard 'float' version!\nAt this time it cannot be read with the 'double' version."),QMessageBox::Ok);
		//	return CC_FERR_WRONG_FILE_TYPE;
		//}
		//else if (sizeof(PointCoordinateType) == 4 && strncmp((char*)&firstBytes,"CCB2",4) != 0)
		//{
		//	CS::Widgets::FramelessMessageBox::information(nullptr, QString("Wrong version"), QString("This file has been generated with the new 'double' version!\nAt this time it cannot be read with the standard 'float' version."),QMessageBox::Ok);
		//	return CC_FERR_WRONG_FILE_TYPE;
		//}
		...
		if (parameters.alwaysDisplayLoadDialog)
		{
			...
			if (parameters.parentWidget)
			{
				pDlg.reset(new ccProgressDialog(false, parameters.parentWidget));
				pDlg->setWindowFlags(Qt::Window | Qt::FramelessWindowHint);
				pDlg->setMethodTitle(QApplication::translate("BinFilter","Opening BIN file",nullptr));
				pDlg->setInfo(QApplication::translate("BinFilter", "Loading: %1", nullptr).arg(QFileInfo(filename).fileName()));
				pDlg->setRange(0, 100);
				pDlg->show();
				pDlg->setValue(5);
			}
...
CC_FILE_ERROR BinFilter::LoadFileV2(QFile& in, ccHObject& container, int flags)
{
	...
	if (!root->fromFile(in, static_cast<short>(binVersion), flags, oldToNewIDMap))
	{
		...
		qDebug()<<(QString("Failed to read file (file position: %1 / %2").arg(in.pos()).arg(in.size()));
		return CC_FERR_CONSOLE_ERROR;
	}
	...
	while (!toCheck.empty())
	{
        ...
        {
            ccNote* label = ccHObjectCaster::To2DNote(currentObject);
            cc2DLabel::PickedPoint pickPoint;
            pickPoint = label->getPickedPoint();

            bool invalidLabel = false;
            if (pickPoint._cloud)
            {
                intptr_t cloudID = (intptr_t)pickPoint._cloud;
                ...
                if (cloud)
                {
                    ...
                    assert(genCloud->size() > pickPoint.index);
                    pickPoint._cloud = genCloud;
                }
                else
                {
                    //we have a problem here ;)
            ...
            if (label) //correct label data
            {
                ...
                label->setPickedPoint(pickPoint);
            }
...
CC_FILE_ERROR BinFilter::LoadFileV1(QFile& in, ccHObject& container, unsigned nbScansTotal, const LoadParameters& parameters)
{
	...
	if (nbScansTotal > 99)
	{
		if (CS::Widgets::FramelessMessageBox::question(nullptr, QString("Oups"), QString("Hum, do you really expect %1 point clouds?").arg(nbScansTotal), QMessageBox::Yes, QMessageBox::No) == QMessageBox::No)
			return CC_FERR_WRONG_FILE_TYPE;
	}
