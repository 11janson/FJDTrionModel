							...
							ccMainAppInterface* app/*=nullptr*/)
{
	...
	for (ccHObject* obj : candidates)
	{
        ...
        const char* currentsfshown;
        if (isshowsf)
        {
            currentsfshown = cloud->getCurrentDisplayedScalarField()->getName();
        }
		...
		if (!success)
		{
            qDebug() << "start delete pcv Field";
			cloud->deleteScalarField(sfIdx);
            qDebug() << "end delete pcv Field";
            if (!isshowsf)
            {
                qDebug() << "set showcolors";
                cloud->showColors(true);
                cloud->showSF(false);
            }
            else
            {
                qDebug() << "set pcv setCurrentDisplayedScalarField";
                int currentIndex = cloud->getScalarFieldIndexByName(currentsfshown);
                if (currentIndex != -1)
                    cloud->setCurrentDisplayedScalarField(currentIndex);
                else
                {
                    //cloud->showColors(true);
                    //cloud->showSF(false);
            ...
            qDebug() << "PCV::Launch failed";
		}
		else
		{
            qDebug() << "PCV::Launch success start";
