...
//aric.tang_2022.12.5
const ccColor::Rgb* ccScalarField::getValueColorHigh(unsigned index)
{
    const ScalarType& val_pre = getValue(index);
    //int valPro = (int)(val_pre * 10.0);
    //float valPush = (float)valPro / 10.0;
    //float valInput;
    //if ((valPush - (float)(valPro / 10)) > 0.5)
    //{
    //    valInput = (float)((int)valPush + 1);
    //}
    //else
    //{
    //    valInput = (float)((int)valPush);
    //}
    const ccColor::Rgb* col = getColor(getValue(index));
    QColor unique_color;
    unique_color.setRed(col->r);
    unique_color.setGreen(col->g);
    unique_color.setBlue(col->b);
