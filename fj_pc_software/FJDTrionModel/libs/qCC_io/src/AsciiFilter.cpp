...
CC_FILE_ERROR AsciiFilter::saveToFile(ccHObject* entity, const QString& filename, const SaveParameters& parameters)
{
	...
	if (parameters.parentWidget)
	{
		...
		pDlg->setMethodTitle(QCoreApplication::translate("AsciiFilter", "Saving cloud", nullptr) 
            + (cloud->getName().length() > 30 ? "[" + cloud->getName().left(30) + "...]" : "[" + cloud->getName() + "...]"));
		pDlg->setInfo(QCoreApplication::translate("AsciiFilter", "Number of points:", nullptr) + QString::number(numberOfPoints));
		pDlg->start();
	}
	...
	if (s_saveColumnsNamesHeader)
	{
...

		{
			...
			if (writeSF)
			{
				...
				{
					QString sfName((*it)->getName());
					if (!(sfName.compare("intensity", Qt::CaseInsensitive) == 0 || sfName.compare("gpstime", Qt::CaseInsensitive) == 0))
					{
						sfName.replace(separator, '_');
						header.append(separator);
						header.append(sfName);
		...
		//cral20221116保存成pts文件修改字段顺序为xyz,intensity,color,gpstime,othersfs
										...
										LoadParameters& parameters)
{
	...
	bool isexistprogressdlg = false;
	if (parameters.progress)
	{
		isexistprogressdlg = true;
										...
										unsigned step = 1)
{
	...
	for (int i = 0; i < seqSize; ++i)
	{
		...
		case ASCII_OPEN_DLG_Scalar:
			...
			break;
		case ASCII_OPEN_DLG_INTENSITY:
			{
				int sfIndex = cloud->getNumberOfScalarFields() + 1;
				QString sfName = "Intensity";
			...
			break;
		case ASCII_OPEN_DLG_GPSTIME:
			{
				int sfIndex = cloud->getNumberOfScalarFields() + 1;
				QString sfName = "GpsTime";
			...
			break;
		case ASCII_OPEN_DLG_CLASSFICATION:
			{
				int sfIndex = cloud->getNumberOfScalarFields() + 1;
				QString sfName = "Classification";
															...
															bool showLabelsIn2D/*=false*/)
{
	...
	if (parameters.parentWidget)
	{
		...
		/*pDlg->setMethodTitle(QObject::tr("Open ASCII File"));
		pDlg->setInfo(QObject::tr("Approximate number of points: %1").arg(approximateNumberOfLines));*/
		npDlg->setMethodTitle(QCoreApplication::translate("AsciiFilter", "Open ASCII File", nullptr));
		npDlg->setInfo(QCoreApplication::translate("AsciiFilter", "Points", nullptr) + ":" + QString::number(approximateNumberOfLines));
		npDlg->setAutoReset(false);
		npDlg->setAutoClose(false);
		npDlg->start();
	}
	CCCoreLib::NormalizedProgress nprogress(npDlg, approximateNumberOfLines);
