...
void ccPickingHub::onActiveWindowChanged(QMdiSubWindow* mdiSubWindow)
{
	if (mdiSubWindow)
	{
		ccGLWindow* myWidget = qobject_cast<ccGLWindow*>(mdiSubWindow->widget());
		if (!myWidget)
		{
			return;
		}
...
void ccPickingHub::processPickedItem(ccHObject* entity, unsigned itemIndex, int x, int y, const CCVector3& P3D, const CCVector3d& uvw)
{
	...
	ccPickingListener::PickedItem item;
	{
		...
		if (entity && entity->isA(CC_TYPES::SPHERE))
		{
			...
			{
				//s_pickSphereCenter = CS::Widgets::FramelessMessageBox::question(m_activeGLWindow->asWidget(), tr("Sphere picking"), tr("From now on, do you want to pick sphere centers instead of a point on their surface?"), QMessageBox::YesToAll | QMessageBox::Yes | QMessageBox::No | QMessageBox::NoToAll, QMessageBox::YesToAll);
				s_pickSphereCenter = QMessageBox::Yes;
			}
