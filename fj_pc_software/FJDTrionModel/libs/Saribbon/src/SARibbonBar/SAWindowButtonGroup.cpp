...
public:
    ...
    void setupMinimizeButton(bool on)
    {
        ...
        if (on) {
            ...
            buttonMinimize->setFixedSize(30, 26);
            buttonMinimize->setFocusPolicy(Qt::NoFocus);  //避免铺抓到
            buttonMinimize->setIconSize(buttonMinimize->size() * 0.8);
			buttonMinimize->setIcon(QIcon(QCoreApplication::applicationDirPath() + "/theme/qssimage/minimize2x.png"));
            buttonMinimize->show();
    ...
    void setupMaximizeButton(bool on)
    {
        ...
        if (on) {
            ...
            buttonMaximize->setFixedSize(30, 26);
            buttonMaximize->setCheckable(true);
            buttonMaximize->setFocusPolicy(Qt::NoFocus);  //避免铺抓到
            buttonMaximize->setIconSize(buttonMaximize->size() * 0.8);
			buttonMaximize->setIcon(QIcon(QCoreApplication::applicationDirPath() + "/theme/qssimage/maximize@2x.png"));
            buttonMaximize->show();
    ...
    void setupCloseButton(bool on)
    {
        ...
        if (on) {
            ...
            buttonClose->setFixedSize(30, 26);
            ...
            buttonClose->setIconSize(buttonClose->size() * 0.8);
			buttonClose->setIcon(QIcon(QCoreApplication::applicationDirPath() + "/theme/qssimage/close.png"));
            buttonClose->show();
        } else {
            if (buttonClose) {
                delete buttonClose;
    ...
    void resize(QSize size)
    {
        ...
        if (buttonMinimize) {
            ...
            //buttonMinimize->setFixedSize(20, 20);
        ...
        if (buttonMaximize) {
            ...
            //受到qss样式影响会导致最小宽度、高度设置不了，因此要设置fixsize再move
            //buttonMaximize->setFixedSize(20, 20);
        ...
        if (buttonClose) {
            ...
            //受到qss样式影响会导致最小宽度、高度设置不了，因此要设置fixsize再move
            //buttonClose->setFixedSize(20, 20);
...
void SAWindowButtonGroup::maximizeWindow()
{
    ...
    if (par) 
    {
        int screenIndex = QApplication::desktop()->screenNumber(par);
        ...
        if (par->height() == availableGeometry.height()) {
            ...
            par->setGeometry(x, y, width, height);
            setMaxButtonDisplayState(true);
        } else {
            par->setWindowState(Qt::WindowMaximized);
            setMaxButtonDisplayState(false);
        }
        
    }
}

void SAWindowButtonGroup::setMaxButtonDisplayState(bool state)
{
    if (state)
    {
        m_d->buttonMaximize->setIcon(QIcon(QCoreApplication::applicationDirPath() + "/theme/qssimage/systembuttonmaximize@2x.png"));
    }
    else
    {
        m_d->buttonMaximize->setIcon(QIcon(QCoreApplication::applicationDirPath() + "/theme/qssimage/maximize@2x.png"));
    }
}