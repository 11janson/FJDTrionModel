...
bool PlyOpenDlg::isValid(bool displayErrors/*=true*/) const
{
	...
	{
		if (displayErrors)
			CS::Widgets::FramelessMessageBox::warning(nullptr, "Error", "At least two vertex coordinates (X,Y,Z) must be defined!");
		return false;
	}
	...
	for (int i = 1; i < n + p + q; ++i)
	{
		...
		{
			if (displayErrors)
				CS::Widgets::FramelessMessageBox::warning(nullptr, "Error", QString("Can't assign same property to multiple fields! (%1)").arg(xComboBox->itemText(i)));
			return false;
		}
	}
