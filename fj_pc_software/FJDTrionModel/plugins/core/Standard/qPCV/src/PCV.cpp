				 ...
				 const QString& entityName/*=QString()*/)
{
	...
	if (progressCb)
	{
		...
		{
			QString m_str = QCoreApplication::translate("PCV", "Shadow processing...", nullptr);
			...
			if (!entityName.isEmpty())
				infoStr = entityName + "\n";
			infoStr.append(QString(QCoreApplication::translate("PCV", "Light: %1. ", nullptr)).arg(numberOfRays));
			if (mesh)
				infoStr.append( QString(QCoreApplication::translate("PCV", "Faces: %1. ", nullptr)).arg(mesh->size()));
			else
				infoStr.append(QString(QCoreApplication::translate("PCV", "Vertex: %1. ", nullptr)).arg(numberOfPoints));
	...
	{
        qDebug() << "start pcv setViewDirection";
        ...
        qDebug() << "end pcv setViewDirection";

		if (success)
		{
            qDebug() << "pcv success";
            qDebug() << "start pcv convert per-vertex accumulators to an intensity scalar field";
            ...
            qDebug() << "end pcv convert per-vertex accumulators to an intensity scalar field";
		}
        else
            qDebug() << "pcv progressing break";

	}
	else
	{
        qDebug() << "PCVContext win failed";
		success = false;
	}
