	...
	: point_cloud(cloud)
{
	...
	params.cloth_resolution = 2;
	//布料刚性参数，平地为3，缓坡为2，陡坡为1
	params.rigidness = 3;
}
...
//CSF主程序 dofiltering
//aric.tang_2023/8/30_bool to int
int CSF::do_filtering(	std::vector<int>& groundIndexes,
						...
						QWidget* parent/*=nullptr*/)
{
    ...
    //static const float memoryLimitedValue = 10.0;//内存限制阈值10g
	...
	try
	{
		...
		//定义该参数阈值是为解决内存占用过大问题；已增加内存异常捕获，可去掉该限制
       /* float max_xyz = (bbMax.x - bbMin.x) > (bbMax.y - bbMin.y) ? (bbMax.x - bbMin.x) : (bbMax.y - bbMin.y);
        max_xyz = max_xyz > (bbMax.z - bbMin.z) ? max_xyz : (bbMax.z - bbMin.z);
        if (max_xyz / params.cloth_resolution > 800.0 && (point_cloud.size() > 20000000))
        {
            return -2;
        }*/
	...
	/*	pDlg.close();
		...
		//slope processing,true,默认执行坡度后处理
