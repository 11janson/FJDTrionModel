...
public:
	...
	enum PICKING_MODE { NO_PICKING,
						...
						LABEL_PICKING,
                        NOTE_PICKING,
                        POINT_PICKING_ENTITY,
						GRAPHIC_ELEMENT_PICKING,
						DEFAULT_PICKING,
	};
	...
	enum INTERACTION_FLAG {
		...
		//segment mode   //已关联密度测量功能
		MODE_SEGMENT = INTERACT_SEND_ALL_SIGNALS | INTERACT_ZOOM_CAMERA | INTERACT_PAN | INTERACT_2D_ITEMS,
		//SectionAnalysis mode
		MODE_SECTIONANALYSIS = INTERACT_SEND_ALL_SIGNALS | MODE_TRANSFORM_CAMERA,
	};
...
#endif
	//是否绘制左上角窗口标题
	void setIsDrawTitle(bool isDraw);

    //[!]获取跟根节点
    inline ccHObject* getGlwindowRootObject() {
        return m_winDBRoot;
    }

	void setIsNeedDrawSelectRect(bool isDraw);

	//设置视角是否锁定
	void SetViewLock(bool islocked);
	...
	void setviewer_for_trans(ccGLMatrixd currentMat, ccGLMatrixd& previous_mat);

	//replacement for the missing methods of QGLWidget
	void renderText(int x, int y, const QString & str, uint16_t uniqueID = 0, const QFont & font = QFont(), const ccColor::Rgba* backcolor = nullptr);
	...
	void displayText(QString text, int x, int y, unsigned char align = ALIGN_DEFAULT, float bkgAlpha = 0.0f, const ccColor::Rgba* color = nullptr, const QFont* font = nullptr, const ccColor::Rgba* backcolor = nullptr) override;
    ...
    QString windowName() { return m_strName; }
    void setWindowName(QString name) { m_strName = name; }
	...
	**/
	...
	//! Sets camera to a predefined view (top, bottom, etc.)
	virtual void setView(CCVector3d eyePos, CCVector3d upPos, bool redraw = true);
	...
	**/
    ...
    //清除二维制图框选
    void clearRect() { m_rect.clear(); redraw(); }
...
public: //LOD
	...
	**/
    ...
    void setShowZAxis(bool state) { m_isShowZAxis = state; }
...
public: //stereo mode
    ...
    *@brief 设置用户绘制的渲染树数据
    */
    void setDrawUserDB(ccHObject *pEntity);

    /**
    *@brief 是否开启点云可是
    */
    void setVisiblePointCloudEnable(bool bEnable) {m_bVisibleEnable = bEnable; }
public:
    void refreshCuttingBoxPickingStatus(QMouseEvent* event);

	//! Applies a 1:1 global zoom
	//默认不重置窗口，需要重置窗口的情况，添加参数true，carl.wang
	void zoomGlobal(bool isopen = false);
	...
	*@brief 绘制背景网格
	*/
	void setDrawBackgroundGridEnable(bool bEnable);

	/**
	*@brief 更新框选位置
	*/
	void updateCrossSelect(CCVector3d pt, bool isMoveing, bool bClear = true);

	/**
	*@brief 更新鼠标位置
	*/
	void updateGLWindowsMousePos(const int& x, const int& y, QVector3D& pos);
...
protected:
    ...
    //! Performs standard moving at the last mouse position (see m_lastMousePos)

signals:
    //[!]当点云被加载到winDBroot时，刷新界面的点大小设置
    void signalCloudWillAddDBRoot();
    //[!]重置视角的时候获取初始状态相机的位置
    void signalCramerInitialStatePos(CCVector3d);
    //[!]相机在垂直屏幕方向上的位置
    void signalCramerCurrentPos(CCVector3d);
    //[!]点击重置视图时刷新2d view窗口的视角
	//[!]state 为true 会刷新相机位置
    void signalsZoomGloba2DviewWindow(bool state = false);

	void sigTransScreenPosTo3DPos(CCVector3d pos, QPointF mousePos);

	void sigTransformClickPos(CCVector3d pos);


	void UpdatePosition(QString str);
	...
	**/
	...
	//右键鼠标释放（非拖动后释放）
	void rightButtonNoMovedRelease(int x, int y);

	//分割时位置改变
	void segmentPositionChanged(int x, int y);

	//分割时左键非拖动放开
	void leftButtonNoMovedRelease(int x, int y);

	//鼠标释放（非右键拖动后释放）
	void buttonNoRightMovedRelease();

	//窗口点击激活信号
	void activeSelectWindow();

    //图像融合FOV角度超过140退出锁定视图功能
    void signalExitFovLocked();
	...
	**/
    ...
    void leftButtonDoubleClicked(int x, int y);
    ...
    *@brief 鼠标
    */
    void signalMouseLeftDoubleClickEntity(CCVector3d P);

	//完整绘制一次
	void signalPainteOnce();
protected: //rendering
	...
	**/
    ...
    void drawCrossRect(CC_DRAW_CONTEXT& context);
    ...
    *@brief 绘制用户自己的绘制参数
    */
    void drawUserDB(CC_DRAW_CONTEXT& context, RenderingParams& params);
...
protected: //other methods
	...
	**/
    ...
    void calculatePointSizeByZoomRatio();
	...
	**/
    ...
    void startOpenGLMovingPicking(const PickingParameters& params, CCVector2d pos, bool needSelect = false);

    ccHObject* getPickingEntity(const PickingParameters& params,
                                std::unordered_set<int>& selectedIDs,
                                int& pickedItemIndex,
                                int& selectedID, bool is_movingPick = false);
    ...
    *@brief 设置注释拾取鼠标样式
    */
    void setPickCursor(bool bEable);

    /**
    *@brief 转化平面坐标系的点到3维空间中
    *param QPoint 二维平面坐标系的点
    *param 输出三维空间位置
    */
    bool covertScreenCoord3DPos(const QPoint &point, CCVector3d &p3d);

    /**
    *@brief 获取改点的临近空间三维点，领域范围8
    */
    bool covertScreenCoordNear3DPos(const QPoint &point, CCVector3d &p3d);

	/**
	*更新激活的
	*/
	void updateActiveGraphics(const QVector3D &pos);
...
protected: //members
	...
	PICKING_MODE m_pickingMode = PICKING_MODE::NO_PICKING;

	//修改默认选点方式后，用于选点模式缓存和读取
	PICKING_MODE m_cashPickingMode;

	//是否更新选点模式缓存
	bool m_isNeedUpdatePickingMode = false;
	...
	**/
    ...
    // last graphics screen pos
    CCVector2d m_lastGraphicsPos;
    ...
    //是否显示LOD效果
    bool m_isShowLodStyle = false;

    //lod定时器
    QTimer m_lodShowTimer;
	...
	bool m_isSectionAnalysisFirstPressIgnore = false;///<剖面分析第一次激活窗口后点击事件忽略

	bool m_Viewlock = false;

	bool m_isNeedDrawSelectRect = false;  ///<是否画选中的白色边框

	bool m_isDrawTitle = false;   ///<是否画左上角标题

    bool m_isRightClick = false;

    bool m_isLeftClick = false;

    bool m_isShowZAxis = true;

	QPointF m_leftClickPos = QPointF(-1,-1);  ///<用于判断左键点击时是否先移动
    ...
    unsigned int m_meshNormalShaderProgram = 0; //meshNormalShaderProgram
    unsigned int m_pointcloudNormalShaderProgram = 0; //pointcloudNormalShaderProgram
    unsigned int m_imageMeshShaderProgram = 0; //imageMeshShaderProgram
private:
    ccHObject  *m_pUserDBRoot = nullptr;
    bool        m_bVisibleEnable = true;
	bool		m_bMosuePickupMoveEnable = true;
	bool		m_bDrawBackgroundGridEnable = false;		///<绘制背景网格
