...
SARibbonPannel* SARibbonCategory::pannelByObjectName(const QString& objname) const
{
    for (SARibbonCategoryItem& i : m_d->mItemList) {
        if (i.pannelWidget) {
			//QString test = i.pannelWidget->objectName();
...
void SARibbonCategoryPrivate::insertPannel(int index, SARibbonPannel* pannel)
{
    ...
    item.separatorWidget->setObjectName("CategorySeparatorWidget");
    //插入list中
 ...
 *@brief 进入窗口 子类基础
 */
void SARibbonCategory::enterWindow()
{
    m_bEnterWindow = true;
    return;
}
...
*@brief 离开窗口 子类基础
*/
void SARibbonCategory::leaveWindow()
{
    m_bEnterWindow = false;
    emit signalLeveWindows();
    return;
}
...
void SARibbonCategory::setPannelsEnabled(bool state, QString pannelObjectName, QString actionObjectName, QString ignoreObjectName)
{
	for (SARibbonCategoryItem& i : m_d->mItemList) {
		if (i.pannelWidget) {
			if (i.pannelWidget->objectName() == ignoreObjectName)
				continue;

			if (pannelObjectName == "" || i.pannelWidget->objectName() == pannelObjectName) {
					i.pannelWidget->setActionsEnabled(state, actionObjectName, ignoreObjectName);
			}
		}
	}
