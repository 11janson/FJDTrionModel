                                        ...
                                        bool isPlaneCut/* = false*/)
{
	...
	if (pickWidth == pickHeight)
	{
		...
		//八叉树算起来太慢了，需要算法看原因，先注释carl.wang
		/*if (octree)
		{
	...
	if (ccbox)
	{
		...
		if (box)
		{
			try
			{
				selectionTable.resize(size());
			}
			catch (const std::bad_alloc&)
			{
				ccLog::Error("Not enough memory!");
			}
			box->flagPointsInside(this, &selectionTable);
		}
	}
	{
...
#pragma omp parallel for
...
#endif
		{
			...
			if (	((!visTable || visTable->at(i) == CCCoreLib::POINT_VISIBLE) || isPlaneCut)
				...
				)
			{
					...
					&&	std::abs(Q2D.y - clickPos.y) <= pickHeight)
				{
					...
					if (nearestPointIndex < 0 || squareDist < nearestSquareDist)
					{
						...
						{
							if ((i < selectionTable.size() && selectionTable[i] == CCCoreLib::POINT_VISIBLE) || isPlaneCut)
							{
