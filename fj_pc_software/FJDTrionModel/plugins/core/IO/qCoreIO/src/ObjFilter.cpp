...
CC_FILE_ERROR ObjFilter::saveToFile(ccHObject* entity, const QString& filename, const SaveParameters& parameters)
{
	...
	if (parameters.parentWidget)
	{
        ...
        pDlg->setMethodTitle(QCoreApplication::translate("PlyFilter", "Saving mesh", nullptr)
            + (mesh->getName().length() > 30 ? "[" + mesh->getName().left(30) + "...]" : "[" + mesh->getName() + "...]"));		pDlg->setInfo(QObject::tr("Writing %1 vertices").arg(nbPoints));
...
CC_FILE_ERROR ObjFilter::loadFile(const QString& filename, ccHObject& container, LoadParameters& parameters)
{
	...
	if (parameters.parentWidget)
	{
		pDlg.reset(new ccProgressDialog(true, parameters.parentWidget));
		pDlg->setWindowFlags(Qt::Window | Qt::FramelessWindowHint);
		pDlg->setMethodTitle(QObject::tr("Opening OBJ file"));
