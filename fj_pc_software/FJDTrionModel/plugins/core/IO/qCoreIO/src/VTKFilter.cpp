...
CC_FILE_ERROR VTKFilter::saveToFile(ccHObject* entity, const QString& filename, const SaveParameters& parameters)
{
    ...
    if(vertices)
	{
        ...
        if (parameters.parentWidget)
        {
            ...
            pDlg->setMethodTitle(QCoreApplication::translate("VTKFilter", "Saving cloud", nullptr) 
                + (vertices->getName().length() > 30 ? "[" + vertices->getName().left(30) + "...]" : "[" + vertices->getName() + "...]"));
            pDlg->start();
        }
        CCCoreLib::NormalizedProgress nprogress(pDlg.data(), ptsCount);
	...
	if (mesh)
	{
        ...
        if (parameters.parentWidget)
        {
            pDlg.reset(new ccProgressDialog(true, parameters.parentWidget));
            pDlg->setWindowFlags(Qt::Window | Qt::FramelessWindowHint);
            pDlg->setMethodTitle(QCoreApplication::translate("VTKFilter", "Saving mesh", nullptr) 
                + (mesh->getName().length() > 30 ? "[" + mesh->getName().left(30) + "...]" : "[" + mesh->getName() + "...]"));
            pDlg->setInfo(QCoreApplication::translate("VTKFilter", "Number of triangles:", nullptr) + QString::number(triCount));
            pDlg->start();
        }
	...
	while (error == CC_FERR_NO_ERROR)
	{
		...
		if (nextline.startsWith("POINTS"))
		{
            ...
            if (parameters.parentWidget)
            {
                ...
                pDlg->setWindowFlags(Qt::Window | Qt::FramelessWindowHint);
                pDlg->setMethodTitle(QCoreApplication::translate("VTKFilter", "Saving cloud", nullptr));
                pDlg->setInfo(QCoreApplication::translate("VTKFilter", "Number of points:", nullptr) + QString::number(ptsCount));
                pDlg->start();
            }
            CCCoreLib::NormalizedProgress nprogress(pDlg.data(), ptsCount);
		...
		else if (nextline.startsWith("POLYGONS") || nextline.startsWith("TRIANGLE_STRIPS"))
		{
            ...
            if (parameters.parentWidget)
            {
                ...
                pDlg->setWindowFlags(Qt::Window | Qt::FramelessWindowHint);
                pDlg->setMethodTitle(QCoreApplication::translate("VTKFilter", "Saving mesh", nullptr));
                pDlg->setInfo(QCoreApplication::translate("VTKFilter", "Number of triangles:", nullptr) + QString::number(elemCount));
                pDlg->start();
            }
            CCCoreLib::NormalizedProgress nprogress(pDlg.data(), elemCount);
