...
using CS::Widgets::FramelessDialog;
namespace ccEntityAction
{
	...
	{	
		...
		dlg.setWindowTitle(QCoreApplication::translate("MainWindow", "Height Ramp", nullptr));
		if (!dlg.exec())
			return false;
	...
	{	
		for (ccHObject* ent : selectedEntities)
		{
			if (ent->isA(CC_TYPES::MESH)/*|| ent->isKindOf(CC_TYPES::PRIMITIVE)*/) //TODO
			{
				...
				{
					if (	mesh->hasColors()
						&&	CS::Widgets::FramelessMessageBox::warning(	parent,
	...
	bool	enhanceRGBWithIntensities(const ccHObject::Container& selectedEntities, QWidget* parent)
	{
		...
		if (CS::Widgets::FramelessMessageBox::question(parent, QObject::tr("Intensity range"), QObject::tr("Do you want to define the theoretical intensity range (yes)\nor use the actual one (no)?"), QMessageBox::Yes, QMessageBox::No) == QMessageBox::Yes)
		{
	...
	bool sfConvertToRGB(const ccHObject::Container &selectedEntities, QWidget* parent)
	{
		...
		ccSFConvertToRGBDlg dlg(parent);
		dlg.setWindowTitle(QObject::tr("Scalar field to RGB"));
		if (dlg.exec())
		{
			ccSFConvertToRGBDlg::SFCONVERTTORGBDLGSTATE state = dlg.getResultStale();
			if (state == ccSFConvertToRGBDlg::SFCONVERTTORGBDLGSTATE::Accepted)
			{
				mixWithExistingColors = true;
			}
			else if (state == ccSFConvertToRGBDlg::SFCONVERTTORGBDLGSTATE::Canceled)
			{
				return false;
			}
		}
    ...
    bool sfSplitCloud(const ccHObject::Container& selectedEntities, ccMainAppInterface* app)
    {
        ...
        for (ccHObject* ent : selectedEntities)
        {
			...
			if (classes.size() > 30 && app && app->getMainWindow() && !tooManyCloudsQuestionAsked) // only in GUI mode
			{
				...
				if (QMessageBox::No == CS::Widgets::FramelessMessageBox::question(app->getMainWindow(), "Too many values", QString("Do you confirm that you want to create %1 clouds").arg(classes.size()), QMessageBox::Yes, QMessageBox::No))
				{
			...
			for (int pointClass : classes)
            {
				...
				try
				{
					...
					if (pc)
					{
						...
						//pc->setName("Classification" + QString::number(pointClass));
	...
	bool	computeNormals(const ccHObject::Container &selectedEntities, QWidget* parent)
	{
		...
		if (!meshes.empty())
		{
			CS::Widgets::FramelessMessageBox question(QMessageBox::Question,
