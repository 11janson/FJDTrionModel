...
QStringList HostOsInfo::hostIpv4Address()
{
	...
	for (int i = 0; i < ipAddressesList.size(); ++i) {
		if (ipAddressesList.at(i) != QHostAddress::LocalHost &&
			...
			strIpList.push_back(ipAddressesList.at(i).toString());
			break;
		}
	}
	// if we did not find one, use IPv4 localhost
	if (strIpList.isEmpty())
		strIpList.push_back( QHostAddress(QHostAddress::LocalHost).toString());
