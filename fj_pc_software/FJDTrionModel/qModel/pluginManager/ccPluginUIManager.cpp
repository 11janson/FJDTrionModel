...
void ccPluginUIManager::init()
{
	...
	for ( ccPluginInterface *plugin : plugins )
	{
		...
		switch ( plugin->getType() )
		{
			...
			case CC_GL_FILTER_PLUGIN: //GL filter
			{
				...
				{
					action = new QAction(QCoreApplication::translate("ccPluginUIManager", "EDL Shader", nullptr), this);
					actioncontainer["EDL Shader"] = action;
				}
				else if(pluginName == "SSAO Shader")
					action = new QAction(QCoreApplication::translate("ccPluginUIManager", "SSAO Shader", nullptr), this);
				else
					action = new QAction( pluginName, this );
				
				action->setToolTip( glPlugin->getDescription() );
				if (action->text() == QCoreApplication::translate("ccPluginUIManager", "EDL Shader", nullptr))
				{
					action->setIcon(QIcon(QCoreApplication::applicationDirPath() + "/theme/qssimage/EDLpng.png"));
				}
				else if (action->text() == QCoreApplication::translate("ccPluginUIManager", "SSAO Shader", nullptr))
				{
					action->setIcon(QIcon(QCoreApplication::applicationDirPath() + "/theme/qssimage/SSAOpng.png"));
				}
				else
				{
	...
	shaderAndFilterMenu()->setIcon(QIcon(QCoreApplication::applicationDirPath() + "/theme/qssimage/RemoveFillterpng.png"));
}
...
{
	m_actionRemoveFilter = new QAction( QIcon( ":/CC/images/noFilter.png" ), tr( "Remove" ), this );
	m_actionRemoveFilter->setEnabled( false );
	actioncontainer["Remove EDL"] = m_actionRemoveFilter;
...
{	
	...
	m_glFilterMenu = new QMenu( tr( "Shaders" ), m_parentWidget );
	m_pluginMenu->setObjectName("actionPlugins");
	m_glFilterMenu->setObjectName("actionShadersFilters");
...
void ccPluginUIManager::addActionsToMenu( ccStdPluginInterface *stdPlugin, const QList<QAction *> &actions )
{
	...
	if ( actions.size() > 1 )
	{
		...
		for ( QAction* action : actions )
		{
			...
			actioncontainer[action->text()] = action;
		}
	...
	{		
		...
		actioncontainer[actions.at(0)->text()] = actions.at(0);
		m_pluginMenu->addAction( actions.at( 0 ) );
	}
}

QAction * ccPluginUIManager::GetActionByName(QString name)
{
	auto iter = actioncontainer.find(name);
	if (iter != actioncontainer.end())
	{
		return iter->second;
	}
	else
...
void ccPluginUIManager::enableGLFilter()
{
	...
	shaderAndFilterMenu()->setIcon(action->icon());
