...
public:
    ...
    void setApplicationButton(QAbstractButton* btn)
    {
        ...
        if (btn) {
            ...
            applicationButton = btn;//janson.yang
            MainClass->connect(applicationButton, &QAbstractButton::clicked, MainClass, &SARibbonBar::applicationButtonClicked);
        }
        //applicationButton = btn;//janson.yang
    }
...
void SARibbonBar::showMinimumModeButton(bool isShow)
{
    if (isShow) {
        ...
        if (nullptr == m_d->minimumCategoryButton) {
            ...
            //action->setIcon(style()->standardIcon(isMinimumMode() ? QStyle::SP_TitleBarUnshadeButton : QStyle::SP_TitleBarShadeButton,0,m_d->minimumCategoryButton));
			action->setIcon(isMinimumMode() ? QIcon(":/image/resource/ArrowUp.png") : QIcon(":/image/resource/ArrowDown.png"));
    ...
    if (parWindow) {
        ...
        if (contextCategoryRegion.y() < 0) {
            ...
            if (rightwidth > leftwidth) {
                ...
                titleRegion.setRect(contextCategoryRegion.y(), widgetBord().top()-4, rightwidth, titleBarHeight()-4);
            } else {
                //说明左边的大一点
...
void SARibbonBar::paintInWpsLiteStyle()
{
    ...
    if (parWindow) {
        ...
        if (width > 20) {
            QRect titleRegion(start, widgetBord().top()-2, width, titleBarHeight()-2);
#ifdef SA_RIBBON_DEBUG_HELP_DRAW
            p.save();
            p.setBrush(QColor(255, 0, 0, 120));
            p.drawRect(titleRegion);
...
void SARibbonBar::resizeInOfficeStyle()
{
    ...
    if (m_d->quickAccessBar) {
        if (m_d->quickAccessBar->isVisible()) {
            ...
            m_d->quickAccessBar->setGeometry(x, y, quickAccessBarSize.width() - 13, validTitleBarHeight);
        }
    }
...
void SARibbonBar::paintWindowTitle(QPainter& painter, const QString& title, const QRect& titleRegion)
{
	...
	QRect thisRect = titleRegion;
	//thisRect.setX(thisRect.x() - 10);
    painter.setPen(m_d->mTitleTextColor);
    painter.drawText(thisRect, Qt::AlignTop | Qt::AlignLeft , title);
    painter.restore();
}

void SARibbonBar::setCategorysEnabled(bool state, QString categoryObjectName, QString pannelObjectName, QString actionObjectName, QString ignoreObjectName)
{
	int c = m_d->stackedContainerWidget->count();
	...
	for (int i = 0; i < c; ++i) {
		...
		if (w) {
			...
			if (w->objectName() == ignoreObjectName)
				continue;

			if (categoryObjectName == "" || w->objectName() == categoryObjectName) {
				w->setPannelsEnabled(state, pannelObjectName, actionObjectName, ignoreObjectName);
			}
