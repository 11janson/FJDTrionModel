...
#define CRASHANDLER_MAX_PATH 2048
...
crashFunCallbackType pCrashCallback = nullptr;
...
#endif
    QString strPath = QString("%1").arg(crashReporterPath);
    if (pCrashCallback) {
        pCrashCallback(strPath);
    }
    ...
    const QString& crashReportDir)
{
    ...
    wcscpy(crashReporterPath, crashReportDir.toStdWString().c_str());
...
void CrashHandler::Init( const QString& reportPath,  
    const QString& appPath,
    const QString& crashReportDir,
    crashFunCallbackType pCallBack)
{
    pCrashCallback = pCallBack;
    d->InitCrashHandler(reportPath, appPath, crashReportDir);
}
