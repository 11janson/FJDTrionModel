                                ...
                                unsigned pointCount/*=0*/)
{
		...
		if (pc && pc->hasScalarFields())
		{
			...
			try
			{
				...
				for (unsigned i = 0; i < sfCount; ++i)
				{
					...
					scalarFieledsName.push_back(pc->getScalarFieldName(i));
				}
			}
			catch (const std::bad_alloc&)
			{
    ...
    if (pointCount == 0)
    {
        ...
        if (progressDialog)
        {
            progressDialog->setMethodTitle(QApplication::translate("ccRasterGrid", "Generating the grid...", nullptr));
    ...
    else
    {
        if (progressDialog)
        {
            curProcessValue = progressDialog->value();
            progressDialog->setMethodTitle(QApplication::translate("ccRasterGrid", "Generating the grid...", nullptr));
            QCoreApplication::processEvents();
        }
    } 
    ...
    if (curProcessValue != -1)
        nProgress.steps(pointCount - cloud->size());
...
bool ccRasterGrid::interpolateEmptyCells(double maxSquareEdgeLength)
{
	...
	for (unsigned k = 0; k < triNum; ++k)
	{
		...
		//now scan the cells
		{
            ...
            //自定义投影平面时，需要根据cell中的点索引得到点坐标，计算到自定义投影平面的距离；所以内插空cell时，应该给其pointIndex也赋值。
            //（最准确的应该是取三个cell的点坐标内插给该空cell，但是cell结构中只有索引pointIndex，没有点坐标；
            //退而求其次，取第一个cell内的点索引赋值给空cell的点索引,三个cell取哪个都行，都不是最准确的，但是是最接近的）
            const unsigned int& pointIndex = rows[P[0].y][P[0].x].pointIndex;
