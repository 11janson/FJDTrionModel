...
void ccHObject::drawNameIn3D(CC_DRAW_CONTEXT& context)
{
	if (context.display && m_nameIn3DPosIsValid)
	{
		...
		if (getShowvolumn())
		{
			double volumn = name.toDouble();
			QStringList m_volumelist = (QStringList() << QObject::tr("cubic meter") << QObject::tr("cubic decimeter") << QObject::tr("cubic centimetre") << QObject::tr("cubic millimetre") << QObject::tr("cubic foot") << QObject::tr("cubic inch"));
			...
			if (context.isshowUnitlabel)
			{
				...
				name = QString("Volumn:") + QString::number(volumn * m_volumnunitshift[context.volumnunit], 'f', context.dispNumberPrecision) + unit;
			}
			else
			{
				name = QString("Volumn:") + QString::number(volumn * m_volumnunitshift[context.volumnunit], 'f', context.dispNumberPrecision);
			}
		}
		QFont titleFont = context.display->getLabelDisplayFont();
		ccColor::Rgba defaultTextColor = context.textDefaultCol;
		ccColor::Rgba labelbackgColor = context.labelDefaultBkgCol;
		context.display->displayText(name,
			static_cast<int>(m_nameIn3DPos.x),
			static_cast<int>(m_nameIn3DPos.y),
			ccGenericGLDisplay::ALIGN_HMIDDLE | ccGenericGLDisplay::ALIGN_VMIDDLE,
			context.labelOpacity / 100.0f,
			&ccColor::black,
			&titleFont,&labelbackgColor);
	}
}
...
void ccHObject::setSelected(bool state) {
	m_selected = state;
	if (m_selected)
	{
		FJStyleManager::Instance()->areaMeasureLabelSelected(getUniqueID());
	}
}